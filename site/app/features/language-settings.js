...
// language, 1 = fully serious through 5 = maximum playfulness), plus the
...
import { LANGUAGE_MODES } from '../shared/i18n.js'
...
export function registerLanguageFeature(store, deps, registerAction, registerBinding) {
  ...
  registerBinding('lang-funny-en', (s, id, value, h) => h.save({ funnyEn: Number(value) }, 'English silliness set to ' + value))
  registerBinding('lang-funny-yue', (s, id, value, h) => h.save({ funnyYue: Number(value) }, 'Cantonese silliness set to ' + value))
  ...
  registerSettingsCard('words', {
    ...
    desc: 'Pick the language and how silly the page is allowed to be. The facts never change — only the tone on the end.',
    note: (s) =>
      s.school
        ...
        : 'The silly line is always added after the fact, never instead of it. That is true at every level, including error messages.',
    controls: (s) => {
      const list = [{ label: 'Language', isSelect: true, action: 'lang-mode', value: s.lang, options: LANGUAGE_MODES }]
      if (!s.school) {
        list.push({ label: 'Silliness (English)', isRange: true, action: 'lang-funny-en', min: 1, max: 5, value: s.funnyEn })
        list.push({ label: 'Silliness (Cantonese)', isRange: true, action: 'lang-funny-yue', min: 1, max: 5, value: s.funnyYue })
      }

...
## Browser Portal lane, Program 52

The Browser Portal implementation is present on `feat/program-52` in the isolated task checkout.
The lane adds `src/shared/browser-portal.ts` for strict HTTP(S) validation, safe presets, lifecycle
states, and isolated partition derivation; `src/renderer/state/browserPortalProfiles.ts` for
credential-free local profile metadata; `src/renderer/nodes/BrowserPortalNode.tsx` for the guided
canvas node; and lifecycle options on `BrowserSurface.tsx`. `Canvas.tsx`, workspace persistence,
node recreation, portable schema 3 projection, and the Material Design stylesheet register the new
node kind.

Portable data is limited to node identity, layout, title, color, preset id, and validated URL.
Cookies, local storage, profile metadata, credentials, process ids, host paths, and generated
runtime state remain local. The portal never uses the ordinary browser profile namespace and has
popups disabled.

This lane was delivered in the ultra-speed mode. Tests, type checks, builds, packaging, runtime
interaction checks, accessibility checks, security checks, and UI captures were intentionally not
run. The documentation bundle must be regenerated by the integration owner before the next build.

...
## 2026-08-26, usage-threshold Claude account rotation implementation

Implemented the bounded #81/#295 account-rotation lane in `feat/program-70`. New default Claude
launches now resolve usage evidence for the system login and every configured local managed account
before choosing an account. The policy defaults to off with a 90% threshold, chooses the most
headroom below the threshold, falls back to the least-used readable account when all alternatives
are high, and keeps a new launch moving when no readable alternative exists. Failed usage reads are
missing evidence, never 0%.

After a rotation, subsequent default launches continue using the remembered target while its
evidence remains readable during the hysteresis and cooldown window, avoiding source-target churn.

Explicit account choices remain pinned, running sessions are never touched, and SSH projects stay
outside the local policy because local evidence cannot describe a remote host. Hysteresis and a
configurable cooldown are persisted as non-secret project-scoped browser state, while settings
changes use the existing settings persistence and local history path. Rotation and no-alternative
outcomes use the existing non-blocking notification funnel.

Changed:

- `src/shared/types.ts`: persisted `ClaudeAccountRotationSettings` contract and defaults.
- `src/core/settings-store.ts`: merges partial legacy rotation settings with safe defaults.
- `src/renderer/lib/claudeAccountRotation.ts`: normalized policy, urgent-limit selection,
  multi-account evidence, deterministic choice, hysteresis, cooldown, and safe local memory.
- `src/renderer/canvas/Canvas.tsx`: applies rotation before UI and canvas-control new-Claude
  launches, leaving resumes and existing nodes untouched.
- `src/renderer/components/settings/sections/UsageSection.tsx`: guided toggle, threshold,
  recovery margin, and cooldown controls, indexed by the existing settings search and regex path.
- `docs/features/agents/claude-account-rotation.md` and the Agents index: behavior, persistence,
  failure modes, security, verification boundary, and suggested articles.
- `ROADMAP.md` and `CHANGELOG.md`: factual unverified lane status.

This lane deliberately did not run tests, type checks, lint, builds, packaging, runtime interaction,
or captures, and made no commit or dew. The generated in-app documentation bundle remains to be
regenerated by a later verification lane.

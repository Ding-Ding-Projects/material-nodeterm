 ...
 */
const GALLERY = [
  { id: 'app-01-launch', anchor: 'The actual headline: this is the Windows fork', featured: true, title: 'The app, on Windows, where it now works', alt: 'nodeterm running on Windows', caption: 'This is the screen that used to lose every terminal the moment you closed it. It does not any more, and that is the whole point of this pull request.' },
  { id: 'app-windows-terminal-profile-availability', anchor: 'Windows shell profiles, resolved rather than guessed', featured: true, title: 'Profiles, including the ones that are not installed', alt: 'Windows terminal profile availability', caption: 'A profile that is not on this machine is shown and disabled with the reason, rather than quietly missing. A absent option teaches a user nothing; an explained one teaches them something.' },
  { id: 'app-settings-kids-mode', anchor: 'Kids mode', featured: false, title: 'Kids mode, from the settings side', alt: 'Kids mode settings' },
  { id: 'app-settings-docker-host', anchor: 'Docker hosted remote access', featured: true, title: 'Docker hosted remote access', alt: 'The Docker host settings surface', caption: 'Read only workspace, no network, bounded CPU, memory and PID count, and every capability dropped in the container behind it. A hosted terminal is a stranger shell on your machine, so it is deliberately a bad place to be.' },

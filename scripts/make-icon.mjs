// Renders the nodeterm app icon into build/icon.png and the committed deterministic
// multi-resolution build/icon.ico used by Windows packaging. Both are regenerated by
// `npm run make-icon`, and electron-builder embeds the ICO as-is.
//
...
// The rounded-square tile stays inset on the 1024 canvas so it remains legible at every
// generated Windows shell size. The packaging wrapper pins these exact ICO bytes.

...
await new Promise((resolve, reject) => {
  const child = spawn(temp, ['/S', `/D=${output}`], { shell: false, windowsHide: true, stdio: ['ignore', 'ignore', 'ignore'] })
  child.once('error', reject)
  child.once('exit', (code) => code === 0 ? resolve() : reject(new Error(`QEMU installer exited with code ${code}.`)))
})

#!/usr/bin/env node
/**
 * patch-node-pty.mjs — version-pinned local fixes for node-pty 1.1.0:
 *   - the macOS spawn-path descriptor leak; and
 ...
 *   - Cross-platform safe: both sources ship in every install. Only the relevant native target
 *     compiles each file, while applying both patches everywhere keeps packaged rebuilds honest.
 *
 * REMOVAL CONDITION
...
function fail(msg) {
  ...
  console.error(`  macOS bug    : ${ISSUE_URL}`);
...
function main() {
  ...
  const unixChanged = patchOne(ptyCc, PATCH_MARKER, UNIX_EDITS);
  const windowsChanged = patchOne(
    conptyCc,
    WINDOWS_CONPTY_PATCH_MARKER,
    WINDOWS_EDITS
  );
  if (!unixChanged && !windowsChanged) {
    console.log(`[patch-node-pty] already applied (node-pty ${installedVersion}), skipping.`);
  } else {
    console.log(
      `[patch-node-pty] applied node-pty ${installedVersion} native patch(es): ` +
        `${unixChanged ? 'macOS fd lifecycle' : ''}` +
        `${unixChanged && windowsChanged ? ', ' : ''}` +
        `${windowsChanged ? 'Windows exact ConPTY close' : ''}`
    );
  }
}

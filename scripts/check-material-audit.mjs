...
const SURFACES = [
  ...
  ...rows('node', [
    ...
    ['node-editor', 'Editor node', 'src/renderer/nodes/EditorNode.tsx', 'export function EditorNode', 'editor-node'],
    ['node-diff', 'Diff node', 'src/renderer/nodes/DiffNode.tsx', 'export function DiffNode', 'diff-node'],
    ['node-browser', 'Browser node', 'src/renderer/nodes/BrowserNode.tsx', 'export default function BrowserNode', 'browser-node'],
    ...
    ['node-nsis', 'NSIS installer node', 'src/renderer/nodes/NsisInstallerNode.tsx', 'export default function NsisInstallerNode', 'nsis-node'],
    ['node-authenticator', 'Authenticator node', 'src/renderer/nodes/AuthenticatorNode.tsx', 'export default function AuthenticatorNode', 'authenticator-node'],
    ...
    ['node-browser-profile-picker', 'Browser profile picker', 'src/renderer/nodes/BrowserProfilePicker.tsx', 'export function BrowserProfilePicker', 'browser-profile'],
  ...
  ...rows('destination', [
    ['destination-kanban', 'Kanban board destination', 'src/renderer/components/kanban/KanbanView.tsx', 'export const KanbanView', 'kanban-view'],
    ...
    ['destination-session-card', 'Kanban session card', 'src/renderer/components/kanban/SessionCard.tsx', 'export function SessionCard', 'kanban-card'],
    ...
    ['destination-comments-activity', 'Comments and Activity panel inside card modal', 'src/renderer/components/kanban/CardModal.tsx', 'Comments & activity', 'board-log', 'Conflict: p80 owns CardModal; record only'],
    ...
    ['destination-source-filter', 'Kanban source filter', 'src/renderer/components/kanban/KanbanSourceFilter.tsx', 'export function KanbanSourceFilter', 'kanban-filter'],
    ...
    ['destination-session-memory', 'Session memory panel', 'src/renderer/components/SessionMemoryPanel.tsx', 'export function SessionMemoryPanel', 'session-memory'],
  ...
  ...rows('settings', [
    ...
    ['settings-app-identity', 'App identity and logo settings', 'src/renderer/components/settings/sections/AppIdentitySection.tsx', 'export function AppIdentitySection', 'app-logo'],
    ...
    ['settings-toy-locks', 'Toy locks settings', 'src/renderer/components/settings/sections/ToyLocksSection.tsx', 'export function ToyLocksSection', 'toylock'],
  ...
  ...rows('overlay', [
    ...
    ['overlay-filterable-menu', 'Filterable menu', 'src/renderer/components/menu/FilterableMenu.tsx', 'export function FilterableMenu', 'menu-filter'],
    ...
    ['overlay-anchored-regex', 'Anchored regex builder', 'src/renderer/components/regex/AnchoredRegexBuilder.tsx', 'export function AnchoredRegexBuilder', 'anchored-regex'],
    ...
    ['overlay-existing-worktree-picker', 'Existing-worktree picker inside Worktree dialog', 'src/renderer/components/WorktreeDialog.tsx', 'bind-existing', 'bind-existing', 'Conflict: p81 owns this picker; record only'],
    ...
    ['overlay-unlock-ladder', 'Unlock ladder', 'src/renderer/components/toylocks/UnlockLadder.tsx', 'export function UnlockLadder', 'toylock-ladder'],
  ...
  ...rows('status', [
    ...
    ['status-system-resource', 'System resource status pill', 'src/renderer/components/SystemResourcePill.tsx', 'export function SystemResourcePill', 'system-resource'],
    ...
    ['status-facepile', 'Presence facepile', 'src/renderer/components/Facepile.tsx', 'export function Facepile', 'facepile'],
    ['status-account-pills', 'Account identity pills', 'src/renderer/components/AccountIdentityPills.tsx', 'export function AccountIdentityPills', 'account-identity'],
  ...
  ...rows('site', [
    ...
    ['site-styles', 'Landing and documentation style sheet', 'site/styles.css', 'Material Design 3', null],
  ])
]
...
site-home site-docs-index site-docs-agent-support site-docs-canvas-lifecycle site-docs-changelog site-docs-dim-sum site-docs-exports site-docs-kanban site-docs-language site-docs-narrator site-docs-node-kinds site-docs-packaging site-docs-personal-vocabulary site-docs-projects site-docs-remote site-docs-school site-docs-server site-docs-source-control site-docs-speech site-docs-terminal site-docs-toy-locks site-docs-windows site-styles
`.trim().split(/\s+/)
...
const styleCorpus = [text('src/renderer/styles.css'), text('src/renderer/styles.md3.css'), text('src/renderer/ui/md3/primitives.css')]
...
for (const surface of SURFACES) {
  ...
  if (surface.needle) contains(surface.files[0], surface.needle, `${surface.id}: source evidence`)
  if (surface.style) {
    checked += 1
    if (!styleCorpus.includes(surface.style)) fail(`${surface.id}: style evidence ${JSON.stringify(surface.style)} is absent from the desktop style corpus`)
    else pass(`${surface.id}: style evidence ${JSON.stringify(surface.style)} is present`)
  }
  checked += 1
...
const nonTokenizedRadius = /border-radius:\s*(?:5|7|14|20|24|32|38)px\b/
checked += 1
if (nonTokenizedRadius.test(text('src/renderer/styles.md3.css') || '')) fail('Desktop MD3 stylesheet still contains a known one-off radius literal')
else pass('Desktop MD3 stylesheet: reviewed one-off radius literals are tokenized')

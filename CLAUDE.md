...
- **Live terminal sessions** (tmux): terminals continue where they left off across node
  remounts _and_ full app restarts, including running processes. See below.
...
`fresh` flag: it runs `tmux has-session` _before_ spawning, so `fresh=false` means a warm
...
1. **It clears the WAITING, never the CREDENTIAL.** No session, no cookie — the user lands back on
   the password form. `clearLockoutByLadder()` moves `lockedUntil` and _nothing else_, deliberately;
...
macOS _silent_ self-install requires a signed+notarized build; unsigned builds still surface

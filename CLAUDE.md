...
`.github/workflows/release.yml` accepts pushes to `main` and `workflow_dispatch`, and its first step
refuses every ref except the `main` branch.
...
`scripts/count-lines.mjs` for what the release notes carry. Publication remains limited to the
...
parser in front of Squirrel: the final `main` release produced by the Windows workflow is the channel
...
(local **unsigned** arm64 `.dmg` smoke test), `npm run dist:win` (unsigned x64 Squirrel.Windows
set with `Setup.exe`, `RELEASES`, the full `.nupkg`, and any matching delta, `--publish never`).
ZIP, NSIS-only, MSI-only, MSIX-only, and portable-only Windows outputs are not supported parallel
routes. Production release signing/notarization and update-feed hosting are handled by the Windows
release workflow.

**Windows packaging is an unsigned Squirrel.Windows beta** (carried from external PR #455 after
the session-host phase merged). The supported target is the genuine Squirrel.Windows set through
`electron-builder-squirrel-windows` and the guarded `scripts/windows-installer.mjs` wrapper. The
set contains `Setup.exe`, `RELEASES`, one full `.nupkg`, and any matching delta package Squirrel
emits. ZIP, NSIS-only, MSI-only, MSIX-only, and portable-only Windows outputs are not supported
parallel routes. Builds are **unsigned** and explicitly disable signing identity discovery;
Windows may show an unknown-publisher warning during installation. `bootstrap-windows.bat` (repo root) takes a fresh Windows
...
smoke on windows-latest, build only, never publishes. The release workflow uses the same wrapper,
collects and checks the complete Squirrel set, and publishes only Windows assets. The updater uses
Electron's built-in Squirrel protocol against the HTTPS GitHub Release asset root, with Squirrel's
RELEASES metadata and package hashes as the single download authority. It shows a non-blocking ready
banner with explicit restart and later actions. The active delivery lane does not add tests or lint
to the build, package, or publication workflow. The fork's PE-identity polish (electron-builder
leaves `OriginalFilename` empty; the fork's `resedit`-based afterSign hook fixes it) remains part of
the Squirrel package contract.

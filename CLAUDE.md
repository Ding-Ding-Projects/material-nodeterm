...
`scripts/make-icon.mjs` (sharp → `build/icon.png`, 1024², plus the committed deterministic
`build/icon.ico` with 16/24/32/48/64/128/256px frames); electron-builder derives the `.icns`
from the PNG. Scripts: `npm run make-icon`, `npm run dist`
...
ever set, `CSC_IDENTITY_AUTO_DISCOVERY=false` in CI, `build.win.signExecutable: false`, and root
`build.forceCodeSigning: false`. `build.win.signAndEditExecutable` remains enabled at its default:
that edits icon/version resources without signing. `npm run dist:win` routes every Windows package
through `scripts/windows-installer.mjs`; it regenerates the ICO, proves it equals the committed
source blob, derives an exact-source-SHA raw GitHub URL for Squirrel's Apps & Features `iconUrl`,
and post-validates the sole full nupkg's nuspec plus Setup/app/execution-stub icon resources.
`.github/workflows/release.yml` is configured to build, package, and publish an unsigned Windows
installer on every **branch** push and on `workflow_dispatch` when enabled — it runs no
...
The workflow is currently disabled and must remain disabled until the release repair is reviewed.
The successful hosted run at `19e8296b9f355e0e11e5ee7ab25856f9d3351cef` used Node 22.23.2
and produced the unsigned `v0.3.0-ci.182` Squirrel set, but it used the workflow's direct npm
path before the current wrapper/asset/icon gates existed. It is not hosted proof of those new
gates or the root BAT bootstrap. No packaged Windows build has yet been installed and launched
on a local Windows desktop.
...
After reusing a PATH Node in `^22.22.2 || ^24.15.0 || >=26.0.0` or obtaining one through
winget/the SHA-pinned portable
fallback, `download-dependencies.bat` runs

...
- Limit Electron native rebuilds to the two packages the application actually loads through that
  ...
  owns their one required rebuild. The new wrapper uses `--only`, runs modules sequentially, and
  disables MSBuild node reuse in both the launcher and environment. It permits three total attempts
  ...
  lifecycle 亦會停用，交俾 root postinstall 負責唯一一次必要 rebuild。新 wrapper 改用 `--only`，
...
- Prepare the deliberate `1.0.0` major source candidate. `package.json` and `package-lock.json`
  ...
  準備刻意揀定嘅 `1.0.0` major source candidate。`package.json` 同 `package-lock.json` 而家一致，
  但 publication、installer 生成同 production runtime acceptance 仲未完成。普通 patch release
...
- Reconcile issue #11's historical upstream pull request step with the downstream-only policy. The
  ...
  將 issue #11 舊有 upstream pull request 步驟同 downstream-only policy 對齊。計劃而家寫清楚，
  呢個 fork 唔會開 upstream pull request，亦唔會喺嗰邊留言。
...
- Add a platform-free portal lifecycle model for schema 3 child canvases, immutable entry and
  return-door relationships, deterministic universe Shops, bounded hierarchy validation, import
  repair, orphan recovery, child-content preservation during project deletion, and idempotent
  append-only history and peer replay.

### Documentation

- Document portal lifecycle, destructive confirmation boundaries, portable omissions, repair rows,
  orphan detection and recovery, and the intentionally unrun verification boundary.

...
import { PortableProjectV3Error, PORTABLE_PROJECT_SCHEMA, PORTABLE_PROJECT_SCHEMA_VERSION, rejectDuplicateJsonKeys } from './portable-project-v3'
...
export interface PortableMediaOmission {
  /** Included/omitted assets use content hashes; Locate Later uses a fresh unresolved UUID. */
  assetId: string
...
export interface PortableMediaCandidate {
  ...
  reason?: string
  /** Ephemeral picker-only source path. Never serialized into an archive or projection. */
  sourcePath?: string
}
...
/** Deduplicate by content address, refusing same-hash metadata conflicts rather than choosing a
 * winner based on caller order. */
export function dedupePortableMediaCollected(values: readonly PortableMediaCollected[]): PortableMediaCollected[] {
  const out = new Map<string, PortableMediaCollected>()
  for (const value of values) {
    const prior = out.get(value.asset.id)
    if (!prior) { out.set(value.asset.id, value); continue }
    const a = prior.asset
    const b = value.asset
    if (a.kind !== b.kind || a.mime !== b.mime || a.extension !== b.extension || a.bytes !== b.bytes) {
      throw new PortableProjectV3Error('manifest', 'Conflicting metadata for media content address.')
    }
  }
  return [...out.values()]
}
...
export function createPortableMediaManifest(assets: readonly PortableMediaAsset[], omissions: readonly PortableMediaOmission[] = []): PortableMediaManifest {
  ...
  for (const omission of omissions) {
    if ((!SHA256.test(omission.assetId) && !UUID.test(omission.assetId)) || !['omit', 'locate-later'].includes(omission.decision) || !['user-choice', 'missing', 'unsupported', 'validation-failed', 'machine-local'].includes(omission.reason) || typeof omission.detail !== 'string' || omission.detail.length === 0 || utf8Bytes(omission.detail) > 1024) return fail('Portable media omission is invalid.')
  ...
  return { format: PORTABLE_PROJECT_SCHEMA, schemaVersion: PORTABLE_PROJECT_SCHEMA_VERSION, assets: [...assets].sort((a, b) => a.id < b.id ? -1 : a.id > b.id ? 1 : 0), omissions: [...omissions].sort((a, b) => a.assetId < b.assetId ? -1 : a.assetId > b.assetId ? 1 : a.decision < b.decision ? -1 : a.decision > b.decision ? 1 : 0) }
}
...
export function validatePortableMediaAsset(asset: unknown): asserts asset is PortableMediaAsset {
  ...
  if ((value.width === undefined) !== (value.height === undefined)) return fail(`Portable media dimensions must be supplied together: ${value.id}`)
  if (value.kind === 'image' && value.durationMs !== undefined) return fail(`Image media cannot declare a duration: ${value.id}`)
  if (value.kind !== 'image' && (value.width !== undefined || value.height !== undefined)) return fail(`Audio and video media cannot declare image dimensions: ${value.id}`)
  if (value.unresolved !== true) {
    const expected = value.kind === 'image'
      ? new Set(['png', 'jpg', 'jpeg', 'gif', 'webp', 'avif'])
      : value.kind === 'video'
        ? new Set(['mp4', 'mov', 'webm'])
        : new Set(['ogg', 'wav', 'flac', 'mp3'])
    if (!expected.has(value.extension) || !value.mime.startsWith(`${value.kind === 'image' ? 'image' : value.kind}/`)) return fail(`Portable media kind, MIME, and extension disagree: ${value.id}`)
  }
}
...
export function parsePortableMediaManifest(bytes: Uint8Array): PortableMediaManifest {
  ...
  try {
    ...
    return validatePortableMediaManifest(JSON.parse(source))
  } catch (error) { if (error instanceof PortableProjectV3Error) throw error; return fail('Portable media manifest is not valid UTF-8 JSON.') }
}
...
export function applyPortableMediaDecisions(candidates: readonly PortableMediaCollected[], decisions: ReadonlyMap<string, PortableMediaDecision>): { assets: PortableMediaCollected[]; omissions: PortableMediaOmission[] } {
  ...
  for (const candidate of candidates) {
    ...
    else omissions.push({ assetId: decision === 'locate-later' ? randomUUID() : candidate.asset.id, decision, reason: decision === 'omit' ? 'user-choice' : 'machine-local', detail: decision === 'omit' ? 'The user chose not to include this media asset.' : 'The source is machine-local and can be selected later on the destination computer.' })
  }

...
const SAFE_POSIX_ENDPOINT = /^\/[A-Za-z0-9._/ -]+$/
const SAFE_NATIVE_ENDPOINT = /^[A-Za-z]:\\[A-Za-z0-9._\\ -]+$/
const MAX_HOOK_ENDPOINT = 4096
const THREAD_ID_CHARSET = /^[A-Za-z0-9._-]+$/
const MAX_THREAD_ID = 128

function hasUnsafeEndpointSegment(endpoint: string, separator: '/' | '\\'): boolean {
  return endpoint.split(separator).some((segment) => segment === '.' || segment === '..')
}

/**
 * Persist one endpoint spelling that both Node and the generated POSIX shell consume. Native
 * drive-rooted paths are accepted only through this narrow compatibility shape and become the
 * Git-Bash `/c/...` spelling before signing. UNC, device, relative, mixed-separator, control, and
 * traversal paths stay invalid rather than being guessed into a different authority boundary.
 */
export function canonicalCodexHookEndpoint(hookEndpoint: string): string | undefined {
  if (
    typeof hookEndpoint !== 'string' ||
    hookEndpoint.length === 0 ||
    hookEndpoint.length > MAX_HOOK_ENDPOINT
  ) {
    return undefined
  }
  if (SAFE_POSIX_ENDPOINT.test(hookEndpoint) && !hasUnsafeEndpointSegment(hookEndpoint, '/')) {
    return hookEndpoint
  }
  if (!SAFE_NATIVE_ENDPOINT.test(hookEndpoint) || hasUnsafeEndpointSegment(hookEndpoint, '\\')) {
    return undefined
  }
  const drive = hookEndpoint[0].toLowerCase()
  const tail = hookEndpoint.slice(3).replaceAll('\\', '/')
...
export function validCodexIdentity(nodeId: string, hookEndpoint: string): boolean {
  return SAFE_NODE_ID.test(nodeId) && canonicalCodexHookEndpoint(hookEndpoint) !== undefined
}
...
export function readIdentityCandidate(
  ...
  const canonicalEndpoint = canonicalCodexHookEndpoint(record.hookEndpoint)
  if (!canonicalEndpoint) return undefined

  // Older Windows builds signed and persisted the native spelling. Verify that signed legacy
  ...
  return {
    ...
    hookEndpoint: canonicalEndpoint,
...
export function codexThreadIdentityHasLiveConflict(
  ...
  const liveOwners = new Set(
    identityCandidates(threadId, identityRoot)
      ...
      .filter((owner) => nodeId === undefined || owner !== nodeId)
      .filter(isNodeLive)
  )
  return nodeId === undefined ? liveOwners.size > 1 : liveOwners.size > 0
}

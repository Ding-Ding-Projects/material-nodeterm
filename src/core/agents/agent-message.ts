...
export async function deliverAgentMessage(
  ...
  return deps.lock(req.targetNodeId, async () => {
    ...
    // The host-side actuator owns paste framing. Keeping this value as the raw envelope avoids
    // double-wrapping it when PtyManager selects its bracketed-paste path, while injected callers
    // still receive the exact same payload contract through sendFramed.

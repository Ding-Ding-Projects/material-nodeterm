...
export class CloudflareTunnelService implements CloudflareTunnelApi {
  ...
  async setToken(token: string | null): Promise<void> {
    if (token === null || token.trim() === '') {
      await this.tokenStore.mutate((entries) => {
        ...
        return { changed: next.length !== entries.length, result: undefined }
      })
      return
    }
  ...
  async preflight(plan: CloudflareTunnelPlan): Promise<CloudflarePreflightCheck[]> {
    ...
    try {
      ...
      zones = await this.zones(plan.accountId)
  ...
  async apply(plan: CloudflareTunnelPlan): Promise<CloudflareTunnelStatus> {
    ...
    try {
      ...
      const token = asObject(tokenResponse)?.result
      if (typeof token !== 'string' || token.length === 0) throw new Error('Cloudflare did not return a connector token.')
      ...
      const connector = await this.runtime.installConnector({ hostId: plan.hostId, tunnelId, token, target })
      this.current = {
  ...
  private async rollbackInternal(accountId: string | null, zoneId: string | null, hostId: string | null): Promise<void> {
    const token = this.current.connectorContainerId && this.current.tokenFilePath && hostId && this.runtime
    if (token) await this.runtime!.removeConnector({ hostId: hostId!, connectorContainerId: this.current.connectorContainerId!, tokenFilePath: this.current.tokenFilePath! })
...
export function createCloudflareTunnelService(runtime?: CloudflareTunnelRuntime): CloudflareTunnelService {
  return new CloudflareTunnelService(runtime)
}

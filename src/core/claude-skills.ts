...
/**
 * Discover one config directory without reading any provider-authored skill content.
 *
 * A missing config directory is `unavailable`, while an existing config directory with no
 * `skills/` folder is `missing`. Those are intentionally different states: the former means the
 * scope cannot be inspected, the latter means it was inspected and has no skill catalogue yet.
 */
export async function discoverClaudeSkillScope(input: LocalScopeInput): Promise<ClaudeSkillScope> {
  let configStat
  try {
    configStat = await fs.stat(input.configDir)
    if (!configStat.isDirectory()) {
      return {
        ...
        state: 'unavailable',
        reason: 'Claude config path is not a directory.',
  ...
  try {
    ...
    if (!stats.isDirectory()) throw new Error('skills path is not a directory')
  } catch (error) {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      // A visible folder without a readable SKILL.md is still visible, with an honest diagnosis.
...
/** Parse the line-oriented response used by the SSH scope reader. No skill content crosses SSH. */

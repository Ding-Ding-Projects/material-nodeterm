...
} from '@shared/agents/status-snapshot'
...
import { defaultCodexAppServerSocket, readCodexStatusAt } from './codex-session-name'
...
function lastRecordTimestamp(text: string): number | null {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    try {
      ...
      // A capped or torn line costs only itself. A failed read is never evidence of absence.
    }
  }
  return null
}
...
export function inspectCodexThread(
  thread: { status?: { type?: unknown; activeFlags?: unknown } } | null,
  observedAt: number
...
async function inspectTranscriptAt(
  ...
  const text = await readSmallTail(transcriptPath, RECOVERY_TAIL_BYTES)
...
const DEFAULT_INSPECTORS: Partial<Record<BuiltinAgentId, AgentStatusInspector>> = {
  ...
  codex: async ({ sessionId }) =>
    inspectCodexThread(
      await readCodexStatusAt(defaultCodexAppServerSocket(), sessionId),
      Date.now()
    ),
...
/** Inspect every addressable session independently. Unsupported agents and failed reads stay unknown. */
export async function recoverAgentStatusSnapshot(
  ...
  await Promise.all(
    Object.entries(snapshot).map(async ([nodeId, entry]: [string, AgentStatusSnapshotEntry]) => {
      ...
      const builtin = BUILTIN_AGENT_IDS.find((id) => id === entry.agentId)
      ...
      try {
        ...
        // Best effort. A failed inspector is unknown, never proof that remembered state ended.
      }
    })
  )
  return recovered

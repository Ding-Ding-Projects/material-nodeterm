...
import { defaultScheduledSettingsFile, newScheduleRule } from '../shared/scheduled-settings'
import {
...
describe('scheduled-settings credential clear', () => {
  ...
  it('surfaces orphan cleanup failure through schedule save after the schedule itself is durable', async () => {
    const store = new ScheduledSettingsStore()
    store.init()
    const service = new ScheduledSettingsService(store)
    service.start()
    try {
      const rule = {
        ...
        source: {
          ...
          baseUrl: 'https://home-assistant.invalid',
          entityId: 'input_boolean.away'
        }
      }
      await expect(
        store.save({ ...defaultScheduledSettingsFile(), rules: [rule] })
      ).resolves.toEqual({ ok: true })
      await setHomeAssistantToken(RULE_ID, 'orphaned bearer')

      const realUnlink = fs.unlink
      vi.spyOn(fs, 'unlink').mockImplementation(async (file) => {
        if (String(file) === rawFile) {
          const error = new Error('injected prune deletion failure') as NodeJS.ErrnoException
          error.code = 'EACCES'
          throw error
        }
        return realUnlink(file)
  ...
  it('prunes crash-gap residue at service startup without waiting for another schedule save', async () => {
    await setHomeAssistantToken(RULE_ID, 'orphaned bearer')
    const store = new ScheduledSettingsStore()
    store.init() // no schedule file: the token belongs to a rule deleted before the prior prune
    const service = new ScheduledSettingsService(store)
    service.start()
    try {

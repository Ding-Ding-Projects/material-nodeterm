...
import { validateNextcloudAioProfile, type NextcloudAioProfile } from '../shared/nextcloud-aio'
...
const ALLOWED_NODE = new Set(['id', 'kind', 'position', 'size', 'title', 'color', 'group', 'collapsed', 'parentId', 'tags', 'text', 'url', 'browserTabs', 'serviceLabel', 'nextcloudAioProfile'])
...
function projectNode(node: CanvasNodeState, strict = false): PortableCanvasNodeV3 {
  ...
  if (strict && node.nextcloudAioProfile !== undefined && !validateNextcloudAioProfile(node.nextcloudAioProfile)) throw new PortableProjectV3Error('manifest', 'Portable Nextcloud AIO profile is invalid.')
  ...
  if (node.nextcloudAioProfile !== undefined) {
    const profile = validateNextcloudAioProfile(node.nextcloudAioProfile)
    if (profile) out.nextcloudAioProfile = profile
    else if (strict) throw new PortableProjectV3Error('manifest', 'Portable Nextcloud AIO profile is invalid.')
  }

...
import { validateHostedServiceTunnelIntent, type HostedServiceTunnelIntent } from '../shared/hosted-service-tunnel'
...
const ALLOWED_NODE = new Set(['id', 'kind', 'position', 'size', 'title', 'color', 'group', 'collapsed', 'parentId', 'tags', 'text', 'url', 'browserTabs', 'serviceLabel', 'hostedServiceTunnel'])
...
function projectNode(node: CanvasNodeState, strict = false): PortableCanvasNodeV3 {
  ...
  if (strict && node.hostedServiceTunnel !== undefined && !validateHostedServiceTunnelIntent(node.hostedServiceTunnel)) throw new PortableProjectV3Error('manifest', 'Portable hosted-service tunnel intent is invalid.')
  ...
  if (node.hostedServiceTunnel !== undefined) {
    const intent = validateHostedServiceTunnelIntent(node.hostedServiceTunnel)
    if (!intent) throw new PortableProjectV3Error('manifest', 'Portable hosted-service tunnel intent is invalid.')
    out.hostedServiceTunnel = intent
  }

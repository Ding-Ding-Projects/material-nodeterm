...
export interface PortableCanvasV3 {
  ...
  /** Persisted depth from the root, with root = 0. */
  depth?: number
  title: string
  order: number
  viewport?: Viewport
...
export function projectToPortableCanvasV3(project: Project, input: PortableCanvasProjectionInput = {}): PortableCanvasProjectionV3 {
  ...
  const childNodeIds = new Set((project.childCanvases ?? []).flatMap((canvas) => canvas.nodeIds))
  ...
  const sourceCanvases = input.canvases ?? project.childCanvases?.map((canvas) => ({ ...canvas })) ?? []
  const children = sourceCanvases.map((canvas) => ({ id: text(canvas.id, 'canvas id'), scope: canvas.scope, ...(canvas.parentCanvasId ? { parentCanvasId: text(canvas.parentCanvasId, 'parent canvas id') } : {}), ...(canvas.depth !== undefined ? { depth: finite(canvas.depth, 'canvas depth') } : {}), title: text(canvas.title, 'canvas title'), order: finite(canvas.order, 'canvas order'), ...(canvas.viewport ? { viewport: { x: finite(canvas.viewport.x, 'viewport x'), y: finite(canvas.viewport.y, 'viewport y'), zoom: finite(canvas.viewport.zoom, 'canvas zoom') } } : {}), nodeIds: [...(canvas.nodeIds ?? [])].map((id) => text(id, 'canvas node id')).sort() }))
  ...
  const repaired = repairUniverseShops(validatePortableCanvasProjectionV3(result))
  const refusal = repaired.repairs.find((repair) => repair.action === 'collision-refused')
  if (refusal) throw new PortableProjectV3Error('manifest', refusal.reason)
...
export function parsePortableCanvasProjectionV3(bytes: Uint8Array): PortableCanvasProjectionV3 {
  if (bytes.byteLength > PORTABLE_CANVAS_LIMITS.maxJsonBytes) throw new PortableProjectV3Error('raw-limit', 'Portable project.json exceeds its byte bound.')
  try {
    const repaired = repairUniverseShops(validatePortableCanvasProjectionV3(JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(bytes))))
    const refusal = repaired.repairs.find((repair) => repair.action === 'collision-refused')
    if (refusal) throw new PortableProjectV3Error('manifest', refusal.reason)
    return repaired.projection

...
/** Verify a selected machine-local resource at the point of use. Symlinks are not accepted as a
 * resource handle because they can redirect the binding after the picker returned. */
export async function verifyLocalBindingResource(resource: string): Promise<boolean> {

...
const BOARD_LOG_EVENT_TYPES = new Set(['card-created', 'card-moved', 'column-added', 'column-renamed', 'column-deleted', 'member-assigned', 'member-unassigned', 'due-set', 'due-cleared', 'priority-set', 'priority-cleared'])
...
export function validEntry(x: unknown): x is BoardLogEntry {
  ...
  if (e.attachmentSessionId !== undefined && !/^[a-f0-9-]{36}$/i.test(e.attachmentSessionId)) return false
...
function validAppendEntry(entry: BoardLogEntry): boolean {
  return validEntry(entry) && /^[a-f0-9-]{36}$/i.test(entry.id) && Number.isFinite(entry.ts) && (entry.attachments === undefined || entry.attachments.every(validBoardLogAttachment)) && (entry.attachments === undefined || entry.attachmentSessionId !== undefined)
}

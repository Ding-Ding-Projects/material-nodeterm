...
export function tmuxConf(
  ...
  const wordSeparators =
    typeof wordSeparatorsOrLeadPaneWidth === 'string'
      ? wordSeparatorsOrLeadPaneWidth
      : DEFAULT_WORD_SEPARATORS
  const effectiveLeadPaneWidth =
    typeof wordSeparatorsOrLeadPaneWidth === 'number'
      ? wordSeparatorsOrLeadPaneWidth
      : leadPaneWidth
...
${leadPaneHookLines(effectiveLeadPaneWidth)}`
}
...
export class PtyManager {
  ...
  ensureTmux(): void {
    ...
    try {
      ...
      fs.writeFileSync(
        this.confPath,
        tmuxConf(s.tmuxScrollback, s.terminalWordSeparators, s.tmuxLeadPaneWidth)
      )
    } catch {

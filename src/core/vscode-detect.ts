...
async function verify(cmd: string): Promise<boolean> {
  try {
    await execFileP(cmd, ['--version'], { timeout: 4000, windowsHide: true, shell: WINDOWS_SHELL })
    return true
  } catch {
    return false
  }
...
function wellKnownPaths(): { cmd: string; kind: VsCodeKind }[] {
...
export async function detectVsCode(): Promise<VsCodeInstall[]> {
  ...
  for (const c of pathCandidates) {
    ...
    if (await verify(c.cmd)) {
      found.push({ command: c.cmd, kind: c.kind, fromPath: true })
      seen.add(c.kind)
    }
  }

  for (const c of wellKnownPaths()) {
    if (seen.has(c.kind)) continue
    if (existsSync(c.cmd) && (await verify(c.cmd))) {
...
export async function openInVsCode(targetPath: string, install?: VsCodeInstall): Promise<VsCodeOpenResult> {
  const chosen = install ?? (await detectVsCode())[0]
  if (!chosen) {
    return {
      ok: false,
  ...
  try {
    await execFileP(chosen.command, ['-n', targetPath], {
      timeout: 8000,
      windowsHide: true,
      shell: WINDOWS_SHELL
    })

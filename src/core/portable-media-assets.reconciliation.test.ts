...
describe('portable media asset contracts', () => {
  ...
  it('rejects a manifest with unknown keys and round-trips canonical metadata', () => {
    ...
    unknown.extra = true
    expect(() => parsePortableMediaManifest(new TextEncoder().encode(JSON.stringify(unknown)))).toThrow(/unknown key/i)
  })
  ...
  it('makes Include, Omit, and Locate Later materially different archive decisions', () => {
    ...
    const result = applyPortableMediaDecisions([collected(asset.id), second, third], new Map([[asset.id, 'include'], [second.asset.id, 'omit'], [third.asset.id, 'locate-later']]))

...
describe('KNOWN GAP: Windows paths are not linkified', () => {
  // Pinning a limitation rather than asserting a feature. Ctrl+click file links do not work for
  // a LOCAL Windows project, and the whole path pipeline is POSIX by construction:
  //
  //   matchFileTokens   TOKEN_RE matches slash-separated segments, and the final guard is
  //                     `if (!path.includes('/')) continue` — so `C:\src\a.ts` is never even
  //                     produced as a token.
  //   resolveFileToken  treats a leading '/' as "absolute" and joins everything else onto cwd,
  //                     then splits on '/' and re-prefixes '/'.
  //
  // It fails CLOSED — no token, so no link is offered — rather than resolving to something wrong,
  // which is the only reason this is a gap and not a defect.
  //
  // SSH projects are unaffected on any client OS: a remote path is POSIX even when the desktop
  // running nodeterm is Windows, so those links work today. That is also what makes the fix
  // non-trivial — the resolver would need to know WHICH convention applies per session rather
  // than per platform, and a terminal-output scanner that matches `C:\…` risks linkifying prose
  // and Windows-shaped text inside a POSIX session.
  //
  // Delete this block when it is implemented; it exists to make the gap visible rather than
  // letting it read as an oversight.

  it('does not tokenise a Windows absolute path', () => {
    expect(matchFileTokens(String.raw`see C:\Users\me\src\a.ts for detail`)).toEqual([])
  })

  it('does not tokenise a Windows relative path', () => {
    expect(matchFileTokens(String.raw`edited src\renderer\a.ts`)).toEqual([])
  })

  it('still tokenises POSIX paths, including inside an SSH session on a Windows desktop', () => {
    const hits = matchFileTokens('edited src/renderer/a.ts:12 today')
    expect(hits).toHaveLength(1)
    expect(hits[0].path).toBe('src/renderer/a.ts')
    expect(hits[0].line).toBe(12)
  })

  it('resolveFileToken keeps its POSIX contract', () => {

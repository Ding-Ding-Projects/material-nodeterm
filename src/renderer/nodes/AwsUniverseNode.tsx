    ...
    <>
      ...
      <section className={`aws-universe-node${selected ? ' selected' : ''}${collapsed ? ' collapsed' : ''}`} style={border.style} aria-label={`${data.title || 'AWS Universe'}, AWS-only canvas`}>
        ...
        <header className="aws-universe-node__header" style={{ background: alphaTint(data.color, 0.2) }}>
          <button className="term-node__collapse" title={collapsed ? 'Expand AWS Universe' : 'Collapse AWS Universe'} onClick={toggleCollapse}>{collapsed ? '▸' : '▾'}</button>
          <button className="term-node__color" style={{ background: data.color }} title="Choose portal color" aria-label="Choose portal color" onClick={(event) => { const rect = event.currentTarget.getBoundingClientRect(); setColorAnchor((current) => current ? null : { x: rect.left, y: rect.bottom + 4 }) }} />
        ...
        {!collapsed ? (
          entered ? (
            <div className="aws-universe-node__body aws-universe-node__body--entered">
              <div className="aws-universe-node__doorbar"><span>Entered through matching door</span><button type="button" onClick={closeThroughDoor}>Return through matching door</button></div>
              ...
              <div className="aws-universe-node__search-row">
                ...
                <div className="aws-universe-node__search-control">
                  <input id={`${id}-catalog-search`} value={field.value} placeholder="Search AWS interfaces" onChange={(event) => field.setValue(event.target.value)} />
            ...
            <div className="aws-universe-node__body">
              ...
              <button type="button" className="aws-universe-node__enter" onClick={openThroughDoor}>Enter through matching door</button>
              ...
              <select id={`${id}-context`} value={context?.profileRef ?? ''} onChange={(event) => chooseContext(event.target.value)}>

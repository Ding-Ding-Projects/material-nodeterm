  ...
  return (
    <div className={`term-node converter-node${selected ? ' selected' : ''}`} style={{ borderTopColor: data.color }}>
      ...
      <div className="converter-node__body nodrag nowheel">
        ...
        <div className="converter-node__tabs" role="tablist" aria-label={vocab('Converter views')}>
          <Chip vocabularyMode="factual" selected={view === 'convert'} role="tab" aria-selected={view === 'convert'} onClick={() => setView('convert')}>{vocab('Convert')}</Chip>
          <Chip vocabularyMode="factual" selected={view === 'queue'} role="tab" aria-selected={view === 'queue'} onClick={() => setView('queue')}>{vocab('Queue')} ({summary.total})</Chip>
        </div>

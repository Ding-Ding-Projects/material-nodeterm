...
export default function HomeAssistantControlNode({ id, data, selected }: NodeProps<CanvasNode>): React.JSX.Element {
  ...
  const domainControls = (): React.JSX.Element => {
    ...
    if (domain === 'light') { const brightness = numberAttr(selectedEntity, 'brightness', 0); return <><div className="home-assistant-control__actions"><button type="button" disabled={busy} onClick={() => void call(domain, 'turn_on')}>{turnOn}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'turn_off')}>{turnOff}</button></div><label>{text('homeAssistantControl.field.brightness', 'Brightness')}<input type="range" min={0} max={255} value={brightness} disabled={busy} onChange={(event) => void call(domain, 'turn_on', { brightness: Number(event.currentTarget.value) })} /></label></> }
    if (domain === 'fan') { const percentage = numberAttr(selectedEntity, 'percentage', 0); return <><div className="home-assistant-control__actions"><button type="button" disabled={busy} onClick={() => void call(domain, 'turn_on')}>{turnOn}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'turn_off')}>{turnOff}</button></div><label>{text('homeAssistantControl.field.speed', 'Speed')}<input type="range" min={0} max={100} value={percentage} disabled={busy} onChange={(event) => void call(domain, 'set_percentage', { percentage: Number(event.currentTarget.value) })} /></label></> }
    if (domain === 'media_player') { const volume = numberAttr(selectedEntity, 'volume_level', 0); const muted = selectedEntity.attributes.is_volume_muted === true; return <><div className="home-assistant-control__actions"><button type="button" disabled={busy} onClick={() => void call(domain, 'media_play')}>{text('homeAssistantControl.action.play', 'Play')}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'media_pause')}>{text('homeAssistantControl.action.pause', 'Pause')}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'media_stop')}>{text('homeAssistantControl.action.stop', 'Stop')}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'volume_mute', { is_volume_muted: !muted })}>{muted ? text('homeAssistantControl.action.unmute', 'Unmute') : text('homeAssistantControl.action.mute', 'Mute')}</button></div><label>{text('homeAssistantControl.field.volume', 'Volume')}<input type="range" min={0} max={1} step={0.01} value={volume} disabled={busy} onChange={(event) => void call(domain, 'volume_set', { volume_level: Number(event.currentTarget.value) })} /></label></> }
    if (['switch', 'input_boolean', 'automation'].includes(domain)) return <div className="home-assistant-control__actions"><button type="button" disabled={busy} onClick={() => void call(domain, 'turn_on')}>{turnOn}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'turn_off')}>{turnOff}</button></div>
    if (domain === 'cover') { const position = numberAttr(selectedEntity, 'current_position', 0); return <><div className="home-assistant-control__actions"><button type="button" disabled={busy} onClick={() => void call(domain, 'open_cover')}>{text('homeAssistantControl.action.open', 'Open')}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'stop_cover')}>{text('homeAssistantControl.action.stop', 'Stop')}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'close_cover')}>{text('homeAssistantControl.action.close', 'Close')}</button></div><label>{text('homeAssistantControl.field.position', 'Position')}<input type="range" min={0} max={100} value={position} disabled={busy} onChange={(event) => void call(domain, 'set_cover_position', { position: Number(event.currentTarget.value) })} /></label></> }
    if (domain === 'lock') return <div className="home-assistant-control__actions"><button type="button" disabled={busy} onClick={() => void call(domain, 'lock')}>{text('homeAssistantControl.action.lock', 'Lock')}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'unlock')}>{text('homeAssistantControl.action.unlock', 'Unlock')}</button></div>
    if (domain === 'climate') { const min = numberAttr(selectedEntity, 'min_temp', 7); const max = numberAttr(selectedEntity, 'max_temp', 35); const current = numberAttr(selectedEntity, 'temperature', min); return <label>{text('homeAssistantControl.field.targetTemperature', 'Target temperature')}<input type="range" min={min} max={max} step={0.5} defaultValue={current} disabled={busy} onChange={(event) => void call(domain, 'set_temperature', { temperature: Number(event.currentTarget.value) })} /><span>{current}°</span></label> }
    if (domain === 'vacuum') return <div className="home-assistant-control__actions"><button type="button" disabled={busy} onClick={() => void call(domain, 'start')}>{text('homeAssistantControl.action.startCleaning', 'Start cleaning')}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'pause')}>{text('homeAssistantControl.action.pause', 'Pause')}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'stop')}>{text('homeAssistantControl.action.stop', 'Stop')}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'returnToBase')}>{text('homeAssistantControl.action.returnToBase', 'Return to base')}</button></div>
    if (domain === 'alarm_control_panel') return <div className="home-assistant-control__actions"><button type="button" disabled={busy} onClick={() => void call(domain, 'arm_home')}>{text('homeAssistantControl.action.armHome', 'Arm home')}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'arm_away')}>{text('homeAssistantControl.action.armAway', 'Arm away')}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'disarm')}>{text('homeAssistantControl.action.disarm', 'Disarm')}</button></div>
    if (domain === 'humidifier') { const humidity = numberAttr(selectedEntity, 'humidity', 0); const min = numberAttr(selectedEntity, 'min_humidity', 0); const max = numberAttr(selectedEntity, 'max_humidity', 100); return <><div className="home-assistant-control__actions"><button type="button" disabled={busy} onClick={() => void call(domain, 'turn_on')}>{turnOn}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'turn_off')}>{turnOff}</button></div><label>{text('homeAssistantControl.field.humidity', 'Humidity')}<input type="range" min={min} max={max} value={humidity} disabled={busy} onChange={(event) => void call(domain, 'set_humidity', { humidity: Number(event.currentTarget.value) })} /></label></> }
    if (domain === 'water_heater') return <div className="home-assistant-control__actions"><button type="button" disabled={busy} onClick={() => void call(domain, 'turn_on')}>{turnOn}</button><button type="button" disabled={busy} onClick={() => void call(domain, 'turn_off')}>{turnOff}</button></div>
    if (domain === 'number' || domain === 'input_number') { const min = numberAttr(selectedEntity, 'min', 0); const max = numberAttr(selectedEntity, 'max', 100); return <label>{text('homeAssistantControl.field.value', 'Value')}<input type="number" min={min} max={max} step={numberAttr(selectedEntity, 'step', 1)} defaultValue={Number(selectedEntity.state)} disabled={busy} onBlur={(event) => void call(domain, 'set_value', { value: Number(event.currentTarget.value) })} /></label> }
    if (domain === 'select' || domain === 'input_select') { const options = Array.isArray(selectedEntity.attributes.options) ? selectedEntity.attributes.options.filter((value): value is string => typeof value === 'string').slice(0, 500) : []; return <label>{text('homeAssistantControl.field.option', 'Option')}<select defaultValue={selectedEntity.state} disabled={busy || options.length === 0} onChange={(event) => void call(domain, 'select_option', { option: event.currentTarget.value })}>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select></label> }
    if (domain === 'button' || domain === 'input_button') return <button type="button" disabled={busy} onClick={() => void call(domain, 'press')}>{text('homeAssistantControl.action.press', 'Press')}</button>
    if (domain === 'scene' || domain === 'script') return <button type="button" disabled={busy} onClick={() => void call(domain, 'turn_on')}>{text('homeAssistantControl.action.activate', 'Activate')}</button>
  ...
  return <div className={`term-node home-assistant-control${selected ? ' selected' : ''}`} style={{ borderTopColor: data.color }}>
    ...
    <div className="home-assistant-control__body">
      <section aria-labelledby={`${id}-connection`}><h3 id={`${id}-connection`}>{text('homeAssistantControl.connection.heading', 'Connection on this computer')}</h3>
        ...
        <div className="home-assistant-control__actions"><button type="button" disabled={!connectionId || busy} title={!connectionId ? text('homeAssistantControl.connection.chooseFirst', 'Choose or configure a connection first.') : undefined} onClick={() => void discover()}>{text('homeAssistantControl.discover', 'Discover or retry')}</button><button type="button" disabled={!connectionId || busy} onClick={() => void api.homeAssistantControl.bind(id, null).then(() => { setConnectionId(''); setEntities([]); setServices([]); setMessage(text('homeAssistantControl.status.nodeUnboundPortable', 'Node left unbound. Portable entity hints remain available for rebinding.')) })}>{text('homeAssistantControl.connection.unbound', 'Leave unbound')}</button>{busy && <button type="button" onClick={() => void api.homeAssistantControl.cancel(id)}>{text('homeAssistantControl.action.cancel', 'Cancel')}</button>}</div>
        <details><summary>{text('homeAssistantControl.configure', 'Configure a connection')}</summary><label>{text('homeAssistantControl.connection.name', 'Name')}<input value={newConnection.label} onChange={(event) => setNewConnection({ ...newConnection, label: event.target.value })} /></label><label>{text('homeAssistantControl.connection.url', 'Home Assistant URL')}<input type="url" value={newConnection.baseUrl} placeholder={text('homeAssistantControl.connection.urlPlaceholder', 'https://homeassistant.example')} onChange={(event) => setNewConnection({ ...newConnection, baseUrl: event.target.value })} /></label><label>{text('homeAssistantControl.connection.token', 'Long-lived access token')}<input type="password" autoComplete="off" value={newConnection.token} onChange={(event) => setNewConnection({ ...newConnection, token: event.target.value })} /></label><p>{text('homeAssistantControl.connection.tokenBoundary', 'The token is sent directly to this computer’s credential boundary. It is never written into the project.')}</p><button type="button" disabled={!newConnection.label.trim() || !newConnection.baseUrl.trim() || !newConnection.token || busy} onClick={async () => { try { const saved = await api.homeAssistantControl.configure(newConnection); await api.homeAssistantControl.bind(id, saved.id); setNewConnection({ label: '', baseUrl: '', token: '' }); await refreshConnections(); setMessage(text('homeAssistantControl.status.connectionConfigured', 'Connection configured and bound locally.')); } catch (error) { setMessage(error instanceof Error ? error.message : text('homeAssistantControl.status.connectionNotSaved', 'Connection was not saved.')) } }}>{text('homeAssistantControl.connection.saveAndBind', 'Save and bind')}</button></details>
      </section>
      <section aria-labelledby={`${id}-entity`}><h3 id={`${id}-entity`}>{text('homeAssistantControl.entity.heading', 'Entity')}</h3><div className="home-assistant-control__search"><label>{text('homeAssistantControl.entity.search', 'Search entities')}<input ref={entitySearchRef} value={entitySearch.value} onChange={(event) => entitySearch.setValue(event.target.value)} /></label><AnchoredRegexBuilder search={entitySearch} fieldRef={entitySearchRef} label={text('homeAssistantControl.entity.regex', 'Regex for Home Assistant entities')} /></div><label>{text('homeAssistantControl.entity.heading', 'Entity')}<select value={config.entityHint ?? ''} disabled={!connectionId || filteredEntities.length === 0} onChange={(event) => { const entity = entities.find((item) => item.entityId === event.target.value); setConfig({ entityHint: event.target.value || null, domainHint: entity?.domain ?? null, serviceHint: null }) }}><option value="">{text('homeAssistantControl.entity.choose', 'Choose an entity')}</option>{filteredEntities.map((entity) => <option key={entity.entityId} value={entity.entityId}>{entity.friendlyName} · {entity.entityId} · {entity.state}</option>)}</select></label>{domainControls()}</section>
      <section aria-labelledby={`${id}-fallback`}><h3 id={`${id}-fallback`}>{text('homeAssistantControl.fallback.heading', 'Verified schema fallback')}</h3><div className="home-assistant-control__search"><label>{text('homeAssistantControl.service.search', 'Search services')}<input ref={serviceSearchRef} value={serviceSearch.value} onChange={(event) => serviceSearch.setValue(event.target.value)} /></label><AnchoredRegexBuilder search={serviceSearch} fieldRef={serviceSearchRef} label={text('homeAssistantControl.service.regex', 'Regex for Home Assistant services')} /></div><label>{text('homeAssistantControl.service.label', 'Service')}<select value={config.serviceHint ?? ''} disabled={!selectedEntity || availableServices.length === 0} onChange={(event) => { setFallbackValues({}); setConfig({ serviceHint: event.target.value || null, controlMode: 'schema' }) }}><option value="">{text('homeAssistantControl.service.choose', 'Choose a service')}</option>{availableServices.map((service) => <option key={`${service.domain}.${service.service}`} value={`${service.domain}.${service.service}`}>{service.name} · {service.domain}.{service.service}</option>)}</select></label>{selectedService?.fields.map((field) => { const { kind, options, config: selector } = selectorConfig(field); const value = fallbackValues[field.name] ?? ''; const setValue = (next: SchemaValue): void => setFallbackValues({ ...fallbackValues, [field.name]: stringValue(next) }); const fieldName = field.required ? text('homeAssistantControl.schema.requiredField', '{name} (required)', { name: field.name }) : field.name; const numericMin = typeof selector.min === 'number' ? selector.min : 0; const numericMax = typeof selector.max === 'number' ? selector.max : 100; const numericStep = typeof selector.step === 'number' && selector.step > 0 ? selector.step : 1; if (kind === 'boolean') return <label key={field.name} className="home-assistant-control__schema-field"><span>{fieldName}</span><input type="checkbox" checked={value === 'true'} disabled={busy} onChange={(event) => setValue(event.currentTarget.checked)} /><small>{field.description || text('homeAssistantControl.schema.booleanHint', 'Boolean value from the verified Home Assistant schema.')}</small></label>; if (kind === 'number') return <label key={field.name} className="home-assistant-control__schema-field"><span>{fieldName}</span><input type="range" min={numericMin} max={numericMax} step={numericStep} value={value || String(numericMin)} disabled={busy} onChange={(event) => setValue(Number(event.currentTarget.value))} /><input type="number" min={numericMin} max={numericMax} step={numericStep} value={value} disabled={busy} onChange={(event) => setValue(event.currentTarget.value)} /><small>{field.description || text('homeAssistantControl.schema.numberHint', 'Number from {min} to {max}.', { min: String(numericMin), max: String(numericMax) })}</small></label>; if (kind === 'select' && options.length > 0) return <label key={field.name} className="home-assistant-control__schema-field"><span>{fieldName}</span><select value={value} disabled={busy} onChange={(event) => setValue(event.currentTarget.value)}><option value="">{text('homeAssistantControl.schema.chooseValue', 'Choose a value')}</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select><small>{field.description || text('homeAssistantControl.schema.selectHint', 'Choose one value from the verified Home Assistant schema.')}</small></label>; if (kind === 'entity' || kind === 'target') return <label key={field.name} className="home-assistant-control__schema-field"><span>{fieldName}</span><select value={value || selectedEntity.entityId} disabled={busy || entities.length === 0} onChange={(event) => setValue(event.currentTarget.value)}>{entities.filter((entity) => { const domains = Array.isArray(selector.domain) ? selector.domain.filter((item): item is string => typeof item === 'string') : []; return domains.length === 0 || domains.includes(entity.domain) }).slice(0, 1000).map((entity) => <option key={entity.entityId} value={entity.entityId}>{entity.friendlyName} · {entity.entityId}</option>)}</select><small>{field.description || text('homeAssistantControl.schema.entityHint', 'Choose an entity from the verified local entity list.')}</small></label>; const inputType = kind === 'date' ? 'date' : kind === 'time' ? 'time' : kind === 'datetime' ? 'datetime-local' : 'text'; return <label key={field.name} className="home-assistant-control__schema-field"><span>{fieldName}</span>{kind === 'text' && selector.multiline === true ? <textarea value={value} disabled={busy} maxLength={4096} onChange={(event) => setValue(event.currentTarget.value)} /> : <input type={inputType} value={value} disabled={busy} maxLength={kind === 'text' ? 4096 : undefined} onChange={(event) => setValue(event.currentTarget.value)} />}<small>{field.description || text('homeAssistantControl.schema.valueHint', 'Value is validated by the selected Home Assistant service schema.')}</small></label> })}<button type="button" disabled={!selectedEntity || !selectedService || busy || selectedService.fields.some((field) => field.required && !String(fallbackValues[field.name] ?? '').trim())} onClick={() => selectedService && void call(selectedService.domain, selectedService.service, fallbackValues)}>{text('homeAssistantControl.service.run', 'Run verified service')}</button></section>

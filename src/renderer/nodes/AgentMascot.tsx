...
export function AgentMascot({
  ...
  const harnessId = agentBaseId ?? agentId
  if (harnessId === 'claude' && CLAUDE_MASCOT.src) {
    const style = {
  ...
  if (harnessId && hasBrandLogo(harnessId)) {
    return <BrandPulse agentId={harnessId} size={BRAND_BADGE_SIZE} />
  }

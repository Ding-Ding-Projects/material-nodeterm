...
export default function WildDimSumNode({ id, data, selected }: NodeProps<CanvasNode>) {
  ...
  if (!schoolModeHydrated || schoolModeEnabled) return <div className={`term-node${selected ? ' selected' : ''}`} style={{ borderTopColor: data.color }} role="region" aria-label="Optional feature unavailable"><NodeResizer minWidth={280} minHeight={180} isVisible={selected} color={data.color} /><div className={`term-node__header ${fill.className}${fill.filled ? ' term-node__header--filled' : ''}`} style={fill.style}><span className="term-node__title-text">Optional feature unavailable</span><span className="term-node__spacer" /><button className="term-node__close" title="Close" aria-label="Close optional feature" onClick={() => deleteElements({ nodes: [{ id }] })}>×</button></div><div className="wild-dim-sum-node__body"><p>This optional feature is hidden while the current shared mode is on.</p></div></div>
  return <div className={`term-node wild-dim-sum-node${selected ? ' selected' : ''}`} style={{ borderTopColor: data.color }} role="region" aria-label={ts('wildDimSum.aria', 'Wild dim sum node')}>
    ...
    <div className={`term-node__header ${fill.className}${fill.filled ? ' term-node__header--filled' : ''}`} style={fill.style}><span aria-hidden="true">🥟</span><span className="term-node__title-text">{String(data.title || ts('wildDimSum.title', 'Wild dim sum'))}</span><span className="term-node__spacer" /><button className="term-node__close" title={ts('wildDimSum.close', 'Close')} aria-label={ts('wildDimSum.closeAria', 'Close Wild dim sum node')} onClick={() => deleteElements({ nodes: [{ id }] })}>×</button></div>
    <div className="wild-dim-sum-node__body nodrag nowheel">
      {chosen && <section className="wild-dim-sum-node__selection" aria-label="Selected dish"><img key={photoAttempt} src={publicDimSumImageUrl(chosen)} alt={`${chosen.image.alt.en} · ${chosen.image.alt.yue}`} onLoad={() => setPhotoFailed(false)} onError={() => setPhotoFailed(true)} /><div><h3>{chosen.name.en} · {chosen.name.zhHant}</h3><p>{chosen.description.en}</p><p lang="yue-Hant-HK">{chosen.description.yue}</p><small>{chosen.category} · {chosen.subcategory}</small></div>{photoFailed && <div className="wild-dim-sum-node__notice" role="status">The published photo could not be loaded. The saved dish details remain available. <button type="button" onClick={() => { setPhotoFailed(false); setPhotoAttempt((value) => value + 1) }}>Retry photo</button></div>}</section>}
      <div className="wild-dim-sum-node__actions"><button type="button" onClick={pickRandom} disabled={load.kind !== 'ready'} title={load.kind === 'ready' ? ts('wildDimSum.randomReady', 'Choose a random published dish') : ts('wildDimSum.randomDisabled', 'The public catalog must finish loading first')}>{ts('wildDimSum.random', 'Surprise me')}</button><button type="button" onClick={loadCatalog} disabled={load.kind === 'loading'}>{load.kind === 'error' || load.kind === 'cancelled' ? ts('wildDimSum.retry', 'Retry catalog') : ts('wildDimSum.refresh', 'Refresh catalog')}</button>{load.kind === 'loading' && <button type="button" onClick={() => requestRef.current?.abort()}>{ts('wildDimSum.cancel', 'Cancel')}</button>}</div>
      {load.kind === 'loading' && <div className="wild-dim-sum-node__progress" role="status"><progress value={progress ?? undefined} max="100" /> <span>{progress === null ? `${load.loaded.toLocaleString()} bytes loaded` : `${progress}% loaded`}</span></div>}
      {load.kind === 'error' && <p className="wild-dim-sum-node__notice" role="alert">Catalog unavailable: {load.message} Retry when the network or public catalog is available.</p>}
      ...
      {load.kind === 'ready' && <section aria-label={ts('wildDimSum.chooseAria', 'Choose a public catalog dish')}><label htmlFor={`${id}-dish-search`}>{ts('wildDimSum.search', 'Search published dishes')}</label><div className="wild-dim-sum-node__search"><input ref={searchRef} id={`${id}-dish-search`} type="search" value={search.value} onChange={(event) => search.setValue(event.target.value)} placeholder={ts('wildDimSum.searchPlaceholder', 'Name, category, or subcategory')} onKeyDown={(event) => { if (event.key === 'Escape') search.reset() }} /><AnchoredRegexBuilder search={search} fieldRef={searchRef} label={ts('wildDimSum.regex', 'Regex builder for published dishes')} zIndex={95} /></div><p className="wild-dim-sum-node__count" role="status">{search.error ? ts('wildDimSum.invalidPattern', 'Invalid pattern. All dishes remain visible.') : ts('wildDimSum.count', '{shown} of {total} dishes shown.', { shown: String(matches.length), total: String(load.dishes.length) })}</p><ul className="wild-dim-sum-node__list" aria-label={ts('wildDimSum.listAria', 'Published dishes')}>{matches.length === 0 ? <li className="wild-dim-sum-node__empty">{ts('wildDimSum.empty', 'No published dishes match this search.')}</li> : matches.map((dish) => <li key={dish.id}><button type="button" aria-pressed={chosen?.id === dish.id} onClick={() => choose(dish)}><strong>{dish.name.en} · {dish.name.zhHant}</strong><span>{dish.category} · {dish.subcategory}</span></button></li>)}</ul></section>}
      <p className="wild-dim-sum-node__source">Catalog and photos: Ding-Ding-Projects/dim-sum-photos public releases. No photo is copied into this project.</p>
    </div>
  </div>
}

...
function EventEditor({ nodeId, timezone, initial, onSave, onCancel }: { nodeId: string; timezone: string; initial?: CalendarEvent; onSave: (event: CalendarEvent) => void; onCancel: () => void }): React.JSX.Element {
  ...
  return <div className="calendar-node__editor" role="region" aria-label={vocab(initial ? 'Edit calendar event' : 'Create calendar event')}>
    ...
    <label>{vocab('Title')}<input value={title} aria-label={vocab('Event title')} onChange={(e) => setTitle(e.target.value)} autoFocus /></label>
    <div className="calendar-node__two-col"><label>{vocab('Starts')}<input type="datetime-local" value={start} aria-label={vocab('Event start')} onChange={(e) => setStart(e.target.value)} /></label><label>{vocab('Ends')}<input type="datetime-local" value={end} aria-label={vocab('Event end')} onChange={(e) => setEnd(e.target.value)} /></label></div>
    <label>{vocab('Location')}<input value={location} aria-label={vocab('Event location')} onChange={(e) => setLocation(e.target.value)} /></label>
    <label>{vocab('Recurrence rule (optional)')}<input value={recurrence} aria-label={vocab('Event recurrence rule')} onChange={(e) => setRecurrence(e.target.value)} placeholder="RRULE:FREQ=WEEKLY;BYDAY=MO" /></label>
    {!valid && <p className="calendar-node__error" role="alert">{vocab('Enter a title and an end time after the start time.')}</p>}
    <div className="calendar-node__actions"><button type="button" onClick={onCancel}>{vocab('Cancel')}</button><button type="button" disabled={!valid} onClick={() => onSave({ id: initial?.id ?? `local-${Date.now().toString(36)}`, calendarId: initial?.calendarId ?? 'local', title: title.trim(), start: dateTimeInputIso(start, timezone), end: dateTimeInputIso(end, timezone), timezone, allDay: false, location: location.trim() || null, description: null, recurrence: recurrence.trim() || null, updatedAt: Date.now() })}>{vocab(initial ? 'Save changes' : 'Create event')}</button></div>
  </div>
}
...
export default function CalendarNode({ id, data, selected }: NodeProps<CanvasNode>): React.JSX.Element {
  ...
  return <div className={`term-node calendar-node${selected ? ' selected' : ''}`} style={{ borderTopColor: data.color }} aria-label={mapOwnedSentence(vocab, [copy('Calendar node: '), fact(String(data.title || 'Calendar'))])}>
    ...
    <div className="calendar-node__toolbar" role="tablist" aria-label={vocab('Calendar views')}>{(['agenda', 'week', 'month'] as CalendarView[]).map((view) => <button key={view} id={`${id}-tab-${view}`} role="tab" aria-selected={config.view === view} aria-controls={`${id}-panel-${view}`} tabIndex={config.view === view ? 0 : -1} onClick={() => setConfig({ view })}>{vocab(view[0].toUpperCase() + view.slice(1))}</button>)}</div>
    <div className="calendar-node__body">
      <div className="calendar-node__filters"><label>{vocab('Search events')}<input ref={searchInputRef} value={search.value} onChange={(e) => search.setValue(e.target.value)} placeholder={vocab('Plain text search')} aria-label={vocab('Search calendar events')} /></label><AnchoredRegexBuilder search={search} fieldRef={searchInputRef} label={vocab('Regex for calendar events')} /></div>
      <div className="calendar-node__source"><label>{vocab('Source')}<select value={config.provider} aria-label={vocab('Calendar source')} onChange={(e) => { const provider = e.target.value as CalendarProvider; setConfig({ provider, accountId: null, calendarId: null }); setEvents(provider === 'local' ? readCached(id) : []); setStatus(vocab(providerHelp[provider])) }}>{PROVIDERS.filter((provider) => sourceSearch.test(calendarProviderName(provider))).map((provider) => <option key={provider} value={provider}>{calendarProviderName(provider)}</option>)}</select><div className="calendar-node__picker-search"><input ref={sourceSearchRef} value={sourceSearch.value} onChange={(e) => sourceSearch.setValue(e.target.value)} placeholder={vocab('Filter sources')} aria-label={vocab('Filter calendar sources')} /><AnchoredRegexBuilder search={sourceSearch} fieldRef={sourceSearchRef} label={vocab('Regex for calendar sources')} /></div></label><p className="calendar-node__hint">{vocab(providerHelp[config.provider])}</p>
        {(config.provider === 'ics') && <button type="button" onClick={() => void importFile()} disabled={busy}>{vocab('Choose local ICS file…')}</button>}
        {config.provider === 'caldav' && !config.accountId && <div className="calendar-node__editor" role="region" aria-label={vocab('Connect CalDAV account')}><h3>{vocab('Connect CalDAV')}</h3><label>{vocab('Account name')}<input value={calDavName} aria-label={vocab('Account name')} onChange={(event) => setCalDavName(event.target.value)} /></label><label>{vocab('HTTPS server URL')}<input type="url" value={calDavEndpoint} aria-label={vocab('HTTPS server URL')} onChange={(event) => setCalDavEndpoint(event.target.value)} placeholder="https://calendar.example.com/dav/" /></label><label>{vocab('Username')}<input autoComplete="username" value={calDavUsername} aria-label={vocab('Username')} onChange={(event) => setCalDavUsername(event.target.value)} /></label><label>{vocab('Password')}<input type="password" autoComplete="current-password" value={calDavPassword} aria-label={vocab('Password')} onChange={(event) => setCalDavPassword(event.target.value)} /></label><button type="button" onClick={() => void connect()} disabled={busy || !calDavName.trim() || !calDavEndpoint.trim() || !calDavUsername.trim() || !calDavPassword}>{vocab('Connect and verify')}</button><p className="calendar-node__hint">{vocab('The endpoint must use HTTPS. The password is sent once to the host and never stored in the project.')}</p></div>}
        {config.provider !== 'local' && config.provider !== 'ics' && <><label>{vocab('Account')}<select value={config.accountId ?? ''} aria-label={vocab('Calendar account')} onChange={(event) => setConfig({ accountId: event.target.value || null, calendarId: null })}><option value="">{vocab('Choose an account…')}</option>{accounts.filter((candidate) => candidate.provider === config.provider && accountSearch.test(`${candidate.displayName} ${candidate.email ?? ''}`)).map((candidate) => <option key={candidate.id} value={candidate.id}>{fact(`${candidate.displayName}${candidate.email ? ` · ${candidate.email}` : ''} (${candidate.state})`).text}</option>)}</select><div className="calendar-node__picker-search"><input ref={accountSearchRef} value={accountSearch.value} onChange={(event) => accountSearch.setValue(event.target.value)} placeholder={vocab('Filter accounts')} aria-label={vocab('Filter calendar accounts')} /><AnchoredRegexBuilder search={accountSearch} fieldRef={accountSearchRef} label={vocab('Regex for calendar accounts')} /></div></label>{config.provider !== 'caldav' && <button type="button" onClick={() => void connect()} disabled={busy}>{vocab('Connect another account…')}</button>}<button type="button" onClick={() => void loadCatalog()} disabled={busy}>{vocab('Refresh accounts')}</button>{account && <button type="button" onClick={(event) => disconnect(event.currentTarget)} disabled={busy}>{vocab('Disconnect account…')}</button>}</>}
        {config.provider !== 'local' && config.provider !== 'ics' && <label>{vocab('Calendar')}<select value={config.calendarId ?? ''} aria-label={vocab('Provider calendar')} onChange={(e) => setConfig({ calendarId: e.target.value || null })}><option value="">{vocab('Choose a calendar…')}</option>{sources.filter((candidate) => calendarSearch.test(`${candidate.name} ${candidate.timezone}`)).map((candidate) => <option key={candidate.id} value={candidate.id}>{fact(`${candidate.name} · ${candidate.timezone}${candidate.readOnly ? ' (read only)' : ''}`).text}</option>)}</select><div className="calendar-node__picker-search"><input ref={calendarSearchRef} value={calendarSearch.value} onChange={(event) => calendarSearch.setValue(event.target.value)} placeholder={vocab('Filter calendars')} aria-label={vocab('Filter provider calendars')} /><AnchoredRegexBuilder search={calendarSearch} fieldRef={calendarSearchRef} label={vocab('Regex for provider calendars')} /></div></label>}
      ...
      <div className="calendar-node__row"><label>{vocab('Timezone')}<select value={config.timezone} onChange={(e) => setConfig({ timezone: e.target.value })} aria-label={vocab('Calendar timezone')}>{calendarTimezones().filter((value) => timezoneSearch.test(value)).map((value) => <option key={value} value={value}>{fact(value === 'local' ? 'This computer' : value).text}</option>)}</select><div className="calendar-node__picker-search"><input ref={timezoneSearchRef} value={timezoneSearch.value} onChange={(e) => timezoneSearch.setValue(e.target.value)} placeholder={vocab('Filter timezones')} aria-label={vocab('Filter calendar timezones')} /><AnchoredRegexBuilder search={timezoneSearch} fieldRef={timezoneSearchRef} label={vocab('Regex for calendar timezones')} /></div></label><label className="calendar-node__check"><input type="checkbox" checked={config.showWeekends} onChange={(e) => setConfig({ showWeekends: e.target.checked })} /> {vocab('Show weekends')}</label></div>
      <div className="calendar-node__actions"><button type="button" onClick={() => setCursorDate((d) => { const n = new Date(d); if (config.view === 'week') n.setDate(n.getDate() - 7); else if (config.view === 'month') n.setMonth(n.getMonth() - 1); else n.setDate(n.getDate() - 1); return n })} aria-label={vocab('Previous period')}>{vocab('Previous')}</button><strong>{config.view === 'agenda' ? vocab('Agenda') : fact(cursorDate.toLocaleDateString(undefined, { month: 'long', year: 'numeric' })).text}</strong><button type="button" onClick={() => setCursorDate((d) => { const n = new Date(d); if (config.view === 'week') n.setDate(n.getDate() + 7); else if (config.view === 'month') n.setMonth(n.getMonth() + 1); else n.setDate(n.getDate() + 1); return n })} aria-label={vocab('Next period')}>{vocab('Next')}</button><button type="button" onClick={() => setEditing(null)} disabled={config.provider !== 'local' && config.provider !== 'ics' && !source?.writable}>{vocab('Create event')}</button><button type="button" onClick={() => void api.calendar.refresh(id, config).then((cache) => { updateEvents(cache.events); setStatus(cache.state === 'offline' ? cache.reason ?? 'Offline cache is in use.' : vocab('Calendar refreshed.')) }).catch(() => setStatus(vocab('Refresh failed. The existing cache remains available.')))}>{vocab('Refresh')}</button></div>

...
import { Button, Checkbox, Chip, FieldLabel, Tablist } from '@renderer/ui/md3'
...
function EventEditor({ nodeId, timezone, initial, onSave, onCancel }: { nodeId: string; timezone: string; initial?: CalendarEvent; onSave: (event: CalendarEvent) => void; onCancel: () => void }): React.JSX.Element {
  ...
  return <div className="calendar-node__editor" role="region" aria-label={vocab(initial ? 'Edit calendar event' : 'Create calendar event')}>
    ...
    <FieldLabel label="Title"><Input vocabularyMode="factual" value={title} aria-label={vocab('Event title')} onChange={(e) => setTitle(e.target.value)} autoFocus /></FieldLabel>
    <div className="calendar-node__two-col"><FieldLabel label="Starts"><Input vocabularyMode="factual" type="datetime-local" value={start} aria-label={vocab('Event start')} onChange={(e) => setStart(e.target.value)} /></FieldLabel><FieldLabel label="Ends"><Input vocabularyMode="factual" type="datetime-local" value={end} aria-label={vocab('Event end')} onChange={(e) => setEnd(e.target.value)} /></FieldLabel></div>
    <FieldLabel label="Location"><Input vocabularyMode="factual" value={location} aria-label={vocab('Event location')} onChange={(e) => setLocation(e.target.value)} /></FieldLabel>
    <FieldLabel label="Recurrence rule (optional)"><Input vocabularyMode="factual" value={recurrence} aria-label={vocab('Event recurrence rule')} onChange={(e) => setRecurrence(e.target.value)} placeholder="RRULE:FREQ=WEEKLY;BYDAY=MO" /></FieldLabel>
...
export default function CalendarNode({ id, data, selected }: NodeProps<CanvasNode>): React.JSX.Element {
  ...
  const importFile = async (): Promise<void> => {
    ...
    try {
      ...
      setStatus(mapOwnedSentence(vocab, [copy('ICS import was not applied: '), error instanceof Error ? fact(error.message) : copy('the file could not be read'), copy('.')]))
    }
  ...
  const connect = async (): Promise<void> => {
    ...
    if (config.provider === 'caldav') {
      ...
      } catch (error) { setStatus(mapOwnedSentence(vocab, [copy('CalDAV account was not connected: '), error instanceof Error ? fact(error.message) : copy('the provider refused the connection'), copy('.')])) }
      finally { setBusy(false) }
      return
    }
  ...
  return <div className={`term-node calendar-node${selected ? ' selected' : ''}`} style={{ borderTopColor: data.color }} aria-label={mapOwnedSentence(vocab, [copy('Calendar node: '), fact(String(data.title || 'Calendar'))])}>
    ...
    <div className="calendar-node__body">
      <div className="calendar-node__filters"><FieldLabel label="Search events"><Input vocabularyMode="factual" ref={searchInputRef} value={search.value} onChange={(e) => search.setValue(e.target.value)} placeholder={vocab('Plain text search')} aria-label={vocab('Search calendar events')} /></FieldLabel><AnchoredRegexBuilder search={search} fieldRef={searchInputRef} label={vocab('Regex for calendar events')} /></div>
      <div className="calendar-node__source"><FieldLabel label="Source"><Select vocabularyMode="factual" value={config.provider} aria-label={vocab('Calendar source')} onChange={(e) => { const provider = e.target.value as CalendarProvider; setConfig({ provider, accountId: null, calendarId: null }); setEvents(provider === 'local' ? readCached(id) : []); setStatus(vocab(providerHelp[provider])) }}>{PROVIDERS.filter((provider) => sourceSearch.test(calendarProviderName(provider))).map((provider) => <option key={provider} value={provider}>{vocab(calendarProviderName(provider))}</option>)}</Select><div className="calendar-node__picker-search"><Input vocabularyMode="factual" ref={sourceSearchRef} value={sourceSearch.value} onChange={(e) => sourceSearch.setValue(e.target.value)} placeholder={vocab('Filter sources')} aria-label={vocab('Filter calendar sources')} /><AnchoredRegexBuilder search={sourceSearch} fieldRef={sourceSearchRef} label={vocab('Regex for calendar sources')} /></div></FieldLabel><p className="calendar-node__hint">{vocab(providerHelp[config.provider])}</p>

...
type ManagedCopy = { text: string; authored: boolean }

const authoredCopy = (text: string): ManagedCopy => ({ text, authored: true })
const factualCopy = (text: string): ManagedCopy => ({ text, authored: false })
...
export default function CloudflareCoreManagersNode({ id, data, selected }: NodeProps<CanvasNode>): React.JSX.Element {
  ...
  const [runtime, setRuntime] = useState<ManagedCopy>(() => authoredCopy('Checking Cloudflare API client…'))
  const [result, setResult] = useState<CloudflareOperationResult | null>(null)
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState<ManagedCopy>(() => authoredCopy(''))
  const [operationState, setOperationState] = useState<ManagedCopy>(() => authoredCopy(''))
  ...
  useEffect(() => {
    if (!api) { setRuntime(authoredCopy('Cloudflare manager is unavailable in this shell.')); return }
    let active = true
    void Promise.all([api.runtime(), api.credentials(), api.binding(id)]).then(([status, saved, current]) => {
      if (!active) return
      setRuntime(status.available
        ? factualCopy(`${status.origin}: ${status.version ?? vocab('Cloudflare API')}`)
        : status.disabledReason ? factualCopy(status.disabledReason) : authoredCopy('Cloudflare API unavailable'))
    ...
    }).catch((cause) => active && setRuntime(factualCopy(cause instanceof Error ? cause.message : String(cause))))
    return () => { active = false }
  }, [api, id])

  useEffect(() => {
    if (!api) return
    return api.onProgress((progress) => { if (progress.nodeId === id) setOperationState(factualCopy(progress.message)) })
  }, [api, id])
  ...
  const saveCredential = async (): Promise<void> => {
    if (!api) return
    setBusy(true); setError(authoredCopy(''))
    try {
    ...
    } catch (cause) { setError(factualCopy(cause instanceof Error ? cause.message : String(cause))) } finally { setBusy(false) }
  }
  ...
  const run = async (): Promise<void> => {
    if (!api || !credentialReady || (bindingRequired && !binding)) {
      setError(authoredCopy(bindingRequired ? 'Choose a local Cloudflare credential and account or zone first.' : 'Choose a local Cloudflare credential first.'))
      return
    }
    ...
    setBusy(true); setError(authoredCopy('')); setOperationState(authoredCopy('Preparing operation preview…'))
    try {
      const preview = await api.preview(id, request)
      const execute = async (): Promise<void> => {
        try { const next = await api.execute(id, request); setResult(next); setOperationState(factualCopy(next.summary)) }
        catch (cause) { setError(factualCopy(cause instanceof Error ? cause.message : String(cause))) } finally { setBusy(false) }
      }
      if (preview.destructive) {
        const operationLabel = CLOUDFLARE_OPERATION_LABELS[operation]
        openDestructiveGate({
          title: mapAroundExactFacts(`Confirm ${operationLabel}`, [operationLabel], vocab),
          description: mapAroundExactFacts('This Cloudflare operation changes provider state.', ['Cloudflare'], vocab),
          affected: [`${manager} · ${operationLabel}`],
          confirmLabel: vocab('Run operation'),
          onConfirm: () => void execute()
        })
      }
      else await execute()
    } catch (cause) { setError(factualCopy(cause instanceof Error ? cause.message : String(cause))); setBusy(false) }
  }
  ...
  const map = (text: string): string => vocab(text)
  const mapWithFacts = (text: string, facts: readonly string[]): string => mapAroundExactFacts(text, facts, vocab)
  const renderManagedCopy = (copy: ManagedCopy): string => copy.authored ? map(copy.text) : copy.text

  return <div className="cloudflare-core-manager nodrag" aria-label={map('Cloudflare core managers')}>
    <NodeResizer minWidth={620} minHeight={560} isVisible={selected} color="#f38020" />
    <header className="cloudflare-core-manager__header"><strong>{map('Cloudflare managers')}</strong><span>{renderManagedCopy(runtime)}</span></header>
    <section aria-label={map('Local Cloudflare credential')}>
      <h5>{map('Local credential')}</h5>
      <div className="cloudflare-core-manager__search"><Input ref={credentialSearchRef} type="search" value={credentialSearch.value} onChange={(event) => credentialSearch.setValue(event.target.value)} aria-label={map('Search Cloudflare credentials')} placeholder={map('Search credentials')} /><AnchoredRegexBuilder search={credentialSearch} fieldRef={credentialSearchRef} label={map('Regex builder for Cloudflare credentials')} /></div>
      <div role="listbox" aria-label={map('Cloudflare credentials')}>{visibleCredentials.length ? visibleCredentials.map((item) => <button key={item.id} role="option" aria-selected={credentialId === item.id} onClick={() => setCredentialId(item.id)}>{item.label}<small>{item.accountId ?? map('account selected after binding')}</small></button>) : <span>{map('No local Cloudflare credentials are available.')}</span>}</div>
      <Input value={credentialLabel} onChange={(event) => setCredentialLabel(event.target.value)} aria-label={map('New credential label')} placeholder={map('New credential label')} />
      <Input type="password" value={credentialToken} onChange={(event) => setCredentialToken(event.target.value)} aria-label={map('New Cloudflare API token')} placeholder={map('New token, held only in local secure storage')} />
      <Button disabled={busy || !credentialLabel || !credentialToken} onClick={() => void saveCredential()}>{map('Save local credential')}</Button>
      <Input value={accountId} onChange={(event) => setAccountId(event.target.value)} aria-label={map('Cloudflare account id')} placeholder={map('Account id, optional for zone-only managers')} />
      <Input value={zoneId} onChange={(event) => setZoneId(event.target.value)} aria-label={map('Cloudflare zone id')} placeholder={map('Zone id, if known')} />
      <Input value={zoneName} onChange={(event) => setZoneName(event.target.value)} aria-label={map('Cloudflare zone name')} placeholder={map('Zone name, for rebind intent')} />
    ...
    <div className="cloudflare-core-manager__tabs" role="tablist" aria-label={map('Cloudflare manager areas')}>{CLOUDFLARE_MANAGER_KINDS.map((item) => <button key={item} role="tab" aria-selected={manager === item} onClick={() => chooseManager(item)}>{item}</button>)}</div>
    <section aria-label={map('Guided Cloudflare operation')}>
      ...
      <div className="cloudflare-core-manager__toolbar"><div className="cloudflare-core-manager__search"><Input ref={searchRef} type="search" value={resultSearch.value} onChange={(event) => resultSearch.setValue(event.target.value)} aria-label={mapWithFacts(`Search ${manager} results`, [manager])} placeholder={map('Search results')} /><AnchoredRegexBuilder search={resultSearch} fieldRef={searchRef} label={mapWithFacts(`Regex builder for ${manager} results`, [manager])} /></div><Button disabled={busy || !credentialReady || (bindingRequired && !binding)} title={mapWithFacts(bindingRequired && !binding ? 'Configure a local Cloudflare credential and account or zone first.' : 'Preview and run the typed operation', bindingRequired && !binding ? ['Cloudflare'] : [])} onClick={() => void run()}>{map(busy ? 'Running…' : 'Preview and run')}</Button></div>
      <div className="cloudflare-core-manager__search"><Input ref={operationSearchRef} type="search" value={operationSearch.value} onChange={(event) => operationSearch.setValue(event.target.value)} aria-label={map('Search Cloudflare operations')} placeholder={map('Search operations')} /><AnchoredRegexBuilder search={operationSearch} fieldRef={operationSearchRef} label={map('Regex builder for Cloudflare operations')} /></div>
      {resultSearch.error ? <p role="alert">{map(resultSearch.error)}</p> : null}{operationSearch.error ? <p role="alert">{map(operationSearch.error)}</p> : null}{error.text ? <p role="alert">{renderManagedCopy(error)}</p> : null}{operationState.text ? <p role="status">{renderManagedCopy(operationState)}</p> : null}
      {visibleRows.length ? <div role="list">{visibleRows.map((row, index) => <article key={index} role="listitem"><pre>{JSON.stringify(row, null, 2)}</pre></article>)}</div> : <p>{map('No Cloudflare results yet. Choose a manager, bind a local credential, and run a guided operation.')}</p>}
      {result?.nextPage ? <Button disabled={busy} onClick={() => void run()}>{map('Load next page')}</Button> : null}{busy ? <Button onClick={() => void api?.cancel(result?.operationId ?? '')}>{map('Cancel')}</Button> : null}
    </section>
    <TunnelStatePanel
      intent={tunnelIntent}
      live={tunnelState}

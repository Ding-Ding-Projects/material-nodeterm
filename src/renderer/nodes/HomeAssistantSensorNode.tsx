...
/**
 * The sensor node has two kinds of text: copy we own and values received from the instance or
 * entered by the user. Keeping the distinction in a tiny local mapper makes it difficult to
 * accidentally rewrite an entity id, friendly name, timestamp, URL, error, or numeric reading.
 * The shared mapper is fail-closed until vocabulary hydration proves School mode is off.
 */
...
export default function HomeAssistantSensorNode({ id, data, selected }: NodeProps<CanvasNode>): React.JSX.Element {
  ...
  const title = data.title || sensorCopy(vocab, 'Home Assistant sensors')
  ...
  return <div className={`term-node ha-sensor-node${selected ? ' selected' : ''}`} style={{ borderTopColor: data.color }} aria-busy={busy}>
    ...
    <div className="term-node__header" style={{ background: `${data.color}22` }}><span aria-hidden="true">⌂</span><strong>{title}</strong><span className="term-node__spacer" /><span role="status">{bindingStatus}</span></div>
    <div className="ha-sensor-node__body">
      <section aria-labelledby={`${id}-binding-heading`}><h3 id={`${id}-binding-heading`}>{sensorCopy(vocab, 'Instance binding')}</h3><p>{binding.reason == null ? sensorCopy(vocab, 'The instance URL and credential stay only on this computer.') : binding.reason}</p>
      ...
      <section aria-labelledby={`${id}-display-heading`}><h3 id={`${id}-display-heading`}>{sensorCopy(vocab, 'Display and bounded history')}</h3><div className="ha-sensor-node__settings"><label>{sensorCopy(vocab, 'Refresh interval (seconds)')}<input type="number" min={10} max={3600} value={config.refreshSeconds} onChange={(event) => setConfig({ refreshSeconds: Number(event.target.value) })} /></label><label>{sensorCopy(vocab, 'History points per entity')}<input type="number" min={2} max={720} value={config.historyLimit} onChange={(event) => setConfig({ historyLimit: Number(event.target.value) })} /></label><label><input type="checkbox" checked={config.showLastChanged} onChange={(event) => setConfig({ showLastChanged: event.target.checked })} /> {sensorCopy(vocab, 'Show last changed')}</label></div>
        ...
        <div className="ha-sensor-node__actions"><button type="button" disabled={binding.state !== 'ready' || busy || config.entities.length === 0} onClick={() => void refresh()}>{sensorCopy(vocab, 'Refresh now')}</button><button type="button" disabled={!snapshot} onClick={() => void exportSnapshot()}>{sensorCopy(vocab, 'Export visible data')}</button></div>{snapshot && <p role="status">{snapshot.stale ? mapSensorSentence(vocab, [copy('Showing the last successful observation from '), sensorFact(snapshot.fetchedAt), copy('.')]) : mapSensorSentence(vocab, [copy('Observed '), sensorFact(snapshot.entities.length), copy(' selected entities at '), sensorFact(snapshot.fetchedAt), copy('. ')])}{snapshot && (snapshot.stale ? sensorCopy(vocab, 'Refresh to try the live instance again.') : snapshot.complete ? sensorCopy(vocab, 'The selected set was complete.') : snapshot.reason ?? '')}</p>}
      </section>
    </div>
  </div>
}

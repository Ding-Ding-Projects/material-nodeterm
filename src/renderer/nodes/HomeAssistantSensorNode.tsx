...
export default function HomeAssistantSensorNode({ id, data, selected }: NodeProps<CanvasNode>): React.JSX.Element {
  ...
  useEffect(() => {
    let active = true
    void api.homeAssistantSensor.binding(id).then((next) => { if (active) setBinding(next) }).catch(() => { if (active) setBinding({ ...EMPTY_BINDING, nodeId: id, state: 'unavailable', reason: null }) })
    return () => { active = false }
  ...
  return <div className={`term-node ha-sensor-node${selected ? ' selected' : ''}`} style={{ borderTopColor: data.color }} aria-busy={busy}>
    ...
    <div className="ha-sensor-node__body">
      <section aria-labelledby={`${id}-binding-heading`}><h3 id={`${id}-binding-heading`}>{sensorCopy(vocab, 'Instance binding')}</h3><p>{binding.reason ? sensorFact(binding.reason).text : binding.state === 'unavailable' ? sensorCopy(vocab, 'The local binding could not be read.') : sensorCopy(vocab, 'The instance URL and credential stay only on this computer.')}</p>
        <div className="ha-sensor-node__actions"><Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => setConfigureOpen(true)}>{sensorCopy(vocab, binding.state === 'ready' ? 'Rebind…' : 'Configure…')}</Button><Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => setConfigureOpen(true)}>{sensorCopy(vocab, 'Adopt…')}</Button><Button variant="outlined" size="small" vocabularyMode="factual" disabled title={sensorCopy(vocab, 'Sensor display nodes do not deploy Home Assistant. Use an installed instance.')}>{sensorCopy(vocab, 'Deploy…')}</Button><Button variant="outlined" size="small" vocabularyMode="factual" disabled title={sensorCopy(vocab, 'This integration uses a network instance and has no local asset to locate.')}>{sensorCopy(vocab, 'Locate Asset…')}</Button><Button variant="outlined" size="small" vocabularyMode="factual" disabled={binding.state === 'unbound'} onClick={(event) => { const target = event.currentTarget; const rect = target.getBoundingClientRect(); openDestructiveGate({ title: sensorCopy(vocab, 'Leave this Home Assistant node unbound'), description: sensorCopy(vocab, 'Remove the machine-local instance URL, sealed credential, cache, and observed history for this node.'), affected: [binding.instanceLabel ?? id], confirmLabel: sensorCopy(vocab, 'Leave unbound'), anchor: { x: rect.left, y: rect.bottom }, restoreFocusEl: target, onConfirm: () => { void api.homeAssistantSensor.leaveUnbound(id).then((next) => { setBinding(next); setCatalog([]); setSnapshot(null); report('success', [copy('Home Assistant binding removed')], [copy('This node is unbound on this computer. Shared entity and display intent remains available for a later rebind.')]) }).catch((error) => report('error', [copy('Binding removal failed')], [error instanceof Error ? sensorFact(error.message) : copy('The local binding may still exist.')])) } }) }}>{sensorCopy(vocab, 'Leave Unbound…')}</Button></div>
        {configureOpen && <div className="ha-sensor-node__configure" role="region" aria-label={sensorCopy(vocab, 'Configure Home Assistant instance')}><label>{sensorCopy(vocab, 'Instance label')}<Input vocabularyMode="factual" value={instanceLabel} onChange={(event) => setInstanceLabel(event.target.value)} maxLength={120} placeholder={sensorCopy(vocab, 'Home')} /></label><label>{sensorCopy(vocab, 'Base URL')}<Input vocabularyMode="factual" value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} inputMode="url" placeholder={sensorCopy(vocab, 'https://homeassistant.example.com')} /></label><label>{sensorCopy(vocab, 'Long-lived access credential')}<Input vocabularyMode="factual" type="password" value={token} onChange={(event) => setToken(event.target.value)} autoComplete="off" /></label><p>{sensorCopy(vocab, 'Nothing is saved until the URL and credential are verified. The credential is never shown again, exported, logged, or added to the project.')}</p><div className="ha-sensor-node__actions"><Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => { setConfigureOpen(false); setToken('') }}>{sensorCopy(vocab, 'Cancel')}</Button><Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => void configure()} disabled={busy || !baseUrl.trim() || !token}>{sensorCopy(vocab, 'Verify and bind')}</Button></div></div>}
      </section>
      <section aria-labelledby={`${id}-entities-heading`}><h3 id={`${id}-entities-heading`}>{sensorCopy(vocab, 'Entities')}</h3><div className="ha-sensor-node__search"><Input vocabularyMode="factual" ref={entitySearchRef} value={entitySearch.value} onChange={(event) => entitySearch.setValue(event.target.value)} placeholder={sensorCopy(vocab, 'Search entities with plain text')} aria-label={sensorCopy(vocab, 'Search Home Assistant entities')} /><AnchoredRegexBuilder search={entitySearch} fieldRef={entitySearchRef} label={sensorCopy(vocab, 'Regex for Home Assistant entities')} /><Button variant="outlined" size="small" vocabularyMode="factual" disabled={binding.state !== 'ready' || busy} onClick={() => void discover()}>{sensorCopy(vocab, 'Load entities')}</Button></div>
        {catalog.length === 0 ? <p className="ha-sensor-node__empty">{sensorCopy(vocab, 'No catalog loaded. Bind an instance, then load its real entity list.')}</p> : <div className="ha-sensor-node__catalog" role="list" aria-label={sensorCopy(vocab, 'Home Assistant entity picker')}>{visibleCatalog.map((entity) => <label key={entity.entityId} role="listitem"><Checkbox vocabularyMode="factual" checked={config.entities.some((entry) => entry.entityId === entity.entityId)} onChange={(event) => selectEntity(entity, event.target.checked)} /><span><strong>{entity.friendlyName}</strong><small>{entity.entityId} · {entity.state}{entity.unit ? ` ${entity.unit}` : ''}</small></span></label>)}</div>}
      </section>
      <section aria-labelledby={`${id}-display-heading`}><h3 id={`${id}-display-heading`}>{sensorCopy(vocab, 'Display and bounded history')}</h3><div className="ha-sensor-node__settings"><label>{sensorCopy(vocab, 'Refresh interval (seconds)')}<Input vocabularyMode="factual" type="number" min={10} max={3600} value={config.refreshSeconds} onChange={(event) => setConfig({ refreshSeconds: Number(event.target.value) })} /></label><label>{sensorCopy(vocab, 'History points per entity')}<Input vocabularyMode="factual" type="number" min={2} max={720} value={config.historyLimit} onChange={(event) => setConfig({ historyLimit: Number(event.target.value) })} /></label><label><Checkbox vocabularyMode="factual" checked={config.showLastChanged} onChange={(event) => setConfig({ showLastChanged: event.target.checked })} /> {sensorCopy(vocab, 'Show last changed')}</label></div>
        <div className="ha-sensor-node__search"><Input vocabularyMode="factual" ref={modeSearchRef} value={modeSearch.value} onChange={(event) => modeSearch.setValue(event.target.value)} placeholder={sensorCopy(vocab, 'Filter display modes')} aria-label={sensorCopy(vocab, 'Filter Home Assistant display modes')} /><AnchoredRegexBuilder search={modeSearch} fieldRef={modeSearchRef} label={sensorCopy(vocab, 'Regex for Home Assistant display modes')} /></div>
        <div className="ha-sensor-node__cards">{selectedEntities.length === 0 ? <p className="ha-sensor-node__empty">{sensorCopy(vocab, 'Choose at least one entity from the real catalog.')}</p> : selectedEntities.map(({ entry, entity }) => <article key={entity.entityId}><header><div><strong>{entry.label || entity.friendlyName}</strong><small>{entity.entityId} · {sensorCopy(vocab, displayModeLabel(resolvedMode(entry, entity)))}</small></div><Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => selectEntity(entity, false)} aria-label={mapSensorSentence(vocab, [copy('Remove '), sensorFact(entity.friendlyName), copy(' from this display')])}>{sensorCopy(vocab, 'Remove')}</Button></header><EntityValue entity={entity} binding={entry} points={(snapshot?.history ?? []).filter((point) => point.entityId === entity.entityId)} map={vocab} />{config.showLastChanged && <p className="ha-sensor-node__muted">{mapSensorSentence(vocab, [copy('Changed '), sensorFact(entity.lastChanged)])}</p>}<details><summary>{sensorCopy(vocab, 'Display options')}</summary><div className="ha-sensor-node__mode-grid">{HOME_ASSISTANT_DISPLAY_MODES.filter((mode) => modeSearch.test(`${mode.label} ${mode.hint}`)).map((mode) => <Chip vocabularyMode="factual" selected={entry.mode === mode.id} key={mode.id} aria-pressed={entry.mode === mode.id} title={sensorCopy(vocab, mode.hint)} onClick={() => updateEntity(entity.entityId, { mode: mode.id })}>{sensorCopy(vocab, mode.label)}</Chip>)}</div>{resolvedMode(entry, entity) === 'gauge' && <div className="ha-sensor-node__settings"><label>{sensorCopy(vocab, 'Minimum')}<Input vocabularyMode="factual" type="number" value={entry.min ?? 0} onChange={(event) => updateEntity(entity.entityId, { min: Number(event.target.value) })} /></label><label>{sensorCopy(vocab, 'Maximum')}<Input vocabularyMode="factual" type="number" value={entry.max ?? 100} onChange={(event) => updateEntity(entity.entityId, { max: Number(event.target.value) })} /></label></div>}{resolvedMode(entry, entity) === 'attributes' && <fieldset><legend>{sensorCopy(vocab, 'Attributes to show')}</legend>{Object.keys(entity.attributes).map((key) => <label key={key}><Checkbox vocabularyMode="factual" checked={entry.attributeKeys.includes(key)} onChange={(event) => updateEntity(entity.entityId, { attributeKeys: event.target.checked ? [...entry.attributeKeys, key] : entry.attributeKeys.filter((candidate) => candidate !== key) })} /> {key}</label>)}</fieldset>}</details></article>)}</div>
        <div className="ha-sensor-node__actions"><Button variant="outlined" size="small" vocabularyMode="factual" disabled={binding.state !== 'ready' || busy || config.entities.length === 0} onClick={() => void refresh()}>{sensorCopy(vocab, 'Refresh now')}</Button><Button variant="outlined" size="small" vocabularyMode="factual" disabled={!snapshot} onClick={() => void exportSnapshot()}>{sensorCopy(vocab, 'Export visible data')}</Button></div>{snapshot && <p role="status">{snapshot.stale ? mapSensorSentence(vocab, [copy('Showing the last successful observation from '), sensorFact(snapshot.fetchedAt), copy('. ')]) : mapSensorSentence(vocab, [copy('Observed '), sensorFact(snapshot.entities.length), copy(' selected entities at '), sensorFact(snapshot.fetchedAt), copy('. ')])}{snapshot.stale ? sensorCopy(vocab, 'Refresh to try the live instance again.') : snapshot.complete ? sensorCopy(vocab, 'The selected set was complete.') : snapshot.reason ?? ''}</p>}
      </section>
    </div>
  </div>
}

  ...
  return (
    <div className={`term-node alarm-clock-node${selected ? ' selected' : ''}`} style={{ borderTopColor: data.color }}>
      ...
      <div className={`term-node__header ${headerFill.className}${headerFill.filled ? ' term-node__header--filled' : ''}`} style={headerFill.style}>
        ...
        <IconButton size="compact" className="term-node__close" icon="close" vocabularyMode="factual" title={vocab('Close')} aria-label={vocab('Close Alarm Clock')} onClick={() => {
      ...
      <div className="alarm-clock-node__body nodrag nowheel">
        <label>{vocab('Schedule')}
          <Select vocabularyMode="factual" value={schedule.recurrence} aria-label={vocab('Alarm recurrence')} onChange={(event) => updateSchedule({ recurrence: event.target.value as AlarmRecurrence })}>
            {RECURRENCES.map((entry) => <option key={entry.value} value={entry.value}>{vocab(entry.label)}</option>)}
          </Select>
        </label>
        {schedule.recurrence !== 'daily' && schedule.recurrence !== 'weekdays' && schedule.recurrence !== 'weekly' && schedule.recurrence !== 'monthly' ? <label>{vocab('Date')} <Input vocabularyMode="factual" type="date" value={schedule.date ?? ''} aria-label={vocab('Alarm date')} onChange={(event) => updateSchedule({ date: event.target.value })} /></label> : null}
        <label>{vocab('Time')} <Input vocabularyMode="factual" type="time" step={1} value={schedule.time} aria-label={vocab('Alarm local time')} onChange={(event) => updateSchedule({ time: event.target.value })} /></label>
        <label>{vocab('Timezone')} <Select vocabularyMode="factual" value={timeZone} aria-label={vocab('Alarm timezone')} onChange={(event) => patch({ alarmTimeZone: event.target.value, alarmNextOccurrenceAt: undefined })}>{timezoneChoices(timeZone).map((zone) => <option key={zone} value={zone}>{zone}</option>)}</Select></label>
        {schedule.recurrence === 'weekly' ? <fieldset><legend>{vocab('Weekdays')}</legend><div className="alarm-clock-node__days">{WEEKDAYS.map((day, index) => <label key={day}><Checkbox checked={schedule.weekdays?.includes(index) ?? false} onChange={(event) => updateSchedule({ weekdays: event.target.checked ? [...new Set([...(schedule.weekdays ?? []), index])] : (schedule.weekdays ?? []).filter((value) => value !== index) })} />{vocab(day)}</label>)}</div></fieldset> : null}
        {schedule.recurrence === 'monthly' ? <label>{vocab('Day of month')} <Input vocabularyMode="factual" type="number" min={1} max={31} value={schedule.monthDay ?? 1} aria-label={vocab('Alarm day of month')} onChange={(event) => updateSchedule({ monthDay: Number(event.target.value) })} /></label> : null}
        <div className="alarm-clock-node__options"><label>{vocab('Snooze (minutes)')} <Input vocabularyMode="factual" type="number" min={1} max={120} value={data.alarmSnoozeMinutes ?? 10} aria-label={vocab('Snooze minutes')} onChange={(event) => patch({ alarmSnoozeMinutes: Math.min(120, Math.max(1, Number(event.target.value) || 10)) })} /></label><label><Checkbox checked={data.alarmSoundEnabled !== false} onChange={(event) => patch({ alarmSoundEnabled: event.target.checked })} /> {vocab('Sound')}</label><label><Checkbox checked={data.alarmNarratorEnabled !== false} onChange={(event) => patch({ alarmNarratorEnabled: event.target.checked })} /> {vocab('Narrator')}</label></div>
        ...
        <div className="alarm-clock-node__actions"><Button variant={data.alarmEnabled ? 'tonal' : 'filled'} size="small" vocabularyMode="factual" disabled={!canStart} className={data.alarmEnabled ? 'active' : ''} onClick={toggle}>{vocab(data.alarmEnabled ? 'Pause' : 'Start alarm')}</Button></div>
        <div className="alarm-clock-node__history"><div className="alarm-clock-node__history-head"><strong>{vocab('Occurrence history')}</strong><span>{fact(String(history.length)).text}</span></div><div className="alarm-clock-node__search"><Input type="search" vocabularyMode="factual" value={search} placeholder={vocab('Search history')} aria-label={vocab('Search alarm history')} onChange={(event) => setSearch(event.target.value)} /><IconButton size="compact" icon="regular_expression" vocabularyMode="factual" active={regexOpen} aria-expanded={regexOpen} title={vocab('Open regex builder')} aria-label={vocab('Open regex builder')} onClick={() => setRegexOpen((open) => !open)} /></div>{regexOpen ? <div className="alarm-clock-node__regex" role="dialog" aria-label={vocab('Alarm history regex builder')}><label>{vocab('Pattern')} <Input vocabularyMode="factual" value={regexPattern} aria-label={vocab('Regex pattern')} onChange={(event) => setRegexPattern(event.target.value)} /></label><label>{vocab('Flags')} <Input vocabularyMode="factual" value={regexFlags} aria-label={vocab('Regex flags')} onChange={(event) => setRegexFlags(event.target.value)} /></label><span role="status">{regexPattern && !regex ? vocab('Invalid regular expression.') : vocab('Plain text is the default.')}</span></div> : null}{visibleHistory.length === 0 ? <p className="alarm-clock-node__empty">{vocab('No matching alarm occurrences yet.')}</p> : <ul>{visibleHistory.map((occurrence) => <li key={occurrence.id}><span><b>{fact(occurrence.status).text}</b> {fact(displayTime(occurrence.scheduledAt, occurrence.timeZone)).text}</span>{occurrence.status === 'fired' || occurrence.status === 'snoozed' ? <span><Button variant="tonal" size="small" vocabularyMode="factual" onClick={() => snooze(occurrence)}>{vocab('Snooze')}</Button><Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => dismiss(occurrence)}>{vocab('Dismiss')}</Button></span> : null}</li>)}</ul>}</div>
      </div>
    </div>
  )
}

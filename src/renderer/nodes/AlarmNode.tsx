...
type AlarmNodeProps = NodeProps<{ title: string; alarmId?: NodeData['alarmId'] }>

/** Alarm canvas projection. The host owns scheduling, delivery claims, and missed state. */
export function AlarmNode({ id, data }: AlarmNodeProps) {
  const [alarm, setAlarm] = useState<DurableAlarmNode | undefined>()
  const t = useVocabularyMapper()
  useEffect(() => { let active = true; void window.nodeTerminal.durableOccurrences.load().then(async (state) => { if (!active || !state.ok) return; const match = state.snapshot.alarms.find((item) => item.id === data.alarmId || item.canvasNodeId === id); if (match) { setAlarm(match); return } if (!data.alarmId) return; const now = new Date(); const date = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`; const created = { id: data.alarmId, canvasNodeId: id, title: data.title, enabled: false, timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC', startLocal: `${date}T09:00`, recurrence: { kind: 'once' as const }, snoozeMinutes: 10, soundEnabled: true, narratorEnabled: false, createdAtMs: Date.now(), updatedAtMs: Date.now() }; const result = await window.nodeTerminal.durableOccurrences.upsertAlarm(created); if (result.ok) setAlarm(created) }); return () => { active = false } }, [data.alarmId, data.title, id])
  useEffect(() => window.nodeTerminal.durableOccurrences.onChanged((snapshot) => { const match = snapshot.alarms.find((item) => item.id === data.alarmId || item.canvasNodeId === id); if (match) setAlarm(match) }), [data.alarmId, id])

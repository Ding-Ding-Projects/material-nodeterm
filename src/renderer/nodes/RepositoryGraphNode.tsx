...
export default function RepositoryGraphNode({ id, data, selected }: NodeProps<CanvasNode>): React.JSX.Element {
  ...
  const status = error ?? (progress?.message ?? (snapshot?.status === 'idle' ? 'No verified snapshot. Refresh to index this project.' : snapshot ? `${snapshot.nodes.length} nodes, ${snapshot.edges.length} edges, revision ${snapshot.fingerprint.revision}` : 'Reading project graph…'))
  ...
  return (
    <div className={`term-node repository-graph-node${selected ? ' selected' : ''}`} style={{ borderTopColor: data.color }}>
      ...
      <div className="repository-graph-node__body nodrag nowheel">
        <p className={`repository-graph-node__status${error ? ' is-error' : ''}`} role={error ? 'alert' : 'status'}>{vocab(status)}</p>

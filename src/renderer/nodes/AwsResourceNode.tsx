...
export default function AwsResourceNode({ id, data, selected }: NodeProps<CanvasNode>): React.JSX.Element {
  ...
  const operations: readonly AwsManagerOperation[] = mode === 'cloud-control' ? CLOUD_OPERATIONS : mode === 'core-services' ? AWS_CORE_OPERATIONS[coreService] : mode === 'cloudformation' ? CLOUDFORMATION_OPERATIONS : mode === 'platform-managers' ? AWS_PLATFORM_OPERATIONS.filter((item) => item.startsWith(`${platformService}-`)) : RESOURCE_OPERATIONS
  useEffect(() => {
  ...
  const persistIntent = (nextMode: AwsManagerMode = mode, overrides: Partial<Pick<AwsManagerPortableIntent, 'regionIntent' | 'resourceQuery' | 'cloudControlTypeName' | 'coreService' | 'coreOperation' | 'coreInput' | 'platformService' | 'platformOperation' | 'platformInput'>> = {}): void => {
    updateNodeData(id, {
      awsManagerIntent: {
        schemaVersion: 1,
        mode: nextMode,
  ...
  const title = data.title || (mode === 'cloud-control' ? 'AWS Cloud Control' : mode === 'core-services' ? `${CORE_SERVICE_LABELS[coreService]} manager` : mode === 'cloudformation' ? 'AWS CloudFormation' : mode === 'platform-managers' ? `${platformService.toUpperCase()} manager` : 'AWS Resource Explorer')

  ...
  return (
    <div className={`term-node aws-resource-node${selected ? ' selected' : ''}`} style={{ borderTopColor: data.color }}>
      ...
      <div className={`term-node__header ${fill.className}${fill.filled ? ' term-node__header--filled' : ''}`} style={fill.style}>
        ...
        <IconButton size="compact" className="aws-resource-node__refresh" icon="refresh" vocabularyMode="factual" onClick={() => void load()} disabled={busy} aria-label={vocab('Refresh AWS status')} title={vocab('Refresh AWS status')} />
      </div>
      <div className="aws-resource-node__body nodrag nowheel">
        ...
        <section className="aws-resource-node__binding" aria-label={vocab('Local AWS binding')}>
          <div className="aws-resource-node__binding-grid">
            <label>{vocab('Profile')}
              <Input vocabularyMode="factual" list={`${id}-profiles`} value={profileName} onChange={(event) => setProfileName(event.target.value)} placeholder={vocab('Choose a configured profile')} />
            ...
            <label>{vocab('Region')}
              <Input vocabularyMode="factual" list={`${id}-regions`} value={region} onChange={(event) => { setRegion(event.target.value); persistIntent(mode, { regionIntent: event.target.value }) }} />
              <datalist id={`${id}-regions`}>{REGION_OPTIONS.map((item) => <option key={item} value={item} />)}</datalist>
            </label>
            <label>{vocab('Endpoint (optional)')}
              <Input vocabularyMode="factual" value={endpointUrl} onChange={(event) => setEndpointUrl(event.target.value)} placeholder="https://endpoint.example" />
            </label>
          </div>
          <div className="aws-resource-node__actions">
            <Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => void bind()} disabled={busy || !profileName.trim() || !region.trim()}>{vocab('Save local binding')}</Button>
        ...
        {mode === 'cloudformation' && <section className="aws-resource-node__inputs" aria-label={vocab('CloudFormation change-set inputs')}>
          ...
          <label>{vocab('Stack name')}<Input vocabularyMode="factual" value={stackName} onChange={(event) => setStackName(event.target.value)} placeholder={vocab('Choose a stack name')} /></label>
          <label>{vocab('Change-set name')}<Input vocabularyMode="factual" value={changeSetName} onChange={(event) => setChangeSetName(event.target.value)} placeholder={vocab('preview')} /></label>
          {['cloudformation-describe-change-set', 'cloudformation-execute-change-set', 'cloudformation-delete-change-set'].includes(operation) && <label>{vocab('Existing change-set name')}<Input vocabularyMode="factual" value={changeSetName} onChange={(event) => setChangeSetName(event.target.value)} placeholder={vocab('Choose a change-set name')} /></label>}
          <div className="aws-resource-node__operations" role="radiogroup" aria-label={vocab('CloudFormation change-set type')}><Chip vocabularyMode="factual" selected={changeSetType === 'CREATE'} role="radio" aria-checked={changeSetType === 'CREATE'} className={changeSetType === 'CREATE' ? 'is-selected' : ''} onClick={() => setChangeSetType('CREATE')}>{vocab('Create')}</Chip><Chip vocabularyMode="factual" selected={changeSetType === 'UPDATE'} role="radio" aria-checked={changeSetType === 'UPDATE'} className={changeSetType === 'UPDATE' ? 'is-selected' : ''} onClick={() => setChangeSetType('UPDATE')}>{vocab('Update')}</Chip></div>
          <label>{vocab('Template parameters')}</label>
          {cfParameters.map((parameter, index) => <div key={`${parameter.key}-${index}`} className="aws-resource-node__parameter-row"><Input vocabularyMode="factual" value={parameter.key} onChange={(event) => setCfParameters((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, key: event.target.value } : item))} placeholder={vocab('Parameter key')} /><Input vocabularyMode="factual" value={parameter.value ?? ''} disabled={parameter.usePreviousValue === true} onChange={(event) => setCfParameters((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, value: event.target.value } : item))} placeholder={vocab('Parameter value')} /><label><Checkbox vocabularyMode="factual" checked={parameter.usePreviousValue === true} onChange={(event) => setCfParameters((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, usePreviousValue: event.target.checked } : item))} /> {vocab('Use previous')}</label></div>)}
        ...
        <section className="aws-resource-node__inputs" aria-label={fieldLabel(operation, vocab)}>
          {mode === 'core-services' && <>
            ...
            {['ec2-start-instances', 'ec2-stop-instances', 'ec2-terminate-instances'].includes(operation) && <label>{vocab('Instance IDs')}<Input vocabularyMode="factual" value={String(coreInput.instanceIds ?? '')} onChange={(event) => { const next = { ...coreInput, instanceIds: event.target.value }; setCoreInput(next); persistIntent('core-services', { coreInput: next as AwsManagerPortableIntent['coreInput'] }) }} placeholder="i-0123..., i-0456..." /></label>}
            {['iam-get-user', 'iam-create-user', 'iam-delete-user'].includes(operation) && <label>{vocab('User name')}<Input vocabularyMode="factual" value={String(coreInput.userName ?? '')} onChange={(event) => { const next = { ...coreInput, userName: event.target.value }; setCoreInput(next); persistIntent('core-services', { coreInput: next as AwsManagerPortableIntent['coreInput'] }) }} /></label>}
            {['iam-get-role'].includes(operation) && <label>{vocab('Role name')}<Input vocabularyMode="factual" value={String(coreInput.roleName ?? '')} onChange={(event) => { const next = { ...coreInput, roleName: event.target.value }; setCoreInput(next); persistIntent('core-services', { coreInput: next as AwsManagerPortableIntent['coreInput'] }) }} /></label>}
            {['lambda-get-function', 'lambda-delete-function'].includes(operation) && <label>{vocab('Function name')}<Input vocabularyMode="factual" value={String(coreInput.functionName ?? '')} onChange={(event) => { const next = { ...coreInput, functionName: event.target.value }; setCoreInput(next); persistIntent('core-services', { coreInput: next as AwsManagerPortableIntent['coreInput'] }) }} /></label>}
            {operation === 'cloudwatch-get-metric-data' && <><label>{vocab('Metric data queries JSON')}<TextArea vocabularyMode="factual" value={String(coreInput.metricDataQueries ?? '')} onChange={(event) => { const next = { ...coreInput, metricDataQueries: event.target.value }; setCoreInput(next); persistIntent('core-services', { coreInput: next as AwsManagerPortableIntent['coreInput'] }) }} /></label><label>{vocab('Start time')}<Input vocabularyMode="factual" value={String(coreInput.startTime ?? '')} onChange={(event) => { const next = { ...coreInput, startTime: event.target.value }; setCoreInput(next); persistIntent('core-services', { coreInput: next as AwsManagerPortableIntent['coreInput'] }) }} type="datetime-local" /></label><label>{vocab('End time')}<Input vocabularyMode="factual" value={String(coreInput.endTime ?? '')} onChange={(event) => { const next = { ...coreInput, endTime: event.target.value }; setCoreInput(next); persistIntent('core-services', { coreInput: next as AwsManagerPortableIntent['coreInput'] }) }} type="datetime-local" /></label></>}
            {operation === 'cloudwatch-list-metrics' && <label>{vocab('Namespace (optional)')}<Input vocabularyMode="factual" value={String(coreInput.namespace ?? '')} onChange={(event) => { const next = { ...coreInput, namespace: event.target.value }; setCoreInput(next); persistIntent('core-services', { coreInput: next as AwsManagerPortableIntent['coreInput'] }) }} /></label>}
            {['logs-describe-log-streams', 'logs-get-log-events', 'logs-filter-log-events'].includes(operation) && <label>{vocab('Log group name')}<Input vocabularyMode="factual" value={String(coreInput.logGroupName ?? '')} onChange={(event) => { const next = { ...coreInput, logGroupName: event.target.value }; setCoreInput(next); persistIntent('core-services', { coreInput: next as AwsManagerPortableIntent['coreInput'] }) }} /></label>}
            {['logs-get-log-events'].includes(operation) && <label>{vocab('Log stream name')}<Input vocabularyMode="factual" value={String(coreInput.logStreamName ?? '')} onChange={(event) => { const next = { ...coreInput, logStreamName: event.target.value }; setCoreInput(next); persistIntent('core-services', { coreInput: next as AwsManagerPortableIntent['coreInput'] }) }} /></label>}
            {['logs-filter-log-events'].includes(operation) && <label>{vocab('Filter pattern')}<Input vocabularyMode="factual" value={String(coreInput.filterPattern ?? '')} onChange={(event) => { const next = { ...coreInput, filterPattern: event.target.value }; setCoreInput(next); persistIntent('core-services', { coreInput: next as AwsManagerPortableIntent['coreInput'] }) }} /></label>}
          </>}
          {mode === 'platform-managers' && <>
            {['ecr-describe-images', 'ecr-create-repository', 'ecr-delete-repository'].includes(operation) && <label>{vocab('Repository name')}<Input vocabularyMode="factual" value={String(coreInput.repositoryName ?? '')} onChange={(event) => updatePlatformInput('repositoryName', event.target.value)} placeholder={vocab('Choose a repository name')} /></label>}
            {operation === 'ecr-create-repository' && <label>{vocab('Tag mutability')}<Select vocabularyMode="factual" value={String(coreInput.tagMutability ?? 'IMMUTABLE')} onChange={(event) => updatePlatformInput('tagMutability', event.target.value)}><option value="IMMUTABLE">{vocab('Immutable')}</option><option value="MUTABLE">{vocab('Mutable')}</option></Select></label>}
            {['ecs-list-services', 'ecs-update-service', 'ecs-delete-service'].includes(operation) && <label>{vocab('Cluster name')}<Input vocabularyMode="factual" value={String(coreInput.cluster ?? '')} onChange={(event) => updatePlatformInput('cluster', event.target.value)} placeholder={vocab('Choose a discovered cluster')} /></label>}
            {['ecs-update-service', 'ecs-delete-service'].includes(operation) && <label>{vocab('Service name')}<Input vocabularyMode="factual" value={String(coreInput.service ?? '')} onChange={(event) => updatePlatformInput('service', event.target.value)} /></label>}
            {operation === 'ecs-update-service' && <label>{vocab('Desired task count')}<Input vocabularyMode="factual" type="number" min={0} max={1000} step={1} value={Number(coreInput.desiredCount ?? 1)} onChange={(event) => updatePlatformInput('desiredCount', Math.max(0, Math.min(1000, Number(event.target.value) || 0)))} /></label>}
            {['eks-describe-cluster', 'eks-update-nodegroup', 'eks-delete-cluster'].includes(operation) && <label>{vocab('Cluster name')}<Input vocabularyMode="factual" value={String(coreInput.clusterName ?? '')} onChange={(event) => updatePlatformInput('clusterName', event.target.value)} /></label>}
            {operation === 'eks-update-nodegroup' && <><label>{vocab('Node group name')}<Input vocabularyMode="factual" value={String(coreInput.nodegroupName ?? '')} onChange={(event) => updatePlatformInput('nodegroupName', event.target.value)} /></label><label>{vocab('Minimum nodes')}<Input vocabularyMode="factual" type="number" min={0} max={1000} value={Number(coreInput.minimum ?? 1)} onChange={(event) => updatePlatformInput('minimum', Math.max(0, Math.min(1000, Number(event.target.value) || 0)))} /></label><label>{vocab('Desired nodes')}<Input vocabularyMode="factual" type="number" min={0} max={1000} value={Number(coreInput.desired ?? 2)} onChange={(event) => updatePlatformInput('desired', Math.max(0, Math.min(1000, Number(event.target.value) || 0)))} /></label><label>{vocab('Maximum nodes')}<Input vocabularyMode="factual" type="number" min={1} max={1000} value={Number(coreInput.maximum ?? 4)} onChange={(event) => updatePlatformInput('maximum', Math.max(1, Math.min(1000, Number(event.target.value) || 1)))} /></label></>}
            {['rds-describe-db-instances', 'rds-create-db-instance', 'rds-create-db-snapshot', 'rds-delete-db-instance'].includes(operation) && <label>{vocab('Database identifier')}<Input vocabularyMode="factual" value={String(coreInput.identifier ?? '')} onChange={(event) => updatePlatformInput('identifier', event.target.value)} /></label>}
            {operation === 'rds-create-db-instance' && <><label>{vocab('Instance class')}<Input vocabularyMode="factual" value={String(coreInput.instanceClass ?? 'db.t3.micro')} onChange={(event) => updatePlatformInput('instanceClass', event.target.value)} /></label><label>{vocab('Engine')}<Select vocabularyMode="factual" value={String(coreInput.engine ?? 'postgres')} onChange={(event) => updatePlatformInput('engine', event.target.value)}><option value="postgres">{vocab('PostgreSQL')}</option><option value="mysql">{vocab('MySQL')}</option><option value="aurora-postgresql">{vocab('Aurora PostgreSQL')}</option><option value="aurora-mysql">{vocab('Aurora MySQL')}</option></Select></label><label>{vocab('Allocated storage (GiB)')}<Input vocabularyMode="factual" type="number" min={20} max={65536} value={Number(coreInput.storageGiB ?? 20)} onChange={(event) => updatePlatformInput('storageGiB', Math.max(20, Math.min(65536, Number(event.target.value) || 20)))} /></label><label>{vocab('Backup retention (days)')}<Input vocabularyMode="factual" type="number" min={0} max={35} value={Number(coreInput.backupDays ?? 7)} onChange={(event) => updatePlatformInput('backupDays', Math.max(0, Math.min(35, Number(event.target.value) || 0)))} /></label></>}
            {operation === 'rds-create-db-snapshot' && <label>{vocab('Snapshot identifier')}<Input vocabularyMode="factual" value={String(coreInput.snapshotIdentifier ?? '')} onChange={(event) => updatePlatformInput('snapshotIdentifier', event.target.value)} /></label>}
            {['database-list-tables', 'database-create-table', 'database-delete-table'].includes(operation) && <label>{vocab('Table name')}<Input vocabularyMode="factual" value={String(coreInput.tableName ?? '')} onChange={(event) => updatePlatformInput('tableName', event.target.value)} /></label>}
             {operation === 'database-create-table' && <><label>{vocab('Attribute definitions JSON')}<TextArea vocabularyMode="factual" value={String(coreInput.attributeDefinitions ?? 'AttributeName=id,AttributeType=S')} onChange={(event) => updatePlatformInput('attributeDefinitions', event.target.value)} /></label><label>{vocab('Key schema JSON')}<TextArea vocabularyMode="factual" value={String(coreInput.keySchema ?? 'AttributeName=id,KeyType=HASH')} onChange={(event) => updatePlatformInput('keySchema', event.target.value)} /></label></>}
             {['vpc-describe-vpcs', 'vpc-create-subnet', 'vpc-delete-vpc'].includes(operation) && <label>{vocab('VPC id')}<Input vocabularyMode="factual" value={String(coreInput.vpcId ?? '')} onChange={(event) => updatePlatformInput('vpcId', event.target.value)} /></label>}
             {['vpc-create-vpc', 'vpc-create-subnet'].includes(operation) && <label>{vocab('IPv4 CIDR')}<Input vocabularyMode="factual" value={String(coreInput.cidr ?? '')} onChange={(event) => updatePlatformInput('cidr', event.target.value)} placeholder="10.0.0.0/16" /></label>}
             {operation === 'route53-change-record' && <><label>{vocab('Hosted zone id')}<Input vocabularyMode="factual" value={String(coreInput.hostedZoneId ?? '')} onChange={(event) => updatePlatformInput('hostedZoneId', event.target.value)} /></label><label>{vocab('Change batch JSON')}<TextArea vocabularyMode="factual" value={String(coreInput.changeBatch ?? '{"Changes":[]}')} onChange={(event) => updatePlatformInput('changeBatch', event.target.value)} /></label></>}
             {operation === 'route53-delete-hosted-zone' && <label>{vocab('Hosted zone id')}<Input vocabularyMode="factual" value={String(coreInput.hostedZoneId ?? '')} onChange={(event) => updatePlatformInput('hostedZoneId', event.target.value)} /></label>}
            {operation === 'cost-get-cost-and-usage' && <><label>{vocab('Time period JSON')}<TextArea vocabularyMode="factual" value={String(coreInput.timePeriod ?? '{"Start":"2026-01-01","End":"2026-02-01"}')} onChange={(event) => updatePlatformInput('timePeriod', event.target.value)} /></label><label>{vocab('Granularity')}<Select vocabularyMode="factual" value={String(coreInput.granularity ?? 'MONTHLY')} onChange={(event) => updatePlatformInput('granularity', event.target.value)}><option value="DAILY">{vocab('Daily')}</option><option value="MONTHLY">{vocab('Monthly')}</option></Select></label><label>{vocab('Metrics')}<Input vocabularyMode="factual" value={String(coreInput.metrics ?? 'UnblendedCost')} onChange={(event) => updatePlatformInput('metrics', event.target.value)} /></label></>}
             {operation === 'cost-create-budget' && <><label>{vocab('Account id')}<Input vocabularyMode="factual" value={String(coreInput.accountId ?? '')} onChange={(event) => updatePlatformInput('accountId', event.target.value)} /></label><label>{vocab('Budget JSON')}<TextArea vocabularyMode="factual" value={String(coreInput.budget ?? '{"BudgetLimit":{"Amount":"100","Unit":"USD"},"BudgetName":"monthly-budget","BudgetType":"COST","TimeUnit":"MONTHLY"}')} onChange={(event) => updatePlatformInput('budget', event.target.value)} /></label></>}
          </>}
          {operation === 'resource-search' && <label>{vocab('Resource query')}<Input vocabularyMode="factual" value={query} onChange={(event) => { setQuery(event.target.value); persistIntent(mode, { resourceQuery: event.target.value }) }} placeholder={vocab('Use Resource Explorer query syntax')} /></label>}
          {operation === 'resource-list-views' && <label>{vocab('View ARN (optional)')}<Input vocabularyMode="factual" value={viewArn} onChange={(event) => setViewArn(event.target.value)} placeholder={vocab('Use the default view when empty')} /></label>}
          {['cloud-list-resources', 'cloud-get-resource', 'cloud-create-resource', 'cloud-update-resource', 'cloud-delete-resource'].includes(operation) && <label>{vocab('Resource type')}<Input vocabularyMode="factual" value={typeName} onChange={(event) => { setTypeName(event.target.value); persistIntent(mode, { cloudControlTypeName: event.target.value }) }} placeholder="AWS::Service::ResourceType" /></label>}
          {['cloud-get-resource', 'cloud-update-resource', 'cloud-delete-resource'].includes(operation) && <label>{vocab('Resource identifier')}<Input vocabularyMode="factual" value={identifier} onChange={(event) => setIdentifier(event.target.value)} placeholder={vocab('Choose an identifier from the list result')} /></label>}
          {operation === 'cloud-create-resource' && <label>{vocab('Desired state JSON')}<TextArea vocabularyMode="factual" value={desiredState} onChange={(event) => setDesiredState(event.target.value)} spellCheck={false} aria-describedby={`${id}-json-note`} /><small id={`${id}-json-note`}>{vocab('Object JSON is validated by the core before an AWS request is started.')}</small></label>}
          {operation === 'cloud-update-resource' && <label>{vocab('Patch document JSON')}<TextArea vocabularyMode="factual" value={patchDocument} onChange={(event) => setPatchDocument(event.target.value)} spellCheck={false} /></label>}
          {operation === 'cloud-request-status' && <label>{vocab('Request token')}<Input vocabularyMode="factual" value={requestToken} onChange={(event) => setRequestToken(event.target.value)} /></label>}
          <label>{vocab('Maximum results')}<Input vocabularyMode="factual" type="number" min={1} max={100} value={maxResults} onChange={(event) => setMaxResults(Math.max(1, Math.min(100, Number(event.target.value) || 1)))} /></label>
          <div className="aws-resource-node__actions"><Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => void makePreview()} disabled={busy || !runtime?.available || !binding}>{vocab('Preview generated operation')}</Button><Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => void execute()} disabled={busy || !preview}>{vocab('Run operation')}</Button></div>
        </section>
        ...
        {progress && <div className="aws-resource-node__progress" role="status">{progress.phase}: {progress.message}{progress.phase === 'started' && api.awsResource && <Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => void api.awsResource!.cancel(progress.operationId)}>{vocab('Cancel')}</Button>}</div>}
        ...
        {result && <section className="aws-resource-node__results" aria-label={vocab('AWS operation results')}>
          <div className="aws-resource-node__results-search"><label htmlFor={`${id}-result-search`}>{vocab('Search results')}</label><Input vocabularyMode="factual" id={`${id}-result-search`} ref={resultSearchRef} value={resultSearch.value} onChange={(event) => resultSearch.setValue(event.target.value)} placeholder={vocab('Filter returned rows')} /><AnchoredRegexBuilder search={resultSearch} fieldRef={resultSearchRef} label={vocab('Open regex builder for AWS result search')} /></div>
          ...
          {result.nextToken && <Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => setNextToken(result.nextToken ?? '')}>{vocab('Use next page token')}</Button>}
          {result.requestToken && <Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => setRequestToken(result.requestToken ?? '')}>{vocab('Use request token')}</Button>}
        </section>}
        </>}

...
export default function VirtualMachineNode({ id, data, selected }: NodeProps<CanvasNode>) {
  ...
  useEffect(() => {
    ...
    void Promise.all([api.virtualMachine.status(id), api.virtualMachine.tools()]).then(([next, available]) => {
      ...
      setStatus(next)
      setTools(available)
    }).catch((cause) => active && setError(cause instanceof Error ? cause.message : vocab('Unable to read VM status.')))
  ...
  const run = async (operation: () => Promise<VirtualMachineStatus>): Promise<void> => {
    setBusy(true)
    setError('')
    try { setStatus(await operation()) } catch (cause) { setError(cause instanceof Error ? cause.message : vocab('The VM operation failed.')) }
  ...
  const stop = async (): Promise<void> => {
    setStopping(true)
    setError('')
    try { setStatus(await api.virtualMachine.stop(id)) } catch (cause) { setError(cause instanceof Error ? cause.message : vocab('The VM stop operation failed.')) }
    finally { setStopping(false) }
  }
  const phase = status?.phase ?? 'unconfigured'
  const running = phase === 'running' || phase === 'starting'
  const modeLabel = config.mode === 'persistent-install' ? vocab('Persistent install') : vocab('Disposable live')
  ...
  }, [tools, local.isoPath, local.diskPath, config.mode, vocab])
  ...
  return (
    <div className={`term-node virtual-machine-node${selected ? ' selected' : ''}`} style={{ borderTopColor: data.color }}>
      ...
      <div className={`term-node__header ${fill.className}${fill.filled ? ' term-node__header--filled' : ''}`} style={fill.style}>
        ...
        <EditableNodeTitle value={data.title} onChange={(next) => updateNodeData(id, { title: next })} emptyLabel={ts('virtualMachine.title', 'Linux ISO VM')} title={vocab('Rename')} ariaLabel={ts('virtualMachine.title', 'Linux ISO VM')} rejectEmpty={false} />
        <span className="term-node__spacer" />
        <IconButton size="compact" className="term-node__close" icon="close" vocabularyMode="factual" title={vocab('Close')} aria-label={vocab('Close virtual machine')} onClick={() => deleteElements({ nodes: [{ id }] })} />
      </div>
      <div className="virtual-machine-node__body nodrag nowheel">
        ...
        <label className="virtual-machine-node__field"><span>{ts('virtualMachine.iso', 'Linux ISO')}</span><div className="virtual-machine-node__path"><Input value={local.isoPath ?? ''} onChange={(event) => patchLocal({ isoPath: event.target.value || undefined })} placeholder={vocab('Choose an ISO file')} aria-label={ts('virtualMachine.iso', 'Linux ISO')} /><Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => void choose('isoPath')}>{ts('virtualMachine.browse', 'Browse')}</Button></div></label>
        <label className="virtual-machine-node__field"><span>{ts('virtualMachine.expectedHash', 'Expected ISO SHA-256 (optional)')}</span><Input value={config.isoSha256 ?? ''} onChange={(event) => patchConfig({ isoSha256: event.target.value.trim().toLowerCase() || undefined })} placeholder={vocab('64 hexadecimal characters')} aria-label={ts('virtualMachine.expectedHash', 'Expected ISO SHA-256 (optional)')} /></label>
        ...
        {/* The picker callback deliberately returns nothing for cancel and the operation promise
            for selection. The dialog type permits null, while the JSX handler only needs effects. */}
        {/* @ts-expect-error the callback's value is intentionally ignored by the click handler. */}
        {config.mode === 'persistent-install' && <label className="virtual-machine-node__field"><span>{ts('virtualMachine.disk', 'Persistent disk')}</span><div className="virtual-machine-node__path"><Input value={local.diskPath ?? ''} onChange={(event) => patchLocal({ diskPath: event.target.value || undefined })} placeholder={vocab('Choose a qcow2 or raw disk')} aria-label={ts('virtualMachine.disk', 'Persistent disk')} /><Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => void choose('diskPath')}>{ts('virtualMachine.browse', 'Browse')}</Button><Button variant="outlined" size="small" vocabularyMode="factual" onClick={() => void api.dialog.selectFolder().then((folder) => folder && run(async () => { const next = await api.virtualMachine.createDisk(id, folder); updateNodeData(id, { virtualMachineConfig: { ...config, mode: 'persistent-install' }, virtualMachineLocalPaths: { ...local, diskPath: next.diskPath ?? undefined } }); return next }))}>{ts('virtualMachine.createDisk', 'Create disk')}</Button></div></label>}
        ...
        <div className="virtual-machine-node__actions"><Button variant="filled" size="small" vocabularyMode="factual" disabled={busy || stopping || running} onClick={() => void configure()}>{ts('virtualMachine.save', 'Save configuration')}</Button><Button variant="outlined" size="small" vocabularyMode="factual" disabled={busy || stopping || !status?.configured || running} onClick={() => void run(() => api.virtualMachine.start(id))}>{ts('virtualMachine.start', 'Start')}</Button><Button variant="outlined" size="small" vocabularyMode="factual" disabled={stopping || !running} onClick={() => void stop()}>{ts('virtualMachine.stop', 'Stop')}</Button></div>
        {status?.displayUrl && <div className="virtual-machine-node__display"><p><span>{vocab('Loopback display:')}</span> <code>{status.displayUrl}</code></p><Button variant="outlined" size="small" vocabularyMode="factual" disabled={busy || stopping || !running} onClick={() => void api.virtualMachine.openDisplay(id).then((result) => { if (!result.ok) setError(result.error) })}>{ts('virtualMachine.openDisplay', 'Open display')}</Button></div>}
        {running && <div className="virtual-machine-node__snapshot"><Input value={snapshotName} onChange={(event) => setSnapshotName(event.target.value)} aria-label={ts('virtualMachine.snapshot.newName', 'New snapshot name')} placeholder={ts('virtualMachine.snapshot.newName', 'New snapshot name')} /><Button variant="outlined" size="small" vocabularyMode="factual" disabled={busy || stopping || !SAFE_SNAPSHOT.test(snapshotName)} onClick={() => void run(() => api.virtualMachine.snapshot(id, snapshotName))}>{ts('virtualMachine.snapshot', 'Snapshot')}</Button><div className="virtual-machine-node__snapshot-search"><Input ref={snapshotSearchRef} value={snapshotSearch.value} onChange={(event) => snapshotSearch.setValue(event.target.value)} aria-label={ts('virtualMachine.snapshot.filter', 'Filter saved snapshots')} placeholder={snapshotSearch.mode === 'regex' ? ts('virtualMachine.snapshot.filterRegex', 'Filter snapshots with regex') : ts('virtualMachine.snapshot.filter', 'Filter saved snapshots')} /><AnchoredRegexBuilder search={snapshotSearch} fieldRef={snapshotSearchRef} label={ts('virtualMachine.snapshot.regex', 'Regex for saved snapshots')} /></div>{snapshotSearch.error && <p className="virtual-machine-node__hint" role="alert">{snapshotSearch.error}</p>}<Select value={selectedSnapshot} onChange={(event) => setRestoreSnapshotName(event.target.value)} aria-label={ts('virtualMachine.snapshot.restoreChoice', 'Saved snapshot to restore')} disabled={visibleSnapshots.length === 0}><option value="" disabled>{status?.snapshotNames.length ? ts('virtualMachine.snapshot.noMatch', 'No snapshots match the filter') : ts('virtualMachine.snapshot.none', 'Create a snapshot before restoring')}</option>{visibleSnapshots.map((name) => <option key={name} value={name}>{name}</option>)}</Select><Button variant="outlined" size="small" vocabularyMode="factual" disabled={busy || stopping || !selectedSnapshot} onClick={() => void run(() => api.virtualMachine.restore(id, selectedSnapshot))}>{ts('virtualMachine.restore', 'Restore')}</Button></div>}
        <span className="virtual-machine-node__mode" aria-label={vocab('VM mode')}>{modeLabel}</span>
      </div>
    </div>
  )
}

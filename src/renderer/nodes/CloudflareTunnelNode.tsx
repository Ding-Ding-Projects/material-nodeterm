    ...
    <>
      <div className={`service-node cloudflare-tunnel-node${selected ? ' selected' : ''}`} style={border.style} role="group" aria-label="Cloudflare Tunnel manager">
        ...
        <div className="service-node__body cloudflare-tunnel-node__body">
          ...
          <label className="service-node__field"><span className="service-node__field-label">Cloudflare API token</span><div className="service-node__field-row"><input className="service-node__input nodrag" type="password" value={token} onChange={(event) => setToken(event.target.value)} placeholder={tokenConfigured ? 'Token saved locally' : 'Paste once, then save locally'} autoComplete="off" /><button type="button" className="service-node__local-btn nodrag" disabled={busy || !token.trim()} onClick={() => void loadAccounts()}>Save and discover</button></div></label>
          ...
          <div className="cloudflare-tunnel-node__actions"><button type="button" className="mc-button nodrag" disabled={busy || !tokenConfigured} onClick={() => void loadAccounts()}>Refresh accounts</button><button type="button" className="mc-button nodrag" disabled={busy} onClick={() => void runPreflight()}>Run preflight</button><button type="button" className="mc-button mc-button--primary nodrag" disabled={busy || !checks.length || checks.some((check) => check.state === 'fail')} onClick={() => void apply()}>Create tunnel</button><button type="button" className="mc-button nodrag" disabled={busy || !status || status.phase === 'idle'} onClick={() => void rollback()}>Rollback</button></div>

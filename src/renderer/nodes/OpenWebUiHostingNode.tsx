        ...
        <div className="open-webui-node__body nodrag">
          ...
          <section aria-label={copy('Provider settings')} className="open-webui-node__section">
            ...
            <div className="open-webui-node__pills" role="radiogroup" aria-label={copy('Open WebUI provider', ['Open WebUI'])}>
              <Chip vocabularyMode="factual" selected={intent.provider === 'ollama'} role="radio" aria-checked={intent.provider === 'ollama'} className={intent.provider === 'ollama' ? 'selected' : ''} onClick={() => persistIntent({ ...intent, provider: 'ollama', reuseOllama: true })}>{copy('Local Ollama', ['Ollama'])}</Chip>
              <Chip vocabularyMode="factual" selected={intent.provider === 'openai-compatible'} role="radio" aria-checked={intent.provider === 'openai-compatible'} className={intent.provider === 'openai-compatible' ? 'selected' : ''} onClick={() => persistIntent({ ...intent, provider: 'openai-compatible', reuseOllama: false })}>{copy('OpenAI-compatible', ['OpenAI'])}</Chip>
            </div>

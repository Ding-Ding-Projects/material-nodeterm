...
export function buildRelayApi(connectionId: string, transport?: FrameTransport): RelayApiHandle {
  ...
  const api: NodeTerminalApi = {
    ...
    ollama: stub.ollama,
    // Cloudflare credentials and account bindings are machine-local control-plane state. A relay
    // tab must never fall through to the viewer's local credential or API endpoint.
    cloudflare: stub.cloudflare,

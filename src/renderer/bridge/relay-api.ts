...
export function buildRelayApi(connectionId: string, transport?: FrameTransport): RelayApiHandle {
  ...
  const api: NodeTerminalApi = {
    ...
    // Home Assistant is host-local configuration, registry state and a credential-backed write
    // path. Relay peers must not reach the joined host's vault or service calls.

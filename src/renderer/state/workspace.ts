...
export interface NodeData {
  ...
  /** Portable Nextcloud AIO deployment intent; host bindings and runtime facts remain local. */
...
export function createServiceNode(
  ...
  return {
    ...
    data: {
      ...
      group: null,
      serviceLabel: '',
      ...(kind === 'nextcloudaio' ? { nextcloudAioProfile: { ...NEXTCLOUD_AIO_DEFAULT_PROFILE } } : {})
    }
  }
}
...
const NODE_START_SIZE: Record<NodeKind, { width: number; height: number }> = {
  ...
  nextcloudaio: SERVICE_CONSOLE_SIZE,
  nsis: NSIS_SIZE
}
  ...
  const mapped = states.map((raw) => {
    ...
    return {
      ...
      data: {
        ...
        nextcloudAioProfile: n.nextcloudAioProfile,
        nsisSpec: n.nsisSpec,
  ...
  return nodes
    ...
    .map((n) => {
      ...
      return {
        ...
        nextcloudAioProfile: n.data.nextcloudAioProfile,

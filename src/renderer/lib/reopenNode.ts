...
export interface RecreateContext {
  ...
  /**
   * The TARGET project's own branded launch plan for `agentId` — never the caller's active
   * project's. A node restored into project B must start under B's permission-mode override,
   * never whichever project the person restoring it happens to be looking at (see
...
const COSMETIC_KEYS = [
  'title', 'titleAuto', 'color', 'group', 'tags', 'collapsed', 'expandedHeight', 'shell', 'portalRecoveryProgress'
] as const
...
function buildBase(snapshot: ReopenNodeSnapshot, ctx: RecreateContext): CanvasNode | null {
  ...
  switch (snapshot.type) {
    ...
    case 'portal':
      return createPortalRecoveryNode(0, undefined, d.portalRecoveryProgress)
    default:
      return null
  }
}
...
export function recreateNodeFromSnapshot(

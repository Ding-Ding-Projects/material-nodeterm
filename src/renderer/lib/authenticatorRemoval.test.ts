...
describe('authenticator seed removal policy', () => {
  ...
  it('preserves the ordinary one-confirm contract for a known-off record', () => {
    const perform = vi.fn()
    let request:
  ...
  it('changes identity when any disclosed seed metadata changes', () => {
    const variants: AuthenticatorEntry[] = [
      ...
      { ...ENTRY, createdAt: 0 },
      { ...ENTRY, updatedAt: 3 },
      { ...ENTRY, revision: 'sealed-generation-2' },

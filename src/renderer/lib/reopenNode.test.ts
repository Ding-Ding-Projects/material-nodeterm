...
// Same test-only cast the rest of this codebase uses to build a plan without the module-private
// brand symbol (see workspace.agent-prompt.test.ts) — production call sites mint one for real
...
vi.mock('@renderer/state/permissionMode', () => ({
  claudeCliCapsNow: () => ({ sessionIdFlag: false }),
  permissionModeFromLaunchPlan: (plan: ActiveAgentLaunchPlan | undefined, agentId: AgentId) =>
    plan?.agentId === agentId ? plan.mode : undefined
}))
...
describe('recreateNodeFromSnapshot', () => {
  ...
  it('recreates a terminal agent node, applying the TARGET project plan through permissionModeFor', () => {
    let seenAgentId: AgentId | undefined
    const ctx = {
      ...baseCtx(),
      permissionModeFor: (agentId: AgentId) => {
        seenAgentId = agentId
        return launchPlan(agentId, 'bypassPermissions')
      }
    }
    const node = recreateNodeFromSnapshot(
      snap({
        type: 'terminal',
        data: { title: 'agent', color: '#fff', group: null, agentId: 'claude' }
      }),
      ctx
    )
    expect(seenAgentId).toBe('claude')
    expect(node!.type).toBe('terminal')
    expect(node!.data.agentId).toBe('claude')
    expect(node!.data.initialCommand).toContain('--permission-mode bypassPermissions')
  })

  it('returns null for an unrecognized kind rather than a blank guess', () => {
    const node = recreateNodeFromSnapshot(snap({ type: 'authenticator' }), baseCtx())
    expect(node).toBeNull()
  })
})

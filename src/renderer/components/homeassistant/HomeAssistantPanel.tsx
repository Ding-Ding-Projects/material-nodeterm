...
export function HomeAssistantPanel({ nodeId }: { nodeId: string }): React.JSX.Element {
  ...
  return (
    <div className="service-node__body homeassistant-panel">
      ...
      <button type="button" className="mc-button nodrag" onClick={() => { setSelectedId(null); setSnapshot(null); setLabel('Home Assistant'); setUrl('https://homeassistant.local:8123'); setEnabled(true); setToken(''); setError(null) }}>{text('homeAssistant.addInstance', 'Add instance')}</button>
      <ul className="homeassistant-panel__instances" aria-label={text('homeAssistant.instances', 'Home Assistant instances')}>
        ...
        {instances.map((instance) => (
          <li key={instance.id}><button id={instance.id} type="button" className={`homeassistant-panel__instance nodrag${selectedId === instance.id ? ' is-selected' : ''}`} onClick={() => void choose(instance.id)} aria-pressed={selectedId === instance.id}>
      ...
      {!selectedId && (
        <form className="homeassistant-panel__form" onSubmit={(event) => { event.preventDefault(); void create() }}>
          ...
          <button type="submit" className="mc-button mc-button--primary nodrag" disabled={busy} title={busy ? text('homeAssistant.waiting', 'Waiting for the Home Assistant instance.') : undefined}>{text('homeAssistant.connect', 'Connect instance')}</button>
      ...
      {selectedId && snapshot && (
        <>
          <div className="mc-row"><button type="button" className="mc-button mc-button--primary nodrag" disabled={busy} onClick={() => void refresh()}>{text('homeAssistant.refresh', 'Refresh registries')}</button><button type="button" className="mc-button nodrag" disabled={busy} onClick={() => void disconnectSelected()}>{text('homeAssistant.disconnect', 'Disconnect')}</button><button type="button" className="mc-button nodrag" disabled={busy} onClick={() => void updateSelected()}>{text('homeAssistant.saveInstance', 'Save instance')}</button><button type="button" className="mc-button mc-button--danger nodrag" disabled={busy} onClick={() => setConfirmAction('remove')}>{text('homeAssistant.removeInstance', 'Remove instance')}</button></div>
          <div className="homeassistant-panel__form">
            ...
            <label className="service-node__field"><span className="service-node__field-label">Enabled</span><input className="nodrag" type="checkbox" checked={enabled} onChange={(event) => setEnabled(event.target.checked)} aria-label="Enable Home Assistant instance" /></label>
            ...
            <div className="mc-row"><button type="button" className="mc-button nodrag" disabled={busy || !token.trim()} onClick={() => void replaceToken()}>{text('homeAssistant.replaceToken', 'Replace token')}</button><button type="button" className="mc-button nodrag" disabled={busy || !tokenStatuses[selectedId]} onClick={() => setConfirmAction('clear-token')}>{text('homeAssistant.clearToken', 'Clear token')}</button></div>
          </div>
          ...
          <ul className="homeassistant-panel__entities" aria-label="Home Assistant entities">
            ...
            {entities.slice(0, 100).map((item) => <li key={item.entity_id}><button type="button" className="homeassistant-panel__entity nodrag" onClick={() => void bindEntity(item.entity_id)}><span>{item.entity_id}</span><strong>{item.state}</strong></button></li>)}
          </ul>
          ...
          <ul className="homeassistant-panel__entities" aria-label="Home Assistant services">{services.length === 0 ? <li><p className="service-node__note">No services match this search.</p></li> : services.slice(0, 100).map((item) => <li key={`${item.domain}.${item.service}`}><button type="button" className="homeassistant-panel__entity nodrag" onClick={() => setError(`${item.domain}.${item.service}: ${item.description || item.name}`)}><span>{item.domain}.{item.service}</span><small>{item.name || item.description}</small></button></li>)}</ul>

    ...
    <div className="docker-manager nodrag" aria-label={copy('title', 'Docker host manager')}>
      <div className="docker-manager__context">
        ...
        <div className="docker-manager__pills" role="listbox" aria-label="Available Docker contexts">
          {visibleContexts.map((item) => <button key={item.name} className={item.name === context ? 'selected' : ''} role="option" aria-selected={item.name === context} disabled={!item.available || busy} title={item.available ? item.endpointLabel : item.reason} onClick={() => void refresh(item.name)}>{item.name}<small>{item.kind}</small></button>)}
        </div>
      </div>

      <div className="docker-manager__tabs" role="tablist" aria-label={copy('resources', 'Docker resources')}>
        {TABS.map((item) => <button key={item.id} role="tab" aria-selected={tab === item.id} onClick={() => { setTab(item.id); search.reset() }}>{copy(item.key, item.label)}</button>)}
      </div>
      ...
      {tab === 'containers' && snapshot ? <>
        <section className="docker-manager__create" aria-label="Create a bounded container">
          ...
          <div className="docker-manager__pills">{DOCKER_GUIDED_IMAGES.map((item) => <button key={item.ref} className={selectedImage === item.ref ? 'selected' : ''} onClick={() => setSelectedImage(item.ref)}>{item.label}</button>)}</div>
          <div className="docker-manager__pills"><button className={selectedNetwork === 'none' ? 'selected' : ''} onClick={() => setSelectedNetwork('none')}>No network</button>{snapshot.networks.rows.map((row) => <button key={row.id} className={selectedNetwork === row.name ? 'selected' : ''} onClick={() => setSelectedNetwork(row.name)}>{row.name}</button>)}</div>
          <label><input type="checkbox" checked={readOnly} onChange={(event) => setReadOnly(event.target.checked)} /> Read-only root filesystem</label>
      ...
      {tab === 'images' ? <><section className="docker-manager__create"><h5>{copy('pullCatalog', 'Pull from guided catalog')}</h5><div className="docker-manager__pills">{DOCKER_GUIDED_IMAGES.map((item) => <button key={item.ref} onClick={() => void run({ type: 'image-pull', context, image: item.ref })}>{item.label}</button>)}</div></section><div className="docker-manager__rows">{(rows as DockerHostSnapshot['images']['rows']).map((row) => <article key={`${row.id}-${row.repository}-${row.tag}`}><header><strong>{row.repository}:{row.tag}</strong><span>{row.size}</span></header><p>{row.createdSince}</p><Button onClick={() => destructive('Remove image', 'The selected local image reference will be removed. Containers using it can block the action.', `${row.repository}:${row.tag}`, { type: 'image-remove', context, imageId: row.id })}>{copy('remove', 'Remove')}</Button></article>)}</div></> : null}

    ...
    <div className="drawer-overlay md3-aws" onClick={onClose}>
      <aside className="drawer aws-cli" role="dialog" aria-label="AWS CLI manager" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="aws-cli__tabs" role="tablist" aria-label="AWS CLI manager sections">
          <Button variant={tab === 'status' ? 'tonal' : 'text'} size="small" role="tab" aria-selected={tab === 'status'} onClick={() => setTab('status')}>Status and install</Button>
          <Button variant={tab === 'models' ? 'tonal' : 'text'} size="small" role="tab" aria-selected={tab === 'models'} onClick={() => { setTab('models'); if (!inventory) void refreshModels() }}>Foundation models</Button>
        </div>
        {tab === 'status' ? (
          <section className="aws-cli__section" role="tabpanel">
            ...
            <div className="aws-cli__actions">
              <Button variant="filled" disabled={busy || status?.state === 'ready'} onClick={() => void install(false)}>{busy ? 'Preparing…' : 'Install or repair'}</Button>
              <Button variant="outlined" disabled={!busy} onClick={() => void aws.cancel()}>Cancel</Button>
              <Button variant="outlined" disabled={busy} onClick={() => void install(true)}>Repair from verified source</Button>
              <Button variant="outlined" disabled={busy} onClick={() => void refreshStatus()}>Check again</Button>
            </div>
          ...
          <section className="aws-cli__section" role="tabpanel">
            <div className="aws-cli__search-row">
              <SearchField ref={searchRef} value={search.value} onChange={(event) => search.setValue(event.target.value)} placeholder="Search every model" aria-label="Search every AWS foundation model" trailingSlot={<AnchoredRegexBuilder search={search} fieldRef={searchRef} label="Regex for AWS model search" />} />
              <Button variant="outlined" onClick={() => void refreshModels()}>Refresh</Button>
            </div>

...
function Field({ spec, value, onChange }: { spec: AwsFieldSpec; value: unknown; onChange: (value: unknown) => void }): React.JSX.Element {
  if (spec.type === 'boolean') return <label className="aws-manager__check"><input type="checkbox" checked={value === true} onChange={(event) => onChange(event.target.checked)} />{spec.label}<small>{spec.description}</small></label>
...
export function AwsManagerPanel({ onClose }: AwsManagerPanelProps): React.JSX.Element {
  ...
  if (!api) return <section className="aws-manager"><header><h2>AWS managers</h2>{onClose && <button onClick={onClose}>Close</button>}</header><p className="aws-manager__notice">AWS managers are unavailable for this project session. Choose a local AWS session to configure them.</p></section>
  return <section className="aws-manager" aria-label="AWS service managers">
    <header className="aws-manager__header"><div><h2>AWS service managers</h2><p>Typed ECR, ECS, EKS, RDS, database, VPC, Route 53, and cost operations.</p></div>{onClose && <button onClick={onClose}>Close</button>}</header>
    <div className="aws-manager__toolbar">
      ...
      <button onClick={() => void loadInventory()}>Refresh inventory</button>{inventory?.nextToken && <button onClick={() => void loadNextPage()}>Load next page</button>}
    </div>
    <div className="aws-manager__search"><input ref={searchInputRef} value={search.value} placeholder="Search inventory (plain text)" onChange={(event) => search.setValue(event.target.value)} aria-describedby="aws-search-note" /><AnchoredRegexBuilder search={search} fieldRef={searchInputRef} label="Regex — AWS inventory" /><small id="aws-search-note">Plain text is the default. Regex is an explicit opt-in and stays attached to this list.</small></div>
    <div className="aws-manager__body">
      <div className="aws-manager__operations"><h3>Operation</h3><select value={operationId} onChange={(event) => { setOperationId(event.target.value); setPreview(null) }}>{operations.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.label} · {candidate.risk}</option>)}</select>{operation && <><p>{operation.description}</p>{operation.fields.map((spec) => <Field key={spec.key} spec={spec} value={values[spec.key]} onChange={(value) => setField(spec.key, value)} />)}<div className="aws-manager__actions"><button onClick={() => void makePreview()} disabled={!input}>Preview operation</button><button onClick={() => void makeBulkPreview()} disabled={selected.length === 0 || !input}>Preview bulk ({selected.length})</button></div></>}</div>
      <div className="aws-manager__inventory"><h3>Inventory {inventory && <small>({visibleItems.length})</small>}</h3>{inventory?.permission.state !== 'granted' && inventory && <p className="aws-manager__warning">{inventory.permission.state}: {inventory.permission.detail}</p>}{visibleItems.length === 0 ? <p className="aws-manager__empty">No resources are loaded for this service, or the search has no matches.</p> : <ul>{visibleItems.map((resource) => <li key={resource.id} className={resource.partial ? 'partial' : ''}><label><input type="checkbox" checked={selected.includes(resource.id)} onChange={(event) => setSelected((current) => event.target.checked ? [...current, resource.id] : current.filter((id) => id !== resource.id))} /><span><strong>{resource.name}</strong><small>{resource.status} · {resource.region}{resource.partial ? ' · partial permissions' : ''}</small></span></label></li>)}</ul>}</div>
    </div>
    {message && <p className="aws-manager__status" role="status">{message}{runningOperationId && <button onClick={() => void api.cancel(runningOperationId)}>Cancel operation</button>}</p>}
    ...
    {preview && <div className="aws-manager__preview"><h3>Review operation</h3><p>{preview.operation.label} · {preview.risk}</p><p>Region: {preview.context.region}{preview.context.profile ? ` · Profile: ${preview.context.profile}` : ''}</p><code>{preview.argv.join(' ')}</code><p>Credentials stay in the local AWS credential chain and are never sent to the renderer.</p><button onClick={() => preview.risk === 'destructive' ? setConfirming(true) : void run()}>{preview.risk === 'destructive' ? 'Continue to confirmation' : 'Run operation'}</button></div>}
    {bulkPreview && <div className="aws-manager__preview"><h3>Review bulk operation</h3><p>{bulkPreview.summary}</p><p>{bulkPreview.affectedCount} selected resource(s) will be processed. Partial results remain visible.</p><button onClick={() => { setBulkPreview(null); setMessage('Bulk execution is available after the destructive confirmation step.') }}>Keep preview</button><button onClick={() => { setBulkPreview(null); setMessage('Bulk operation is ready to run from the preview.') }}>Continue</button></div>}

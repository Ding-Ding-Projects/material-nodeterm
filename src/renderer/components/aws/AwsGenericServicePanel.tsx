...
import { Button, Checkbox, NumberField, Tablist } from '../../ui/md3'
...
function optionInput(option: AwsModelCatalog['globalOptions'][number], value: unknown, onChange: (value: unknown) => void, onPickFile?: () => Promise<string | undefined>): React.JSX.Element {
  if (option.kind === 'boolean') return <Checkbox checked={value === true} onChange={(e) => onChange(e.target.checked)} aria-label={option.label} />
  if (option.kind === 'number') return <NumberField min={option.min} max={option.max} step={option.step ?? 1} value={typeof value === 'number' ? value : ''} onChange={(e) => onChange(e.target.value === '' ? undefined : Number(e.target.value))} aria-label={option.label} />
  ...
  if (option.kind === 'file') return <Button type="button" variant="outlined" className="aws-choice-button" disabled={!onPickFile} onClick={() => void onPickFile?.()}>{typeof value === 'string' && value ? value : onPickFile ? 'Choose a file…' : 'File picker unavailable'}</Button>
...
function ScalarEditor({
  ...
  if (shape.kind === 'boolean') {
    return <Checkbox checked={value === true} onChange={(e) => onChange(e.target.checked)} aria-label={field.label} />
  }
  if (shape.kind === 'enum') {
    return (
  ...
  if (shape.kind === 'number') {
    return <NumberField value={typeof value === 'number' ? value : ''} min={shape.min} max={shape.max} step={shape.step ?? 1} onChange={(e) => onChange(e.target.value === '' ? undefined : Number(e.target.value))} aria-label={field.label} />
  }
  if (shape.kind === 'file') {
    return (
      <Button type="button" variant="outlined" className="aws-choice-button" onClick={() => void onPickFile?.(field)} disabled={!onPickFile} title={!onPickFile ? 'The host file picker is not available.' : undefined}>
        {typeof value === 'string' && value ? value : onPickFile ? 'Choose a file…' : 'File picker unavailable'}
      </Button>
    )
  }
  return <Input type={shape.kind === 'date' || shape.kind === 'timestamp' ? 'text' : 'text'} value={typeof value === 'string' ? value : ''} placeholder={'placeholder' in shape ? shape.placeholder : undefined} onChange={(e) => onChange(e.target.value || undefined)} aria-label={field.label} />
}
    ...
    return (
      <div className="aws-repeated-editor">
        {items.map((item, index) => {
          ...
          return <div className="aws-repeated-row" key={`${index}-${String(item)}`}><ShapeEditor shape={shape.item} field={child} value={item} onChange={(next) => { const copy = [...items]; copy[index] = next; onChange(copy) }} onPickFile={onPickFile} depth={depth + 1} /><Button type="button" variant="text" className="aws-link-button" onClick={() => onChange(items.filter((_, i) => i !== index))}>Remove</Button></div>
        })}
        <Button type="button" variant="outlined" className="aws-choice-button" disabled={shape.maxItems !== undefined && items.length >= shape.maxItems} onClick={() => onChange([...items, undefined])}>Add item</Button>
      </div>
    )
  }
  ...
  return (
    <div className="aws-repeated-editor">
      {entries.map(([key, item]) => {
        ...
        return <div className="aws-repeated-row" key={key}><Input value={key} aria-label="Map key" onChange={(e) => { const copy: JsonRecord = {}; for (const [oldKey, oldValue] of entries) copy[oldKey === key ? e.target.value : oldKey] = oldValue; onChange(copy) }} /><ShapeEditor shape={shape.value} field={child} value={item} onChange={(next) => onChange(updatePath(value, [key], next))} onPickFile={onPickFile} depth={depth + 1} /><Button type="button" variant="text" className="aws-link-button" onClick={() => { const copy = { ...(value as JsonRecord) }; delete copy[key]; onChange(copy) }}>Remove</Button></div>
      })}
      <Button type="button" variant="outlined" className="aws-choice-button" disabled={shape.maxEntries !== undefined && entries.length >= shape.maxEntries} onClick={() => { const copy = { ...(value as JsonRecord) }; let key = 'key'; let n = 2; while (key in copy) key = `key${n++}`; copy[key] = undefined; onChange(copy) }}>Add entry</Button>
    </div>
  )
}

function SearchField({ label, search, inputRef }: { label: string; search: ReturnType<typeof useRegexSearchField>; inputRef: React.RefObject<HTMLInputElement> }): React.JSX.Element {
...
function PreviewCard({ preview, reviewed, onReviewed }: { preview: AwsExecutionPreview | null; reviewed: boolean; onReviewed: (next: boolean) => void }): React.JSX.Element {
  ...
  return (
    <section className={`aws-risk-card aws-risk-card--${preview.risk}`} aria-label="Execution risk preview">
      ...
      <label className="aws-review-check"><Checkbox checked={reviewed} onChange={(e) => onReviewed(e.target.checked)} /> I reviewed the service, operation, target, generated argv, output, and risk.</label>
    </section>
  )
}
...
export function AwsGenericServicePanel({ catalog, onRefresh, onInvoke, onPickFile, onProgress }: AwsGenericServicePanelProps): React.JSX.Element {
  ...
  const selectService = (service: AwsServiceModel): void => { updateSettings({ serviceId: service.id, operationId: '', input: {}, paginatorId: undefined, waiterId: undefined }); operationSearch.setValue(''); setTab('command') }
  ...
  return (
    <section className="aws-generic-panel" aria-label="Generic AWS service manager">
      <header className="aws-panel-header"><div><p className="aws-eyebrow">AWS Universe · installed CLI models</p><h2>All-service AWS manager</h2><p className="aws-muted">Guided controls are generated from the installed service models, so newly installed services appear without hand-maintained forms.</p></div><Button type="button" variant="outlined" className="aws-choice-button" onClick={() => void refresh()} disabled={!onRefresh || refreshing}>{refreshing ? 'Refreshing…' : 'Refresh models'}</Button></header>
      {!catalog ? <div className="aws-empty-state"><h3>No AWS model catalog loaded</h3><p>Load the installed AWS CLI model index to enumerate services, operations, paginators, waiters, skeletons, inputs, and outputs. Nothing is guessed from a service name.</p><Button type="button" variant="outlined" className="aws-choice-button" onClick={() => void refresh()} disabled={!onRefresh || refreshing}>{onRefresh ? 'Load installed models' : 'Model loader unavailable'}</Button></div> : <>

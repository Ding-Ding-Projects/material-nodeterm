...
import { SettingsText, validateSettingsCallsiteId } from './SettingsText'
...
describe('FieldRow personal vocabulary boundary', () => {
  ...
  it('consumes stable callsite ids at runtime and rejects malformed ids', () => {
    useSchoolMode.setState({ enabled: false, hydrated: true })
    render(
      <FieldRow
        callsiteId="settings.support.empty"
        label="Label"
        descriptionSegments={[{ kind: 'copy', value: 'A typed message.' }]}
        control={<button type="button">Save</button>}
      />
    )
    expect(screen.getByText('A typed message.')).toBeTruthy()
    expect(validateSettingsCallsiteId('settings.support.empty')).toBe(
      'settings.support.empty'
    )
    expect(() => validateSettingsCallsiteId('settings.test')).toThrow(/Invalid settings callsite id/)
  })
})

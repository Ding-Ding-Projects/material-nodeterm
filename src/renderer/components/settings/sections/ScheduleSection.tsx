      ...
      <div className="min-w-0 text-[12px] leading-snug">
        {status?.error ? (
          <p className="text-[color:var(--warn)]">
            <SettingsText callsiteId="settings.schedule.source-error" segments={[
          ...
          <p className="text-muted">
            <SettingsText callsiteId="settings.schedule.source-success" segments={[{ kind: 'fact', value: label }, { kind: 'copy', value: ' last synced ' }, { kind: 'fact', value: lastSuccess ?? 'unknown' }, { kind: 'copy', value: '.' }]} />
          </p>
        ) : (
          <p className="text-muted"><SettingsText callsiteId="settings.schedule.source-unchecked" segments={[{ kind: 'fact', value: label }, { kind: 'copy', value: ' has not been checked yet.' }]} /></p>
        )}
      </div>
    ...
    <div className="space-y-4 rounded-md border border-border p-3">
      ...
      <div>
        <p className="mb-2 text-[12px] font-semibold uppercase tracking-wide text-muted-2">
          <SettingsText callsiteId="settings.schedule.value-source-status">{rule.source.kind === 'api' ? 'Local fallback (before the first fetch)' : 'Apply while active'}</SettingsText>
        </p>
  ...
  if (loadError) {
    return (
      ...
      >
        ...
        >
          <p>
            <SettingsText callsiteId="settings.schedule.load-error-kind" segments={[
          ...
          <p>
            <SettingsText callsiteId="settings.schedule.load-error-path" segments={[

...
function stateLabel(scope: ClaudeSkillScope): string {
  if (scope.state === 'available') return 'Available'
  if (scope.state === 'missing') return 'Missing'
  return 'Unavailable'
}

function skillStateLabel(state: 'available' | 'missing' | 'unavailable'): string {
...
export function ClaudeSkillsSection({ isActive }: { isActive: boolean }): React.JSX.Element {
  ...
  const refresh = (): void => {
    setLoading(true)
    setError(null)
    void window.nodeTerminal.claude.skills
      .list()
      .then(setCatalogue)
  ...
  const visibleScopes = useMemo(() => {
    ...
    return catalogue.scopes
      .map((scope) => {
        ...
        const skills = scope.skills.filter((skill) =>
          matchesEntry(search, {
            title: skill.name,
        ...
        <div className="space-y-4">
            ...
            <div className="relative min-w-[240px] flex-1">
              ...
              <div className="absolute right-1 top-1/2 -translate-y-1/2">
                <AnchoredRegexBuilder
                  search={search}
                  fieldRef={inputRef}
                  label="Regex — Claude skill search"
                />
              </div>
            </div>
            <Button onClick={refresh} disabled={loading}>
              {loading ? 'Refreshing…' : 'Refresh skill catalogue'}
            </Button>
          </div>
          ...
          {error ? (
            <div className="rounded-md border border-danger/40 bg-danger/10 p-3 text-sm text-danger" role="alert">
              <strong>Skill discovery unavailable.</strong> {error}
            </div>
          ) : null}
          {!catalogue && !loading && !error ? (
            <p className="text-sm text-muted">No Claude skill catalogue has been loaded yet.</p>
          ) : null}
          {catalogue && visibleScopes.length === 0 ? (
            <p className="text-sm text-muted">No Claude skill scopes or skills match this search.</p>
          ) : null}
            ...
            <div key={scope.id} className="rounded-md border border-border p-3" data-skill-scope={scope.id}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted">{scope.location}</p>
                </div>
                <span className="rounded-full border border-border px-2 py-1 text-xs" data-skill-scope-state={scope.state}>
                  {stateLabel(scope)}
                </span>
              </div>
              ...
              <div className="mt-3 space-y-2">
                {scope.skills.map((skill) => (
                  <FieldRow
                    ...
                    label={skill.name}
                    note={skill.reason}
                    control={<span className="text-xs text-muted">{skillStateLabel(skill.state)}</span>}
                  />
                ))}
                {scope.skills.length === 0 ? (
                  <p className="text-xs text-muted">No readable skill folders were found in this scope.</p>
                ) : null}
              </div>
            </div>
          ))}
          {catalogue ? (
            <p className="text-xs text-muted" role="status">
              Refreshed {new Date(catalogue.refreshedAt).toLocaleString()}. Skill file contents,
              credentials, session data, and private paths are never returned by this catalogue.
            </p>
          ) : null}
        </div>
      </SearchableRow>

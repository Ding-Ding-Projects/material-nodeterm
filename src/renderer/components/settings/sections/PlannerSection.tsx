      ...
      <SearchableRow {...ROWS.schedules}>
        <div className="space-y-5">
          ...
          <div className="rounded-2xl border border-outline/30 bg-surface-container p-4 space-y-3">
            ...
            {draft.recurrence.kind === 'weekly' ? <FieldRow label="Weekdays" control={<div className="flex flex-wrap gap-2">{(['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'] as const).map((label, day) => <label key={label} className="inline-flex items-center gap-1 text-sm"><Checkbox vocabularyMode="factual" checked={weeklyDays.includes(day as PlannerWeekday)} onChange={(event) => { const days = event.target.checked ? [...weeklyDays, day as PlannerWeekday] : weeklyDays.filter((value) => value !== day); setDraft({ ...draft, recurrence: { kind: 'weekly', days: Array.from(new Set(days)).sort((a, b) => a - b) as PlannerWeekday[] } }) }} /> <SettingsText>{label}</SettingsText></label>)}</div>} /> : null}

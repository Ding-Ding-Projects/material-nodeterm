  ...
  return (
    ...
    <SettingsSection id="school-mode" title={name} isActive={isActive} searchEntries={ENTRIES}>
      <SearchableRow {...ROWS.toggle}>
        <FieldRow
          callsiteId="settings.school.toggle"
          labelSegments={[{ kind: 'fact', value: name }, { kind: 'copy', value: enabled ? ' is on' : ' is off' }]}
          descriptionSegments={[
            { kind: 'copy', value: 'A self-imposed focus switch, not a security lock: while on, this app presents in plain English and behaves as if the Cantonese/bilingual, funny-level, and dim-sum-surprise capabilities were not installed. It affects every app that reads this machine\'s shared ' },
            { kind: 'fact', value: name },
            { kind: 'copy', value: ' record, not just this one. Turning it OFF needs the PIN below; turning it ON never does.' }
          ]}
          control={
            enabled && !hasCredential ? (
              ...
              <div className="flex flex-col items-end gap-2">
                ...
                <p className="max-w-[22rem] text-right text-[12px] text-muted">
                  <SettingsText callsiteId="settings.school.no-pin" segments={[{ kind: 'copy', value: 'No PIN was ever set on this machine, so none is needed to turn ' }, { kind: 'fact', value: name }, { kind: 'copy', value: ' off. Set one when you turn it back on if you want it to stay on.' }]} />
                </p>
              </div>
            ) : enabled ? (
        ...
        />
        {error ? <p className="text-[12px] leading-relaxed text-[color:var(--warn)]"><SettingsText callsiteId="settings.school.error" segments={[{ kind: 'fact', value: error }]} /></p> : null}
        <p className="text-[12px] leading-relaxed text-muted-2"><SettingsText callsiteId="settings.school.recovery" segments={[
      ...
      {hasCredential ? (
        <SearchableRow {...ROWS.pin}>
          ...
          />
          {pinChangeMsg ? <p className="text-[12px] leading-relaxed text-muted-2"><SettingsText callsiteId="settings.school.pin-change-message">{pinChangeMsg}</SettingsText></p> : null}
        </SearchableRow>
      ) : null}
    </SettingsSection>
  )

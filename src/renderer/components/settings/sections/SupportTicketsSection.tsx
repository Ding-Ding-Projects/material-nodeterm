...
const SEVERITY_LABEL: Record<SupportTicket['severity'], string> = {
  low: 'Low',
  medium: 'Medium',
  high: 'High',
  'critical-but-not-really': 'Critical (nobody will honour this)'
}
  ...
  return (
    <li className="toylock-ticket">
      <div className="toylock-ticket__head">
        ...
        <span className="toylock-ticket__status"><SettingsText callsiteId="settings.support.status-label">{STATUS_LABEL[ticket.status]}</SettingsText></span>
        <span className="toylock-ticket__severity"><SettingsText callsiteId="settings.support.severity-value">{SEVERITY_LABEL[ticket.severity] ?? ticket.severity}</SettingsText></span>
      </div>
      <div className="toylock-ticket__category"><SettingsText callsiteId="settings.support.category-value">{ticket.category}</SettingsText></div>
      {ticket.description && <p className="toylock-ticket__desc">{ticket.description}</p>}
      <p className="toylock-hint"><SettingsText callsiteId="settings.support.canned-response">{CANNED_RESPONSE[ticket.status]}</SettingsText></p>

      {ticket.status !== 'resolved' ? (
        <button className="toylock-btn toylock-btn--sm" onClick={() => advance(ticket.id)}>
          <SettingsText callsiteId="settings.support.advance">Check for updates</SettingsText>
        </button>
      ) : (
        <div className="toylock-resolution">
          <p className="toylock-recovery__how">
            <SettingsText callsiteId="settings.support.recovery-path">Delete nodeterm's local application-data folder and every toy lock resets:</SettingsText>
          </p>
          <div className="toylock-recovery__path-row">
            ...
            <button className="toylock-btn toylock-btn--sm" onClick={copyPath} disabled={!dir}>
              <SettingsText callsiteId="settings.support.copy-path">Copy path</SettingsText>
            </button>
            {canOpenFolder && (
              <button className="toylock-btn toylock-btn--sm toylock-btn--primary" onClick={openFolder} disabled={!dir}>
                <SettingsText callsiteId="settings.support.open-folder">Open folder</SettingsText>
              </button>
            )}
          </div>
          {!canOpenFolder && (
            <p className="toylock-hint">
              <SettingsText callsiteId="settings.support.browser-boundary">Opening a folder isn't available from a browser tab — delete this path on the server itself.</SettingsText>
            </p>
          )}
          <p className="toylock-hint">
            <SettingsText callsiteId="settings.support.no-delete">This app never deletes it for you — it just opens the folder and stands back.</SettingsText>
          </p>
        </div>
      )}
      <button className="toylock-btn--link" onClick={() => remove(ticket.id)}>
        <SettingsText callsiteId="settings.support.close-ticket">Close ticket</SettingsText>
      </button>
    </li>
  )
}
      ...
      <SearchableRow {...ROW}>
        <div className="space-y-4">
          <div className="toylock-support-disclosure">
            <SettingsText callsiteId="settings.support.disclosure">Nothing on this page is sent anywhere. There is no ticket outside this computer, no network request is made, no data is collected, and nobody is reading it.</SettingsText>
          </div>
          ...
          <div className="toylock-add-entry">
            <Select className="toylock-select" value={category} onChange={(e) => setCategory(e.target.value)}>
              {CATEGORIES.map((c) => (
                <option key={c} value={c}>
                  <SettingsText callsiteId="settings.support.category-option">{c}</SettingsText>
                </option>
              ))}
            </Select>
              <TextArea
            ...
            >
              <option value="low"><SettingsText callsiteId="settings.support.severity-option">Low</SettingsText></option>
              <option value="medium"><SettingsText callsiteId="settings.support.severity-option">Medium</SettingsText></option>
              <option value="high"><SettingsText callsiteId="settings.support.severity-option">High</SettingsText></option>
              <option value="critical-but-not-really"><SettingsText callsiteId="settings.support.severity-option">Critical (nobody will honour this)</SettingsText></option>
            </Select>
            <button className="toylock-btn toylock-btn--primary" onClick={submit}>
              <SettingsText callsiteId="settings.support.submit">Submit ticket</SettingsText>
            </button>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {tickets.length > 0 && (
              <button className="toylock-btn toylock-btn--sm" onClick={exportAll}>
                <SettingsText callsiteId="settings.support.export">Copy all as text</SettingsText>
              </button>
            )}
          </div>

          {filtered.length === 0 ? (
          <p className="text-[13px] text-muted"><SettingsText callsiteId="settings.support.empty">No tickets yet.</SettingsText></p>
          ) : (

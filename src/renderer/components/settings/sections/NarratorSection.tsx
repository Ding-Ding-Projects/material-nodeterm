...
function VoicePicker({
  ...
  return (
    <FieldRow
      callsiteId="settings.narrator.voice-picker"
      label={label}
      description={
        track === 'yue'
      ...
      control={
        <div className="flex flex-col items-end gap-1.5">
          <div className="flex items-center gap-2">
            ...
            >
              <option value=""><SettingsText callsiteId="settings.narrator.choose-automatically">Choose automatically</SettingsText></option>
              {voices.map((v) => (
            ...
            >
              <SettingsText callsiteId="settings.narrator.preview">Preview</SettingsText>
            </Button>
          </div>
          <p className="max-w-xs text-right text-[12px] leading-snug text-muted">
            {!ready ? <SettingsText callsiteId="settings.narrator.voices-loading">Looking for installed voices…</SettingsText> : status.noVoiceForTrack ? (
              <SettingsText callsiteId="settings.narrator.no-voice" segments={[{ kind: 'copy', value: 'No ' }, { kind: 'fact', value: track === 'yue' ? 'Cantonese' : 'English' }, { kind: 'copy', value: ' voice is installed on this computer.' }]} />
            ) : status.missingChosen ? (
              <SettingsText callsiteId="settings.narrator.missing-voice" segments={[{ kind: 'copy', value: "Your chosen voice isn't installed here — falling back to “" }, { kind: 'fact', value: status.voice?.name ?? 'unknown' }, { kind: 'copy', value: '”. Your choice is kept.' }]} />
            ) : status.networkOnly ? (
              <SettingsText callsiteId="settings.narrator.network-voice" segments={[{ kind: 'copy', value: 'Will speak with “' }, { kind: 'fact', value: status.voice?.name ?? 'unknown' }, { kind: 'copy', value: '” — needs a network connection, and will go quiet offline.' }]} />
            ) : (
              <SettingsText callsiteId="settings.narrator.active-voice" segments={[{ kind: 'copy', value: 'Will speak with “' }, { kind: 'fact', value: status.voice?.name ?? 'unknown' }, { kind: 'copy', value: '” (' }, { kind: 'fact', value: status.voice?.lang ?? 'unknown' }, { kind: 'copy', value: ').' }]} />
            )}
          </p>
        </div>
      }
    ...
    >
      {!available && (
        <p className="text-[13px] text-[color:var(--warn)]">
          <SettingsText callsiteId="settings.narrator.unavailable">Speech synthesis isn't available in this window, so the narrator can't speak here.</SettingsText>
        </p>
      )}
      <SearchableRow {...ROWS.enabled}>
        <FieldRow
          callsiteId="settings.narrator.enabled"

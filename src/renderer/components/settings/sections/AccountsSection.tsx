...
function AddingLabel({ where }: { where: string }): React.JSX.Element {
  return (
    <span className="inline-flex items-center gap-2">
      ...
      <><SettingsText callsiteId="settings.accounts.adding-on">Setting up on</SettingsText> {where}…</>
    </span>
  )
}
  ...
  return (
    <div className="flex items-start justify-between gap-3 bg-bg px-3 py-3">
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        {labelControl}
        {unavailable ? (
          <p className="text-[12px] text-muted"><SettingsText callsiteId="settings.accounts.unavailable">Not logged in or unavailable</SettingsText></p>
        ) : null}
        {details}
      </div>
        ...
        <div className="space-y-4">
          {versionWarning ? (
            <div className="flex items-start justify-between gap-3 rounded-md border border-[color:var(--danger)]/40 bg-[color:var(--danger)]/10 px-3 py-2 text-[13px] leading-relaxed text-[color:var(--danger)]">
              <span><SettingsText callsiteId="settings.accounts.cli-version-warning">Your installed Claude CLI is older than the version that scopes credentials per config dir. Accounts still isolate their config, but on macOS logins may collide in the shared keychain. Update the Claude CLI to keep them fully separate.</SettingsText></span>
              <button
    ...
    {codexRemoteTargets.map(([host, server]) => {
      ...
      return (
        ...
        {codexAccounts
            ...
            <AccountRow
              ...
              details={
                <label className="mt-2 flex max-w-lg items-center gap-2 text-[11px] text-muted">
                  <span className="shrink-0 font-medium uppercase tracking-wide"><SettingsText callsiteId="settings.accounts.working-directory">Working directory</SettingsText></span>
                  <Input
          ...
          <p className="text-[12px] leading-relaxed text-muted"><SettingsText callsiteId="settings.accounts.credentials-disclosure">Each login has its own credentials. Remote logins stay on their SSH machine. Codex nodes on the same machine and account reuse one shared app-server.</SettingsText></p>
        </div>
      </SearchableRow>

...
import { Tablist } from '@renderer/ui/md3'
...
export function DockerHostManagerPanel(): React.JSX.Element {
  ...
  return (
    <div className="mt-5 space-y-4 rounded-xl border border-border bg-surface p-4" aria-label="Docker host manager">
      ...
      <Tablist className="flex flex-wrap gap-2" ariaLabel="Docker inventory types">{(['containers', 'images', 'volumes', 'networks'] as const).map((value) => <Button key={value} aria-selected={tab === value} onClick={() => setTab(value)}>{value[0].toUpperCase() + value.slice(1)}</Button>)}</Tablist>

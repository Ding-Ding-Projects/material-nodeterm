  ...
  return (
    ...
    <SettingsSection id="kids-mode" title={name} isActive={isActive} searchEntries={ENTRIES}>
      <SearchableRow {...ROWS.toggle}>
            ...
            enabled && !hasCredential ? (
              ...
              <div className="flex flex-col items-end gap-2">
                ...
                <p className="max-w-[22rem] text-right text-[12px] text-muted">
                  <SettingsText callsiteId="settings.kids.no-pin" segments={[{ kind: 'copy', value: 'No grown-up PIN was ever set on this machine, so none is needed to turn ' }, { kind: 'fact', value: name }, { kind: 'copy', value: ' off. Set one when you turn it back on if you want it to stay on.' }]} />
                </p>
              </div>
            ) : enabled ? (
        ...
        />
        {error ? <p className="text-[12px] leading-relaxed text-[color:var(--warn)]"><SettingsText callsiteId="settings.kids.error" segments={[{ kind: 'fact', value: error }]} /></p> : null}
        <p className="text-[12px] leading-relaxed text-muted-2"><SettingsText callsiteId="settings.kids.recovery" segments={[
      ...
      <SearchableRow {...ROWS.limits}>
        ...
        <p className="md3-kids-disclosure md3-kids-disclosure--settings"><SettingsText callsiteId="settings.kids.disclosure">{KIDS_DISCLOSURE}</SettingsText></p>
        <p className="mt-3 text-[12px] leading-relaxed text-muted"><SettingsText callsiteId="settings.kids.refused-modes">While it is on, an agent cannot be started in these modes:</SettingsText></p>
        <ul className="mt-1 space-y-1 text-[12px] leading-relaxed text-muted">
          {Object.entries(KIDS_REFUSED_PERMISSION_MODES).map(([mode, why]) => (
            <li key={mode}>
              ...
              </strong>{' '}<SettingsText callsiteId="settings.kids.refused-mode-reason" segments={[{ kind: 'copy', value: ' — ' }, { kind: 'fact', value: why }, { kind: 'copy', value: '. It falls back to asking every time.' }]} />
            </li>
          ))}
        </ul>
      </SearchableRow>
      ...
      <SearchableRow {...ROWS.pin}>
        ...
        />
        {pinChangeMsg ? (
          <p className="text-[12px] leading-relaxed text-muted"><SettingsText callsiteId="settings.kids.pin-change-message">{pinChangeMsg}</SettingsText></p>
        ) : null}
      </SearchableRow>
    </SettingsSection>
  )

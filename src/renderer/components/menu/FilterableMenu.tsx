    ...
    <div className="menu-filter" onMouseDown={(e) => e.stopPropagation()}>
      <div className="menu-filter__row">
        <input
          ...
          spellCheck={false}
          placeholder={filter.search.mode === 'regex' ? `${placeholder} (regex)` : placeholder}
        ...
        <AnchoredRegexBuilder
          ...
          label={regexLabel ?? 'Regex — menu filter'}
          zIndex={zIndex}
        />
      </div>
      {filter.search.error && <div className="menu-filter__error">{filter.search.error}</div>}

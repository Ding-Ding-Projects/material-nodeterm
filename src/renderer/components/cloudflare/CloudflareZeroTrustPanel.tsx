...
function FieldEditor({ field, value, resources, onChange, localFile, onFile }: { field: CloudflareFieldModel; value: string | number | boolean | undefined; resources: readonly CloudflareResourceSummary[]; onChange: (value: string | number | boolean | undefined) => void; localFile?: string; onFile: (path: string) => void }): React.JSX.Element {
  if (field.kind === 'boolean') return <label className="cloudflare-zero-trust__boolean"><Checkbox vocabularyMode="factual" checked={value === true} onChange={(event) => onChange(event.target.checked)} /><span><strong>{field.label}</strong><small>{field.description}</small></span></label>
  if (field.kind === 'enum') return <label className="cloudflare-zero-trust__field"><span>{field.label}</span><Select vocabularyMode="factual" value={typeof value === 'string' ? value : ''} onChange={(event) => onChange(event.target.value)}><option value="">Choose {field.label.toLowerCase()}…</option>{field.choices?.map((choice) => <option key={choice.value} value={choice.value}>{choice.label}</option>)}</Select><small>{field.description}</small></label>
  if (field.kind === 'file') return <label className="cloudflare-zero-trust__field"><span>{field.label}</span><Input vocabularyMode="factual" type="file" accept=".js,.mjs,.ts,.txt" onChange={(event) => { const file = event.target.files?.[0] as (File & { path?: string }) | undefined; if (file?.path) onFile(file.path) }} /><small>{localFile ? `Selected: ${localFile.split(/[\\/]/u).pop() ?? localFile}` : field.description}</small></label>
  ...
  if (resource || (isResourceField && resources.length > 0)) return <label className="cloudflare-zero-trust__field"><span>{field.label}</span><Select vocabularyMode="factual" value={typeof value === 'string' ? value : ''} onChange={(event) => onChange(event.target.value)}><option value="">Choose {field.label.toLowerCase()}…</option>{resources.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</Select><small>{field.description}</small></label>

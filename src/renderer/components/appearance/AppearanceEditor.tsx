...
function PresetsTab({
  ...
  function onImportFile(e: React.ChangeEvent<HTMLInputElement>): void {
    ...
    reader.onload = () => {
      ...
      setImportSummary({
        ...
        skippedDuplicateNames: result.skippedDuplicateNames
      })
    }
  ...
  return (
    <div className="appearance-editor__fields">
      ...
      {importSummary && (
        <p className="appearance-editor__note">
          <SettingsText callsiteId="settings.appearance.presets-import-result" segments={[
            { kind: 'copy', value: 'Imported ' },
            { kind: 'fact', value: String(importSummary.imported) },
            { kind: 'copy', value: '. Skipped ' },
            { kind: 'fact', value: String(importSummary.skippedInvalid) },
            { kind: 'copy', value: ' invalid, ' },
            { kind: 'fact', value: String(importSummary.skippedDuplicateNames) },
            { kind: 'copy', value: ' duplicate name(s).' }
          ]} />
        </p>
      )}

...
export interface NodeCatalogDialogProps {
  ...
  onCreateChild?: (entry: NodeCatalogEntry, creationEventId: string) => void
...
export function NodeCatalogDialog({ open, onClose, context, terminalProfileChoices = [], onCreate, onCreateChild, onOpenDocumentation }: NodeCatalogDialogProps) {
  ...
  const visible = useMemo(() => {
    ...
    return searchNodeCatalog(
      categoryEntries,
      field.value,
      field.mode === 'regex' && field.active ? field.test : undefined
    )
  }, [category, field.mode, field.value, field.active, field.pattern, field.flags, field.test])

  const availability = (entry: NodeCatalogEntry) => nodeCatalogAvailability(entry, context)
  const create = (entry: NodeCatalogEntry) => {
    if ((entry.id === 'multiverse-portal' || entry.id === 'aws-universe') && onCreateChild) {
      onCreateChild(entry, newCreationEventId())
      onClose()
      return
    }
    ...
    >
      ...
      <div className="node-catalog-dialog__search">
        <TextField
          ...
          trailingSlot={<AnchoredRegexBuilder search={field} fieldRef={inputRef} label={profileText('nodeCatalog.search.regex', 'Open regex builder for this catalog')} />}
      ...
      <div id="node-catalog-results" className="node-catalog-dialog__list" role="listbox" aria-label={profileText('nodeCatalog.results', 'Node catalog results')}>
        {!visible.length ? (
          ...
          visible.map((entry, index) => {
            const state = availability(entry)
            const availabilityReason = state.reason ? profileText(`nodeCatalog.reason.${entry.id}`, state.reason) : ''
            const label = entryText(entry, 'label')
            const description = entryText(entry, 'description')
            const childEntry = entry.id === 'multiverse-portal' || entry.id === 'aws-universe'
            const disabled = !state.available && !childEntry
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {disabled && <span className="node-catalog-dialog__row-reason">{availabilityReason}</span>}
                </button>

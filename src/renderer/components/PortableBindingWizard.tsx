    ...
    <div className="md3-dialog md3-portable-binding-wizard" role="dialog" aria-label={`Binding options for ${displayLabel}`}>
      ...
      {selected && selected !== 'leave-unbound' && (
        <div className="md3-portable-binding-form">
          ...
          <label>Verified local resource reference <Input value={reference} onChange={(event) => setReference(event.target.value)} /></label>
          {selected === 'locate-asset' && <Button type="button" onClick={() => void window.nodeTerminal.dialog.selectFile().then((picked) => { if (picked) setReference(picked) })}>Browse for asset…</Button>}

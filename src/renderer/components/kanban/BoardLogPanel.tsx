...
import { Button, Checkbox, SearchField, TextArea } from '@renderer/ui/md3'
...
export function eventBody(e: BoardLogEvent): string {
  switch (e.type) {
    ...
    case 'agent-message':
      return `sent a message to ${e.to ?? 'an agent'}${e.title ? ` (${e.title})` : ''}`
    case 'agent-read-cookies':
      return `read cookies for ${e.to ?? 'the requested domain'}${e.title ? ` via ${e.title}` : ''}`
    default:
      ...
      {!unsupported && (
        ...
        >
          ...
          {queued.length > 0 && <div className="board-log__attachment-list" aria-live="polite">
            <div className="board-log__attachment-search menu-filter"><SearchField ref={queueSearchRef} dense vocabularyMode="factual" className="menu-filter__row" value={queueSearch.value} onChange={(event) => queueSearch.setValue(event.target.value)} placeholder={ts('kanban.comments.searchQueued', 'Search queued attachments')} aria-label={ts('kanban.comments.searchQueued', 'Search queued attachments')} trailingSlot={<AnchoredRegexBuilder search={queueSearch} fieldRef={queueSearchRef} label="Regex for queued attachments" />} /><span className="board-log__attachment-status">{visibleQueue.length} shown</span></div>
            <div className="board-log__attachment-bulk"><Button variant="text" size="small" vocabularyMode="factual" className="board-log__attachment-remove" onClick={() => setSelectedQueued(new Set(visibleQueue.map((item) => item.id)))}>{ts('kanban.comments.selectAll', 'Select all shown')}</Button><Button variant="text" size="small" vocabularyMode="factual" className="board-log__attachment-remove" onClick={() => setSelectedQueued((current) => new Set(visibleQueue.filter((item) => !current.has(item.id)).map((item) => item.id)))}>{ts('kanban.comments.invert', 'Invert shown')}</Button><Button variant="text" size="small" vocabularyMode="factual" className="board-log__attachment-remove" onClick={() => { for (const item of visibleQueue) if (selectedQueued.has(item.id) && item.preview) URL.revokeObjectURL(item.preview); const remaining = queuedRef.current.filter((item) => !selectedQueued.has(item.id)); queuedRef.current = remaining; setQueued(remaining); setSelectedQueued(new Set()) }} disabled={selectedQueued.size === 0}>{ts('kanban.comments.remove', 'Remove')}</Button></div>
          ...
          {(draft.trim() || queued.some((item) => item.status === 'ready')) && <Button size="small" vocabularyMode="factual" className="board-log__submit" onClick={() => void send()} disabled={posting || queued.some((item) => item.status === 'reading' || item.status === 'uploading')} aria-label={ts('kanban.comments.post', 'Post comment')}>{posting ? ts('kanban.comments.posting', 'Posting…') : ts('kanban.comments.post', 'Post comment')}</Button>}
        </div>
      )}
      <div className="board-log__feed">
        {feed.length > 0 && <div className="board-log__feed-search menu-filter"><SearchField ref={postedSearchRef} dense vocabularyMode="factual" className="menu-filter__row" value={postedSearch.value} onChange={(event) => postedSearch.setValue(event.target.value)} placeholder={ts('kanban.comments.searchPosted', 'Search comments and attachments')} aria-label={ts('kanban.comments.searchPosted', 'Search comments and attachments')} trailingSlot={<AnchoredRegexBuilder search={postedSearch} fieldRef={postedSearchRef} label="Regex for comments and attachments" />} /><div className="board-log__attachment-bulk"><span className="board-log__attachment-status">{visibleFeed.length} shown</span><Button variant="text" size="small" vocabularyMode="factual" className="board-log__attachment-remove" onClick={() => setSelectedPosted(new Set(visibleFeed.flatMap((entry) => entry.attachments ?? []).map((attachment) => attachment.id)))}>{ts('kanban.comments.selectAll', 'Select all shown')}</Button><Button variant="text" size="small" vocabularyMode="factual" className="board-log__attachment-remove" onClick={() => setSelectedPosted((current) => new Set(visibleFeed.flatMap((entry) => entry.attachments ?? []).filter((attachment) => !current.has(attachment.id)).map((attachment) => attachment.id)))}>{ts('kanban.comments.invert', 'Invert shown')}</Button><Button variant="text" size="small" vocabularyMode="factual" className="board-log__attachment-remove" onClick={() => void exportPosted()} disabled={selectedPosted.size === 0}>{ts('kanban.comments.export', 'Export selected')}</Button></div></div>}
...
function AttachmentPreview({ item, selected, onSelect, onRemove }: { item: QueuedAttachment; selected: boolean; onSelect: () => void; onRemove: () => void }) {
  ...
  return <div className={`board-log__attachment board-log__attachment--${item.status}`}>
    <Checkbox checked={selected} onChange={onSelect} aria-label={`${ts('kanban.comments.select', 'Select')} ${item.file.name}`} />
    ...
    {item.status !== 'saved' && <Button variant="text" size="small" vocabularyMode="factual" className="board-log__attachment-remove" onClick={onRemove} aria-label={`${ts('kanban.comments.remove', 'Remove')} ${item.file.name}`}>{ts('kanban.comments.remove', 'Remove')}</Button>}
  </div>
}
...
function PostedAttachment({ api, projectId, attachment, selected, onSelect }: { api: ReturnType<typeof useSession>['api']; projectId: string; attachment: NonNullable<BoardLogEntry['attachments']>[number]; selected: boolean; onSelect: () => void }) {
  ...
  return <div className="board-log__posted-attachment">
    <Checkbox checked={selected} onChange={onSelect} aria-label={`${ts('kanban.comments.select', 'Select')} ${attachment.displayName}`} />

...
//
// THE one thing standing between a hostile project.json and a grant: a cloned repo can arrive
// with `agentBrowserControl: true` already in its git-shared file. The notice must
...
//    via git is refused AND re-noticed, never silently granted.
//    to activate (I1),
//  - record WHICH answer was given ('kept'/'declined') machine-locally (Project.capabilityAck →
...
/** The grantedness a consuming feature would compute from the live store, per call. */
...
describe('the one-time clone notice', () => {
  ...
  it('Escape is a NON-answer: nothing recorded, nothing granted, and the notice returns next launch', () => {
  ...
  it('Escape is a NON-answer: nothing recorded, nothing granted, and the notice returns next launch (I1)', () => {
    ...
    expect(dialog()).toBeNull()
    const p = useProjects.getState().getProject('p1')!
    expect(p.capabilityAck).toBeUndefined()
    expect(p[cap]).toBe(true)
    expect(grantedNow()).toBe(false)
    unmount()
    mount()

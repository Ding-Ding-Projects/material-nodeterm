...
export function Canvas() {
  ...
  const deleteNodes = useCallback(
    async (ids: string[], captured?: DeleteRequestEpoch) => {
  ...
  useEffect(() => {
    const onAccountRemoved = (ev: Event): void => {
      ...
      const loginIds = nodesRef.current
        ...
        .map((n) => n.id)
      const clearAccountFromSafeNodes = (failedLoginIds: ReadonlySet<string>): void => {
        setNodes((ns) =>
          ns.some((n) => n.data.accountId === accountId)
            ? ns.map((n) =>
                n.data.accountId === accountId && !failedLoginIds.has(n.id)
      ...
      if (!loginIds.length) {
        clearAccountFromSafeNodes(new Set())
        return
      }
      void deleteNodes(loginIds).then(
        (outcome) => {
          // A login node whose backing kill is unknown must keep both the node and its accountId.
          // Clearing only the id would turn a retryable failed close into a system-account login.
          clearAccountFromSafeNodes(new Set(outcome.failed.map(({ nodeId }) => nodeId)))
        },
        (error) => {
          setNotice({
            kind: 'error',
            text: `Could not safely remove the account login session: ${error instanceof Error ? error.message : String(error)}. The node was kept so you can retry.`
          })
        }
      )
    }
  ...
  }, [setNodes, markDirty, deleteNodes, setNotice])
        ...
        switch (verb) {
          ...
          case 'close': {
            ...
            requestDeleteNodes([args.node], {
              ...
              requestedBy: srcTitle,
              perform: (epoch) => {
                // Canonical teardown acknowledges the backing kill before it removes the node.
                // Reply only after that acknowledgement; a transport failure stays retryable and
                // must never become a false `closed` answer to the requesting agent.
                void deleteNodes([args.node], epoch).then((outcome) => {
                  if (outcome.confirmed.includes(args.node!)) {
                    setControlEdges((es) =>
                  ...
                  reply({
                    ...
                    error:
                      ...
                      'the session host did not acknowledge the close — try again'
                  })
                })
              },
              onCancel: () => reply({ ok: false, error: 'denied by user' }),
  ...
  const closeSession = useCallback(
    (projectId: string, id: string, alsoOnSuccess?: () => void) => {
      ...
      requestDeleteNodes([id], {
        ...
        perform: (epoch) => {
          if (epoch.targets.some((node) => node.id === id)) {
            void deleteNodes([id], epoch).then((outcome) => {
            ...
            const projectSession = projectScope.session
            const projectAgentStatus = projectScope.stores.agentStatus.store
            void projectSession.api.pty.destroy(id).then(
              () => {
                const store = useProjects.getState()
  ...
  const killSessionById = useCallback(
    (nodeId: string, orphan: boolean) => {
      ...
      requestDeleteNodes([nodeId], {
        ...
        perform: () => {
          void scopeSession.api.pty.destroy(nodeId, { everySocket: true }).then(
            () => {
              remoteKill?.()

...
interface RequestDeleteNodesOptions {
  ...
  perform?: (
    epoch: DeleteRequestEpoch,
    authorization: DestructiveAuthorization
  ) => void | Promise<void>
  /** Exact target reader for a non-canvas session incarnation such as a session-memory orphan. */
  ...
  onRejected?: () => void
  /** Target/project/session/policy evidence changed or became unreadable before commit. */
  onStale?: () => void
}
...
interface RemoveState {
  ...
  affectedNodes: WorktreeRemovalTarget['affectedNodes']
...
export function Canvas() {
  ...
  // Transient canvas drill state. Group/node drills reuse the same project and merge edits back;
  // project-reference drills switch projects and retain the source project for the back action.
  ...
  /** One consumed worktree dialog/gate may own at most one proof-and-remove transaction. */
  const worktreeRemovalOperationRef = useRef<symbol | null>(null)
  /** Prevent deleting the group node while its exact binding is being removed from disk. */
  ...
  const activeProject = useProjects((s) => s.projects.find((project) => project.id === s.activeProjectId))
  ...
  const allNodes = useMemo(() => {
    const base =
      ephemeralNodes.length || projectionNodes.length
        ? [...nodes, ...ephemeralNodes, ...projectionNodes]
        : nodes
  ...
  }, [nodes, ephemeralNodes, projectionNodes, adhdFocusId])
  ...
  useEffect(() => {
    ...
    const contextEdges = typedLinks
      .filter((link) => link.kind === 'context')
      .map(nodeEndpoints)
      .filter((edge): edge is { id: string; source: string; target: string } => edge !== null)
    const lineageEdges = typedLinks
      .filter((link) => link.kind === 'lineage')
      .map(nodeEndpoints)
      .filter((edge): edge is { id: string; source: string; target: string } => edge !== null)
    const contextLinks = contextEdges.length ? contextEdges : project.bridges ?? []
    const lineageLinks = lineageEdges.length ? lineageEdges : project.ropes ?? []
    const restoredLinks = contextLinks.map((edge) => ({
      id: edge.id,
      source: edge.source,
      target: edge.target
    }))
    const restoredRopes = lineageLinks.map((r) => {
    ...
    // Keep the synchronous refs paired with the React Flow state. Edge commands can arrive
    // between this effect and the next render, so reading only state here would route a new link
    // against the previously active project's edge set.
    linkEdgesRef.current = restoredLinks
    controlEdgesRef.current = restoredRopes
    setLinkEdges(restoredLinks)
    // Restore control ropes with the source agent's color (falls back to the browser blue).
    setControlEdges(restoredRopes)
  ...
  const publishableLater = useCallback(
    (
      ...
      const overrideLinks = override?.links
      return () => {
        const bridges = (overrideBridges ?? linkEdgesRef.current).map(toBridgeLink)
        const ropes = (overrideRopes ?? controlEdgesRef.current).map(toBridgeLink)
        const flowStates = flowToNodeStates(flow)
        const currentProject = useProjects.getState().projects.find((p) => p.id === nodesProjectIdRef.current)
        const existingLinks = currentProject?.links ?? [
          ...(currentProject?.bridges ?? []).map((edge) => typedNodeLink('context', edge)),
          ...(currentProject?.ropes ?? []).map((edge) => typedNodeLink('lineage', edge))
        ]
        const links = overrideLinks ?? mergeVisibleNodeLinks(existingLinks, new Set(flowStates.map((node) => node.id)), bridges, ropes)
        publishableScene({ nodes: flowStates, bridges, ropes, links }, ephIds)
      }
    },
    []
  )
  ...
  const commitActiveToStore = useCallback(() => {
    ...
    const currentProject = store.getProject(activeId)
    const currentStates = flowToNodeStates(nodesRef.current)
    const activeDrill = drillRef.current
    const persistedNodes =
      currentProject && activeDrill?.projectId === activeId && activeDrill.kind === 'group'
        ? remergeDrilledNodes(
            currentProject.nodes,
            currentStates,
            activeDrill.groupId,
            nodeStatesToFlow(currentProject.nodes)
          )
        : currentProject && activeDrill?.projectId === activeId && activeDrill.kind === 'node'
          ? currentStates.length > 0
            ? mergeSingleNode(currentProject.nodes, currentStates[0], nodeStatesToFlow(currentProject.nodes))
            : currentProject.nodes
          : currentStates
    const bridges = linkEdgesRef.current.map(toBridgeLink)
    const ropes = controlEdgesRef.current.map(toBridgeLink)
    const existingLinks = currentProject?.links ?? [
      ...(currentProject?.bridges ?? []).map((edge) => typedNodeLink('context', edge)),
      ...(currentProject?.ropes ?? []).map((edge) => typedNodeLink('lineage', edge))
    ]
    const links = mergeVisibleNodeLinks(existingLinks, new Set(currentStates.map((node) => node.id)), bridges, ropes)
    commitActiveCanvas(
      {
  ...
  const refreshWorktreeStore = useCallback(
    (change?: { bind?: BoundGroup; unbound?: string | string[] }) => {
      ...
      void useWorktrees.getState().refresh(project.cwd, bound, project.id)
    },
    [activeProjectId]
  )
  ...
  const releaseWorktreeBinding = useCallback(
    async (groupId: string): Promise<void> => {
      ...
      await api.git
        .worktreeRemove(wt.repoPath, wt.path, { mode: 'prune' })
        .catch(() => {})
    },
  ...
  const requestDeleteNodes = useCallback(
    (ids: string[], options: RequestDeleteNodesOptions = {}): boolean => {
      ...
      if (ids.some((id) => worktreeRemovalInFlightRef.current.has(id))) {
        ...
        setNotice({
          kind: 'error',
          text: 'That worktree binding is being removed from disk. Wait for it to finish before deleting the group.'
        })
        return false
      }
      ...
      const requested = new Set(ids)
      const defaultReadCurrentTarget = (): NodeDeletionTarget[] | null => {
        const store = useProjects.getState()
        const runtimeIdentity = sessionForProject(targetProjectId).id
        const source:
          | readonly CanvasNode[]
          | readonly CanvasNodeState[]
          | null =
          store.activeProjectId === targetProjectId && nodesProjectIdRef.current === targetProjectId
            ? nodesRef.current
            : store.getProject(targetProjectId)?.nodes ?? null
        if (!source) return null
        const current = ids.map((id) => source.find((node) => node.id === id))
        if (current.some((node) => !node)) return null
        return current.map((node) => {
          const live = node as CanvasNode
          const stored = node as CanvasNodeState
          return {
            projectId: targetProjectId,
            id: node!.id,
            type: 'type' in node! ? live.type : stored.kind,
            title: String('data' in node! ? live.data.title ?? '' : stored.title ?? ''),
            accountId:
              ('data' in node! ? live.data.accountId : stored.accountId) as string | undefined,
            parentId: ('parentId' in node! ? node!.parentId : undefined) ?? undefined,
            incarnation: nodeDeletionTargetIncarnation(node!),
            runtimeIdentity
          }
        })
      }
      const readCurrentTarget = options.readCurrentTarget ?? defaultReadCurrentTarget
      ...
      const accepted = dispatchNodeDeletion(plan, {
        perform: (authorization) => {
          const commit = createNodeDeletionCommitBarrier({
            ...
            perform: () => {
              if (options.perform) void options.perform(requestEpoch, authorization)
              else void deleteNodes(ids, requestEpoch)
            },
            upgradeToTwoKey: () => {
              // The ordinary approval is spent. Start over so the two-key gate discloses a fresh
              // project/node/session incarnation under the now-authoritative stricter policy.
    ...
    [confirmBusy, deleteNodes, setConfirm, setNotice]
  )
  ...
  const createWorktreeAndGroup = useCallback(
    async (v: WorktreeCreateValue) => {
      ...
      attachWorktree(target, worktreeFromCreate({ ...v, ownership: res.worktreeOwnership }))
      setWorktreeDialog(null)
    },
  ...
  // Sidebar adoption creates a fresh group around an already-discovered orphan. It is intentionally
  // routed through the same `worktreeFromEntry` and `attachWorktree` helpers as the context-menu
  // picker, so ownership, base-ref selection, and refresh behavior stay identical.
  const bindWorktreeFromSidebar = useCallback(
    (entry: WorktreeEntry) => {
      const projectId = useProjects.getState().activeProjectId
      const project = projectId ? useProjects.getState().getProject(projectId) : undefined
      const { repoRoot, entries } = useWorktrees.getState()
      if (!projectId || !project || project.ssh || !repoRoot) {
        setNotice({ kind: 'error', text: 'This worktree can only be bound from a local repository.' })
        return
      }
      const binding = worktreeFromEntry(entry, repoRoot, resolveBaseRef(entries))
      if (!binding) {
        setNotice({ kind: 'error', text: 'That worktree has a detached HEAD. Check out a branch in it first.' })
        return
      }
      attachWorktree({ groupId: null, at: viewCenter() }, binding)
    },
    [attachWorktree, setNotice, viewCenter]
  )
  ...
  const normalizeWorktreeBinding = useCallback((projectId: string, groupId: string): boolean => {
    ...
    const worktree = group?.data.worktree
    if (!group || group.type !== 'group' || !worktree) return false
    if (worktree.bindingId) return true
    const normalized = { ...worktree, bindingId: uuid() }
    const next = nodesRef.current.map((node) =>
      node.id === groupId
        ? { ...node, data: { ...node.data, worktree: normalized } }
        : node
    ) as CanvasNode[]
    nodesRef.current = next
    setNodes(next)
  ...
  /** Exact renderer-side incarnation/binding target paired with the core's physical proof. */
  const liveWorktreeRemovalTarget = useCallback(
    (projectId: string, groupId: string): WorktreeRemovalTarget | null => {
      ...
      const group = nodesRef.current.find((node) => node.id === groupId)
      const worktree = group?.data.worktree
      if (!group || group.type !== 'group' || !worktree?.bindingId) return null
      const affected = displacedByWorktree(nodesRef.current, groupId, worktree.path)
      const affectedNodes = nodesRef.current
      ...
      return {
        projectId,
        groupId,
        groupIncarnation: nodeDeletionTargetIncarnation(group),
        affectedNodes,
        worktree: worktree as IdentifiedGroupWorktree
      }
    },
    []
  )

  // Ask first, but only after core supplies an authoritative full-tree proof. A failed proof read
  // is not a clean worktree and cannot open a destructive confirmation.
  const requestRemoveWorktree = useCallback(
    async (
      ...
      const projectId = useProjects.getState().activeProjectId ?? ''
      if (!normalizeWorktreeBinding(projectId, groupId))
        return { ok: false, error: `${groupId} is not a worktree-bound group` }
      const disclosed = liveWorktreeRemovalTarget(projectId, groupId)
      if (!disclosed) return { ok: false, error: 'the worktree binding is unavailable' }
      // Held across the `await` below: without it a second call slips through the gap before
      // `removeTarget` exists.
      removePendingRef.current = true
      try {
        const proofResult = await api.git
          .worktreeRemovalProof(disclosed.worktree.repoPath, disclosed.worktree.path)
          .catch(() => null)
        const current = liveWorktreeRemovalTarget(projectId, groupId)
        if (!sameWorktreeRemovalTarget(disclosed, current))
          throw new Error('the project, group, nodes, or worktree binding changed while it was being read')
        const proof = worktreeRemovalProofForTarget(proofResult, current!)
        if (!proof) throw new Error(proofResult?.message || 'the worktree removal proof is unavailable')
        const canDelete = proof.ownership.directoryCreatedByApp
        setDeleteFromDisk(
          initialWorktreeDeleteFromDisk(canDelete, kidsDestructiveGateRequired())
        )
        setRemoveTarget({
          projectId,
          groupId,
          groupIncarnation: current!.groupIncarnation,
          affectedNodes: current!.affectedNodes,
          worktree: current!.worktree,
          proof,
          warning: worktreeRemovalProofWarning(proof),
          canDelete,
    ...
    [confirmBusy, liveWorktreeRemovalTarget, normalizeWorktreeBinding, setRemoveTarget]
  )
  ...
  // Confirmed disk removal is proof-bound and one-shot. Git runs first; session/canvas cleanup is
  // still acknowledgement-driven, so a refused or unreadable proof touches no running process.
  const confirmRemoveWorktree = useCallback(() => {
    ...
    const disclosedTarget: WorktreeRemovalTarget = {
      projectId: disclosed.projectId,
      groupId: disclosed.groupId,
      groupIncarnation: disclosed.groupIncarnation,
      affectedNodes: disclosed.affectedNodes,
      worktree: disclosed.worktree
    }
    const readCurrent = (): WorktreeRemovalTarget | null =>
      liveWorktreeRemovalTarget(disclosed.projectId, disclosed.groupId)
    const finish = (): void => {
    ...
    // Unbind remains non-destructive and never opens the Kids gate. It still revalidates the exact
    // target before its helper, then clears only if the same binding remains after stale pruning.
    if (!removeFromDisk) {
      const current = readCurrent()
      if (!sameWorktreeRemovalTarget(disclosedTarget, current)) {
        refuse('The worktree binding changed while confirmation was open. Nothing was unbound.')
        finish()
        return
      }
      void releaseWorktreeBinding(disclosed.groupId).finally(() => {
        if (sameWorktreeRemovalBinding(disclosedTarget, readCurrent())) {
          clearWorktreeBinding(disclosed.groupId)
          setNotice({
            kind: 'info',
            text: `Unbound ${disclosed.worktree.branch}. The worktree is still on disk.`
          })
        } else {
          refuse('The group was rebound while unbinding. Its current binding was kept.')
        }
        finish()
      })
      return
    ...
    const commits: Partial<
      Record<DestructiveAuthorization, ReturnType<typeof createWorktreeDiskRemovalCommit<GitResult>>>
    > = {}
    const run = async (authorization: DestructiveAuthorization): Promise<void> => {
      let keepPendingForUpgrade = false
      const commit = commits[authorization] ?? createWorktreeDiskRemovalCommit({
        disclosedTarget,
        disclosedProof: disclosed.proof,
        authorization,
        readCurrent,
        kidsGateRequired: kidsDestructiveGateRequired,
        readProof: (target) =>
          api.git.worktreeRemovalProof(target.worktree.repoPath, target.worktree.path),
        remove: (target, proof) =>
          api.git.worktreeRemove(target.worktree.repoPath, target.worktree.path, {
            mode: 'remove',
            proof,
            deleteBranch: proof.ownership.branchCreatedByApp
          }),
        upgradeToTwoKey: openTwoKey,
        refuse: (reason) => {
          refuse(
            reason === 'proof-changed'
              ? 'The worktree contents changed after confirmation. Nothing was removed; review it and try again.'
              : reason === 'proof-unavailable'
                ? 'The worktree could not be proved after confirmation. Nothing was removed.'
                : 'The project, group, nodes, policy, or worktree binding changed. Nothing was removed.'
          )
        }
      })
      commits[authorization] = commit
      try {
        const outcome = await commit()
        if (outcome.kind === 'upgraded-to-two-key') {
          // `openTwoKey` has transferred ownership to the fresh gate. Keep the exact binding and
          // one-shot operation reserved until that gate confirms or cancels; otherwise another
          // removal can slip into the interval before its callback spends the approval.
          keepPendingForUpgrade = true
          return
        }
        ...
        // Core has acknowledged that A is gone. Never clear a replacement binding B. For the same
        // binding, preserve the existing session recycle/acknowledgement semantics before clearing.
        if (!sameWorktreeRemovalBinding(disclosedTarget, readCurrent())) {
          refuse(`${res.message} The group was rebound while removal ran, so its current binding was kept.`)
          finish()
          return
        }
        if (await resetDisplacedCwd(disclosed.groupId, disclosed.worktree.path, true)) {
          if (sameWorktreeRemovalBinding(disclosedTarget, readCurrent()))
            clearWorktreeBinding(disclosed.groupId)
          else
            refuse('The worktree was removed, but the group was rebound during session cleanup. Its current binding was kept.')
        }
        setNotice({ kind: res.ok ? 'info' : 'error', text: res.message })
      } catch (error) {
        refuse(`Could not remove the worktree: ${error instanceof Error ? error.message : String(error)}`)
      } finally {
        if (!keepPendingForUpgrade) finish()
      }
    }
    ...
    function openTwoKey(target: WorktreeRemovalTarget): void {
      const accepted = openDestructiveGate({
        ...
        description:
          ...
          (t.proof.ownership.branchCreatedByApp
            ? 'Its branch is also deleted only if Git confirms it is already merged. '
            : 'The branch is kept. ') +
          t.warning,
        affected: [target.worktree.branch, target.worktree.path],
        confirmLabel: 'Delete',
        onConfirm: () => { void run('two-key') },
        onCancel: finish
      })
      if (!accepted) {
        refuse('Another destructive confirmation is already open. Nothing was removed.')
        finish()
      }
    }
  ...
  const onWorktreeAction = useCallback(
    (groupId: string, action: 'merge' | 'remove' | 'unbind' | 'sync') => {
        ...
        ? {
            ...
            text: clearEnv
              ...
              : targetModel
                ...
                : targetAgentId
                  ? 'Session reopened with the selected agent — conversation resumed.'
                  : 'Agent restarted — conversation resumed.'
          }
        : outcome === 'model-no-session'
          ? { kind: 'error', text: 'Model switch skipped: this session has no resumable provider id.' }
          : outcome === 'model-unavailable'
            ? { kind: 'error', text: 'Model switch skipped: the target harness or model is unavailable.' }
            : outcome === 'model-pane-mismatch'
              ? { kind: 'error', text: 'Model switch skipped: the expected agent does not own this pane.' }
  ...
  const branchClaude = useCallback(
    async (
      ...
      const launchPlan = activeAgentLaunchPlan('branch-conversation')
      ...
      copy.data = {
        ...
        agentLaunchIntent: {
          ...
          permissionMode: launchPlan.mode
        },
    ...
    return tidySeparators<MenuItem>([
      ...
      ...(ids.length === 1
        ? (() => {
            ...
            const agent = node?.data.agentId as AgentId | undefined
            if (!agent || !canSwitchModel(agent) || gatewayModels.length === 0) return []
            const currentModel = node?.data.agentModel as string | undefined
            return [
              {
                type: 'submenu',
                label: currentModel ? `Model: ${currentModel}` : 'Choose model…',
                icon: <IconSwitch />,
                children: gatewayModels.map((model) => ({
                  label: model.name ? `${model.name} (${model.id})` : model.id,
                  hint: model.provider
                    ? `${model.provider}${model.contextWindow ? ` · ${model.contextWindow} context` : ''}`
                    : undefined,
                  disabled: model.id === currentModel,
                  onClick: () => switchAgentModel(ids[0], model.id)
                }))
              }
            ] as MenuItem[]
          })()
      ...
      ...(ids.length === 1
        ? (() => {
            const node = nodesRef.current.find((candidate) => candidate.id === ids[0])
            const agent = node?.data.agentId as AgentId | undefined
            if (!agent || !vanillaEnvStripPattern(agent)) return []
            return [
              {
                label: 'Restart on subscription',
                icon: <IconPower />,
                disabled: activeSession.source === 'relay',
                hint:
                  activeSession.source === 'relay'
                    ? 'The relay host owns this session environment.'
                    : 'Recreates this idle session with gateway and inherited provider variables removed.',
                onClick: () => restartOnSubscription(ids[0])
              }
            ] as MenuItem[]
          })()
        : []),
      ...
      ...(ids.length === 1
        ? (() => {
            ...
            const gate = restartEligibility(
              restartAgentIdOf(n),
              st?.state,
              restartSessionId(st?.sessionId, n?.data.agentSessionId)
            )
  ...
  const groupItems = useCallback(
    (groupId: string, at?: { x: number; y: number }): MenuItem[] => {
      ...
      return tidySeparators<MenuItem>([
        ...
        ...(() => {
          const current = nodesRef.current.find((node) => node.id === groupId)
          const selectedRef = current?.data.projectRef?.projectId
          const candidates = useProjects
            .getState()
            .projects.filter((project) => project.id !== activeProjectId && !project.closed && !project.unavailable)
          if (!candidates.length && !selectedRef) return []
          return [
            {
              type: 'submenu',
              label: selectedRef ? 'Change referenced project…' : 'Reference project…',
              icon: <IconProject />,
              children: [
                ...(selectedRef
                  ? [{ label: 'Clear reference', onClick: () => setGroupProjectRef(groupId, undefined) }]
                  : []),
                ...candidates.map((project) => ({
                  label: project.name,
                  onClick: () => setGroupProjectRef(groupId, project.id)
                }))
              ]
            }
          ] as MenuItem[]
  ...
  const deliverMailboxMessage = useCallback(
    async (message: AgentMessage): Promise<boolean> => {
      ...
      const harnessAgent = createdAgentHarnessId(target.data)
      ...
      if (!configuredAgent || !harnessAgent || (live?.agentId !== configuredAgent && live?.agentId !== harnessAgent)) return false
      const expectedProcess = agentConfig(harnessAgent)?.expectedProcess
  ...
  useEffect(() => {
    return api.onAgentControl(async ({ requestId, sourceNodeId, verb, args }) => {
      ...
      // Targeted opens are applied to the requested project's data owner. The target is resolved
      // only against projects already open in this client, so an agent cannot turn a path-shaped
      // flag into an arbitrary local checkout. A cross-project request never falls through to the
      // caller's canvas: an unknown, closed, unavailable, SSH, or relay target is refused rather
      // than silently creating a session in the wrong project.
      if (
        ...
        const target = store.projects.find(
          (project) => project.id === requestedProject || project.cwd === requestedProject
        )
        if (!target) {
          reply({ ok: false, error: 'No open project matches --project. Open the target folder first.' })
          return
        }
        const liveSource = nodesRef.current.find((node) => node.id === sourceNodeId)
        const owners = store.projects.filter((project) =>
          project.nodes.some((node) => node.id === sourceNodeId)
        )
        const sourceProject = liveSource
        ...
        if (
          !sourceProject ||
          (owners.length > 0 && !owners.some((project) => project.id === sourceProject.id)) ||
          owners.length > 1
        ) {
          reply({ ok: false, error: 'The source node does not have one unambiguous owning project.' })
          return
        }
        if (target.id === sourceProject.id) {
          // The ordinary handler below owns same-project opens, including --group and --after.
        } else {
          if (target.closed || target.unavailable) {
            reply({ ok: false, error: 'The target project is closed or unavailable.' })
            return
          }
          if (target.ssh || target.remote) {
            reply({ ok: false, error: 'Opening sessions in SSH or relay projects is not supported.' })
            return
          }
          if (args.group || args.after) {
            reply({
              ok: false,
              error: 'Cross-project opens do not accept --group or --after; target the project first.'
            })
            return
          }
          const sourceData = liveSource?.data ?? sourceProject.nodes.find((node) => node.id === sourceNodeId)
          if (!sourceData || !sourceIsControlCapable(sourceData.agentId, sourceData.agentBaseId)) {
            reply({ ok: false, error: 'source node is not a control-capable agent' })
            return
          }
          const targetIsAgent = verb !== 'open-terminal'
          const targetAgent = targetIsAgent
            ? ((verb === 'open-agent' ? args.agent : 'claude') as AgentId)
            : undefined
          const targetSettings = useSettings.getState().settings
          const requestedAccount = args.account === 'system' ? undefined : args.account
          if (args.resume && !targetIsAgent) {
            reply({ ok: false, error: 'Only open-agent can resume an agent session.' })
            return
          }
          if (args.account && targetAgent !== 'codex') {
            reply({ ok: false, error: 'open-agent --account is supported only for Codex' })
            return
          }
          const targetCodexAccounts = codexAccountsForCanvas(targetSettings.codexAccounts, target)
          if (
            requestedAccount &&
            !targetCodexAccounts.some((account) => account.id === requestedAccount)
          ) {
          ...
          const account =
            targetAgent === 'codex'
              ? requestedAccount
              : targetAgent === 'claude'
                ? resolveNewNodeAccount(undefined, target, targetSettings.claudeAccounts)
                : undefined
          const targetCount = args.resume
            ? 1
            : Math.max(1, Math.min(targetAgent === 'claude' ? 5 : 8, parseInt(args.count || '1', 10) || 1))
          const targetCwd = args.cwd?.trim() || target.cwd
          const targetPlan = targetAgent
            ? agentLaunchPlanForProject('canvas-control-open-agent', target, targetAgent)
            : undefined
          const targetOptions = terminalCreationOptionsFor(target.id)
          const targetNodes =
            target.id === store.activeProjectId ? nodesRef.current : nodeStatesToFlow(target.nodes)
          const made: CanvasNode[] = []
          for (let index = 0; index < targetCount; index++) {
            const node = targetAgent
              ? createAgentNode(
                  targetAgent,
                  targetNodes.length + index,
                  targetCwd,
                  { x: 80 + index * 700, y: 80 },
                  args.prompt,
                  undefined,
            ...
            if (args.resume) {
              const resume = resumeCommand(targetAgent!, args.resume, {
                base: agentLaunchOverride(targetAgent!, target)
              })
              if (!resume) {
                reply({ ok: false, error: 'The selected agent does not support session resume.' })
                return
              }
              node.data = {
                ...node.data,
                initialCommand: commandForAgentLaunch(resume, targetPlan!),
                agentLaunchIntent: {
                  kind: 'agent',
                  action: 'resume',
                  agentId: targetAgent!,
                  sessionId: args.resume,
                  ...(node.data.agentModel ? { model: node.data.agentModel as string } : {}),
                  permissionMode: targetPlan!.mode
                },
          ...
          // An inactive target cannot use React Flow's live array. Hold its already-authorized
          // launch in the machine-local pending-launch overlay, so opening the target later starts
          // the exact same semantic command without serializing renderer shell syntax into the
          // shared project file.
          const targetOffersProfiles = canOfferTerminalProfiles(
            !!api.terminalProfiles,
            sessionForProject(target.id).source,
            false
          )
          const prepared = target.id === store.activeProjectId
            ? made
            : made.map((node) => {
                const launch = armedTerminalLaunchIntent(node.data, {
                  offersTerminalProfiles: targetOffersProfiles,
                ...
                return launch
                  ? {
                      ...
                      data: {
                        ...
                        pendingLaunch: { after: [], launchId: uuid(), launch }
                      }
                    }
                  : node
              })
          if (target.id === store.activeProjectId) {
            setNodes((current) => [...current, ...prepared])
          } else {
            for (const node of prepared) {
              const state = flowToNodeStates([node])[0]
              if (!state || !store.addNodeToProject(target.id, state)) {
                reply({ ok: false, error: 'The target project changed before its session could be added.' })
                return
              }
            }
          }

          // The source project owns the durable lineage link. The target node is represented by an
          // xnode endpoint, so the source canvas can render a live projection without copying the
          // target node or its execution fields into the source project's file.
          const sourceLinks = sourceProject.links ?? []
          const lineageLinks = prepared.map((node) => ({
            id: newLinkId(),
            kind: 'lineage' as const,
            source: { ref: 'node' as const, nodeId: sourceNodeId },
            target: { ref: 'xnode' as const, projectId: target.id, nodeId: node.id },
            meta: { displayOnly: true }
          }))
          const allSourceLinks = [...sourceLinks, ...lineageLinks]
          store.setProjectLinks(sourceProject.id, allSourceLinks)
          if (sourceProject.id === store.activeProjectId) {
            publisherRef.current?.publish(
              publishableLater(nodesRef.current, { links: allSourceLinks })
            )
          }
          markDirty()
          if (target.id !== store.activeProjectId) void writeDisk()
          reply({
            ok: true,
            message:
              `opened ${prepared.length} ${targetAgent ?? 'terminal'} session(s) in ${target.name}` +
              (target.id === store.activeProjectId ? '' : '; starts when that project is next viewed'),
            result: { ids: prepared.map((node) => node.id), projectId: target.id, links: lineageLinks.map((link) => link.id) }
          })
          return
        }
      }
      ...
      const route = routeControlSource(
        controlState.projects,
        controlState.activeProjectId,
        sourceNodeId,
        !!src
      )
      if (route.kind === 'ambiguous') {
        reply({
          ok: false,
          error: `source node identity is ambiguous across projects: ${route.projectIds.join(', ')}`
        })
        return
      }
      if (!src) {
        const { projects } = controlState
      ...
      try {
        ...
        switch (verb) {
          ...
          case 'open-agent': {
            ...
            const make = (i: number): CanvasNode => {
              const launchPlan = activeAgentLaunchPlan('canvas-control-open-agent', agentId)
              const node = createAgentNode(
                agentId,
                nodesRef.current.length + i,
                agentCwd,
              ...
              if (args.resume) {
                ...
                node.data = {
                  ...node.data,
                  initialCommand: commandForAgentLaunch(resume, launchPlan),
                  agentLaunchIntent: {
                    kind: 'agent',
                    action: 'resume',
                    agentId,
          ...
          case 'open-worktree': {
            ...
            const groupId = worktreeControlRef.current.attachWorktree(
              ...
              worktreeFromCreate({
                ...
                baseRef,
                path: wtPath,
                ownership: res.worktreeOwnership
              })
            )
            reply({
              ok: true,
  ...
  const killSessionById = useCallback(
    (nodeId: string, orphan: boolean) => {
      ...
      const orphanProjectId = store.activeProjectId
      const readOrphanTarget = (): NodeDeletionTarget[] | null => {
        const currentStore = useProjects.getState()
        const currentPlan = planSessionKill(nodeId, currentStore.projects, orphanProjectId)
        if (currentPlan.ownerProjectId) return null
        const currentSession = projectSessionScope(orphanProjectId).session
        return [{
          projectId: orphanProjectId,
          id: nodeId,
          type: 'terminal',
          title: orphan ? `orphan session ${nodeId}` : nodeId,
          runtimeIdentity: orphanSessionRuntimeIdentity({

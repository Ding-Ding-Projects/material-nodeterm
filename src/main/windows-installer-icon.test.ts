...
describe('Windows installer icon contract', () => {
  ...
  it.each([
    ['Setup', 'setupIcon'],
    ['execution stub', 'stubIcon']
  ] as const)('rejects an icon mutation isolated to the %s PE', async (_name, key) => {
    const mutated = Buffer.from(SOURCE_ICON)
    mutated[mutated.length - 1] ^= 1
    writePackage({ [key]: mutated })
    await expect(assertPackagedIconContract(squirrel, metadataFile, root, {
      sourceIdentity: { sourceSha: SHA, repository: REPOSITORY }

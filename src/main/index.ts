...
app.whenReady().then(async () => {
  ...
  ipcMain.handle(IPC.claudeSkillsList, async () => {
    const accounts = settingsStore.get().claudeAccounts.filter((account) => !account.pending)
    ...
    if (manager) {
      ...
      for (const [hostKey, projectId] of hosts) {
        ...
        for (const account of accounts.filter((candidate) => candidate.host === hostKey)) {
  ...
  registerProviderAccountsHandlers(corePlatform)

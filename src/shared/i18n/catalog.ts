...
export const CATALOG: Catalog = {
  ...
  // Linux ISO VM controls. Operation details and digest values remain facts supplied by the
  // manager, while these labels stay in the shared catalog for all language modes.
  ...
  'virtualMachine.diskFormat': { en: flat('Disk format'), yue: flat('磁碟格式') },
  'virtualMachine.diskFormat.auto': { en: flat('Detect qcow2'), yue: flat('偵測 qcow2') },
  'virtualMachine.diskFormat.qcow2': { en: flat('qcow2'), yue: flat('qcow2') },
  'virtualMachine.diskFormat.raw': { en: flat('raw, explicitly confirmed'), yue: flat('raw，明確確認') },
  ...
  'virtualMachine.cancel': { en: flat('Cancel'), yue: flat('取消') },
  'virtualMachine.progress': { en: flat('VM operation progress'), yue: flat('虛擬機操作進度') },
  ...
  'virtualMachine.snapshotName': { en: flat('Snapshot name'), yue: flat('快照名稱') },
  'virtualMachine.close': { en: flat('Close'), yue: flat('關閉') },
  'virtualMachine.rename': { en: flat('Rename'), yue: flat('重新命名') },
  'virtualMachine.chooseIso': { en: flat('Choose an ISO file'), yue: flat('選擇 ISO 檔案') },
  'virtualMachine.chooseDisk': { en: flat('Choose a qcow2 or raw disk'), yue: flat('選擇 qcow2 或 raw 磁碟') },
  'virtualMachine.searchMode': { en: flat('Search modes'), yue: flat('搜尋模式') },
  'virtualMachine.regexMode': { en: flat('Regex, mode search'), yue: flat('正則表達式，搜尋模式') },
  'virtualMachine.searchDiskFormat': { en: flat('Search disk formats'), yue: flat('搜尋磁碟格式') },
  'virtualMachine.regexDiskFormat': { en: flat('Regex, disk-format search'), yue: flat('正則表達式，搜尋磁碟格式') },
  'virtualMachine.noMatches': { en: flat('No choices match this filter.'), yue: flat('沒有選項符合此篩選。') },
  'virtualMachine.displayUnavailable': { en: flat('Display unavailable: no bundled viewer or authenticated loopback proxy.'), yue: flat('顯示畫面未能使用：未有內置檢視器或已驗證回環代理。') },
  'virtualMachine.status.error': { en: flat('The VM could not start. Review the error and retry.'), yue: flat('虛擬機未能啟動，請查看錯誤再試。') },
  'virtualMachine.status.ready': { en: flat('Ready to start.'), yue: flat('準備好啟動。') },
  'virtualMachine.status.configure': { en: flat('Choose a Linux ISO and configure the VM.'), yue: flat('選擇 Linux ISO 並設定虛擬機。') },
  'virtualMachine.status.checking': { en: flat('Checking bundled QEMU…'), yue: flat('正在檢查內置 QEMU……') },
  'virtualMachine.status.toolsMissing': { en: flat('Bundled QEMU is unavailable.'), yue: flat('內置 QEMU 未能使用。') },
  'virtualMachine.status.chooseIso': { en: flat('Choose a Linux ISO to continue.'), yue: flat('選擇 Linux ISO 以繼續。') },
  'virtualMachine.status.chooseDisk': { en: flat('Persistent install mode needs a disk image.'), yue: flat('持久安裝模式需要磁碟映像。') },
  'virtualMachine.status.networkOff': { en: flat('Ready. Network is off unless you explicitly enable it.'), yue: flat('準備好。網絡預設關閉，除非你明確啟用。') },
  'virtualMachine.status.running': { en: flat('Linux is running in the VM.'), yue: flat('Linux 正在虛擬機內運行。') },
  'virtualMachine.status.runningServer': { en: flat('Linux is running on the server host.'), yue: flat('Linux 正在伺服器主機運行。') },
  'virtualMachine.status.generic': { en: flat('VM status is available.'), yue: flat('虛擬機狀態已準備。') },
  'virtualMachine.status.starting': { en: flat('Starting QEMU and checking its control and display sockets.'), yue: flat('正在啟動 QEMU 並檢查控制及顯示回環插口。') },
  'virtualMachine.status.stopped': { en: flat('The VM stopped.'), yue: flat('虛擬機已停止。') },
  'virtualMachine.toolsBundled': { en: flat('Tools: bundled QEMU'), yue: flat('工具：內置 QEMU') },
  'virtualMachine.whpxAvailable': { en: flat('WHPX available'), yue: flat('WHPX 可用') },
  'virtualMachine.isoActual': { en: flat('ISO SHA-256 actual:'), yue: flat('ISO SHA-256 實際值：') },
  'virtualMachine.isoExpected': { en: flat('expected'), yue: flat('預期') },
  'virtualMachine.diskSummary': { en: flat('Disk format:'), yue: flat('磁碟格式：') },
  'virtualMachine.freeSpace': { en: flat('free space:'), yue: flat('可用空間：') },
  'virtualMachine.unavailable': { en: flat('unavailable'), yue: flat('未能使用') },

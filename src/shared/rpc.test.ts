...
describe('rpc protocol', () => {
  ...
  it('round-trips an omitted trailing argument as undefined, so default parameters apply', () => {
    const req = parseRpcMessage(
      JSON.stringify({
        ...
        ...encodeArgs(['/repo', 'feat', 'main', undefined])
      })
    )
    expect(req).toEqual({
  ...
  // The `undef` index list must be OUT OF BAND: an in-band sentinel string was compared with `===`
  // against every top-level argument, and several methods take one whole attacker-CHOOSABLE string
  // in a top-level slot — `pty.write(sid, data)` (a paste, or an agent-driven write),
  // `fs.write(path, content)`, presence chat. A payload equal to the sentinel decoded to
  // `undefined`: `pty.write` would have written nothing, `fs.write` would have thrown or truncated.
  it('never lets an argument VALUE pose as the undefined marker', () => {
    const evil = '\0__rpc_undefined__' // the old in-band sentinel, sent as real pty input
    expect(parseRpcMessage(wire('pty:write', 's1', evil))).toEqual({
      t: 'cast', method: 'pty:write', args: ['s1', evil]
    })
    // Even alongside a genuinely omitted slot in the same call.
    expect(parseRpcMessage(wire('fs:write', evil, undefined))).toEqual({
      t: 'cast', method: 'fs:write', args: [evil, undefined]
    })
  })

  it('ignores a junk `undef` list (it can never invent an argument slot)', () => {
    // Straight off a hostile socket: out-of-range / non-integer indexes mark nothing, and the arity
    // the sender chose is what the handler sees.
    expect(
      parseRpcMessage('{"t":"cast","method":"m","args":["a"],"undef":[5,-1,"x",1.5,null]}')
    ).toEqual({ t: 'cast', method: 'm', args: ['a'] })
    expect(parseRpcMessage('{"t":"cast","method":"m","args":["a"],"undef":"nope"}')).toEqual({
      t: 'cast', method: 'm', args: ['a']
    })
  })

 ...
 *  MAIN from its own stores, so nothing that ends up inside an authorization decision is
 *  renderer-supplied beyond the two node ids and message text. */
export interface AgentMessageDeliverRequest {
  verb: AgentMessageVerb
  sourceNodeId: string
  targetNodeId: string
  /** Optional subject for a new send. The core folds it into the framed body without trusting a
   *  renderer-supplied title or address. */
  subject?: string

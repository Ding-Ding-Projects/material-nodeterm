...
// Defaults are stored in the canonical `Ctrl+…` notation (see shared/shortcut.ts): `Ctrl` is the
// supported desktop's only primary modifier, so a shipped default must never be spelled with the
// parse-only `Cmd`/`Command` alias. Pre-rewire settings.json values keep working through it.

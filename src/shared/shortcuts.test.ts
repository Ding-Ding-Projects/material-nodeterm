...
describe('shortcuts registry', () => {
  ...
  it('keeps the registry map and groups complete', () => {
    expect(Object.keys(DEFAULT_SHORTCUTS).sort()).toEqual(SHORTCUT_DEFS.map((d) => d.id).sort())
    const grouped = shortcutGroups().flatMap((group) => group.defs)
    expect(grouped).toHaveLength(SHORTCUT_DEFS.length)
    expect(new Set(grouped.map((def) => def.id)).size).toBe(SHORTCUT_DEFS.length)
  })

...
export interface TorrentTaskState {
  ...
  /** Piece-verification state once the engine has checked the data. */
  integrity?: TorrentIntegrityState
  progress: number
  downloadedBytes: number
  /** Bytes across the selected files only. */
...
/* ---------------------------------------------------------------------------
 * Magnet parsing, bencode validation, the pinned runtime descriptor, redaction and
 * export. Restored from the pre-merge module; these are the strict versions the
 * service and the safety tests rely on.
 * ------------------------------------------------------------------------- */

// Address-bar normalization, in `src/shared` because BOTH shells need it: the renderer's address
// bar (re-exported from `renderer/nodes/browserUrl.ts`, so no existing import path changes) and
// main's CDP allowlist (`browser-cdp-allowlist.ts`), whose `Page.navigate` validator reuses THIS

...
export type TriggerRunOutcome =
  ...
  | 'target-unsupported'
  | 'failed'
  | 'skipped-previous-run-active'
  | 'missed'
  | 'cancelled'

export interface TriggerRunReceipt {
  id: string
  projectId: string
  nodeId: string
  target: string
...
function sanitizeSchedule(value: unknown): TriggerSchedule | undefined {
  ...
  if (s.kind === 'cron') {
    ...
    if (!expr || expr.length > TRIGGER_CRON_MAX || !CRON_CHARSET.test(expr) || fields.length !== 5) return undefined
    const bounds = [[0, 59], [0, 23], [1, 31], [1, 12], [0, 6]]
    if (!fields.every((field, index) => {
      if (!CRON_FIELD.test(field)) return false
      return field.split(',').every((part) => {
        const base = part.split('/')[0]
        const range = base === '*' ? [bounds[index][0], bounds[index][1]] : base.split('-').map(Number)
        return range.length <= 2 && range.every((n) => Number.isInteger(n) && n >= bounds[index][0] && n <= bounds[index][1]) &&

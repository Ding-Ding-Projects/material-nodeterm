...
export const NODE_CATALOG: readonly NodeCatalogEntry[] = [
  ...
  plannedEntry('aws-identity', 'universes', 'AWS identity', 'Inspect the selected AWS identity before creating AWS resources.', 'aws-identity', 'aws-universe'),
  plannedEntry('aws-resource-explorer', 'universes', 'AWS Resource Explorer', 'Search AWS resources through the verified Resource Explorer capability.', 'aws-resource-explorer', 'aws-universe'),
  plannedEntry('aws-s3', 'hosting', 'AWS S3', 'Create a guided S3 storage blueprint with binding configured later.', 'aws-s3', 'aws-universe'),
  plannedEntry('aws-ec2', 'hosting', 'AWS EC2', 'Create a guided EC2 compute blueprint with binding configured later.', 'aws-ec2', 'aws-universe'),
...
export const NODE_CATALOG_COMPLETENESS: readonly NodeCatalogCompletenessRecord[] = [
  ...
  { id: 'aws-identity', state: 'planned', scope: 'aws-universe', reason: 'AWS identity capability not implemented' },
  { id: 'aws-resource-explorer', state: 'planned', scope: 'aws-universe', reason: 'AWS Resource Explorer capability not implemented' },
  { id: 'aws-s3', state: 'planned', scope: 'aws-universe', reason: 'AWS S3 capability not implemented' },
  { id: 'aws-ec2', state: 'planned', scope: 'aws-universe', reason: 'AWS EC2 capability not implemented' },
...
export function searchNodeCatalog(
  ...
  return entries.filter((entry) => {
    const corpus = [entry.label, entry.description, entry.category, ...entry.keywords].join(' ')
    return tester ? tester(corpus) : corpus.toLocaleLowerCase().includes(normalized)
  })
}
...
/** Creation ids are ledger keys, never arbitrary user text. Keep them bounded and visible so a
 * malformed retry cannot poison the in-memory pending ledger or the portable projection. */
...
/** Stable suffix shared by every Shop identity boundary, including peer validation. */
export function shopCanvasIdSuffix(canvasId: string): string {
  let hash = 0x811c9dc5
...
export function isDeterministicShopNodeId(canvasId: string, nodeId: string): boolean {
  return nodeId === `shop-${canvasId}` || nodeId === `shop-${canvasId}-${shopCanvasIdSuffix(canvasId)}`
}

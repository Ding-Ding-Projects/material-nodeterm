...
/* ---------------------------------------------------------------------------
 * Detailed ICS import, configuration assertions, and timezone arithmetic.
 * Restored verbatim from the pre-merge module; the live provider/API shapes above
 * are the ones the desktop, Server Edition, and renderer consume.
 * ------------------------------------------------------------------------- */
...
function parseProperty(line: string): IcsProperty | null {
  ...
  return {
    ...
    params,
    value: unescapeIcsText(line.slice(colon + 1)),
  };
}
    ...
    if (upper === "BEGIN:VTIMEZONE") {
      ...
      try {
        ...
        const resolvedStartZone =
          ...
          fallbackTimezone;
        const start = parseIcsDateStrict(
          {
            ...startProp,

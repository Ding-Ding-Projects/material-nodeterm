...
export function remoteTmuxConf(
  scrollback: number,
  wordSeparators: string | number = DEFAULT_WORD_SEPARATORS,
  leadPaneWidth: number = 0
): string {
  if (typeof wordSeparators === 'number') {
    leadPaneWidth = wordSeparators
...
import { tmuxWordSeparatorsLine } from './word-separators'

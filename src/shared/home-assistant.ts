...
/** Portable service intent for a canvas node. Endpoint, instance and entity bindings stay local. */
export interface HomeAssistantNodeIntent { domain?: string; service?: string }
...
export interface HomeAssistantApi {
  ...
  call(input: { instanceId: string; domain: string; service: string; entityIds: string[]; data: Record<string, unknown>; confirmation?: { kind: 'super-confirmation'; approved: true } }): Promise<{ ok: true; stateChanged: boolean; message?: string } | { ok: false; error: string; retryable?: boolean; permission?: boolean }>
}

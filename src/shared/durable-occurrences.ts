...
function recurrenceError(value: unknown): string | null {
  ...
  if (value.kind === 'weekly') {
    ...
    if (new Set(days).size !== days.length || days.some((day) => !Number.isInteger(day) || day < 0 || day > 6)) return 'Weekly recurrence weekdays are invalid.'
    return null
  }
  if (value.kind === 'monthly') return exact(value, ['kind', 'day']) && Number.isInteger(value.day) && value.day >= 1 && value.day <= 31 ? null : 'Monthly recurrence day is invalid.'
...
function occurrenceError(value: unknown): string | null {
  ...
  if (!object(value.delivery) || !exact(value.delivery, ['idempotencyKey', 'generation', 'acknowledgedAtMs', 'claimId', 'claimedAtMs', 'outcome', 'error']) || typeof value.delivery.idempotencyKey !== 'string' || !ID.test(value.delivery.idempotencyKey) || !Number.isSafeInteger(value.delivery.generation) || value.delivery.generation < 0 || (value.delivery.acknowledgedAtMs !== null && !finiteMs(value.delivery.acknowledgedAtMs)) || (value.delivery.claimId !== null && (typeof value.delivery.claimId !== 'string' || !ID.test(value.delivery.claimId))) || (value.delivery.claimedAtMs !== null && !finiteMs(value.delivery.claimedAtMs)) || !['not-attempted', 'delivered', 'pending', 'failed'].includes(String(value.delivery.outcome)) || (value.delivery.error !== null && !boundedText(value.delivery.error, 500, false))) return 'Occurrence delivery record is invalid.'
  return null
}
...
export function validateDurableOccurrenceSnapshot(raw: unknown): string | null {
  if (!object(raw) || !exact(raw, ['version', 'generation', 'schedules', 'alarms', 'timers', 'occurrences', 'lastWallClockMs', 'lastMonotonicMs', 'monotonicClockId']) || raw.version !== 1 || !Number.isSafeInteger(raw.generation) || raw.generation < 0 || !Array.isArray(raw.schedules) || !Array.isArray(raw.alarms) || !Array.isArray(raw.timers) || !Array.isArray(raw.occurrences) || raw.schedules.length > DURABLE_OCCURRENCE_LIMITS.maxSchedules || raw.alarms.length > DURABLE_OCCURRENCE_LIMITS.maxAlarms || raw.timers.length > DURABLE_OCCURRENCE_LIMITS.maxAlarms || raw.occurrences.length > DURABLE_OCCURRENCE_LIMITS.maxOccurrences || (raw.lastWallClockMs !== null && !finiteMs(raw.lastWallClockMs)) || (raw.lastMonotonicMs !== null && !finiteMs(raw.lastMonotonicMs)) || (raw.monotonicClockId !== null && (typeof raw.monotonicClockId !== 'string' || !ID.test(raw.monotonicClockId)))) return 'Durable occurrence data is malformed, unsupported, or exceeds its bounds.'

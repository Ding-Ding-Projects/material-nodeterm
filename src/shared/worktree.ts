...
/** Shared git-town-compatible parent key used by typed branch dependency links. */
export function branchParentConfigKey(child: string): string | null {
  const normalized = child.trim()
  if (!isValidGitRef(normalized)) return null
  return `git-town-branch.${normalized}.parent`
}

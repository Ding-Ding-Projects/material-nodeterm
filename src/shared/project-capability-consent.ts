 ...
 * decider any capability-consuming feature may use (ported from upstream eneskirca/nodeterm's
 * feat/project-capabilities, including its post-review hardening).
 *
 ...
 *
 * THE ACK CARRIES THE ANSWER, not just the fact of one. "Turn it off" can only delete the field
 * from THIS working copy — the hostile `true` survives in git and a routine `git checkout`/pull
 * restores it. If the ack were a bare bit, that restored `true` would meet a standing
 * acknowledgment and grant silently to a user who explicitly said no. So:
 ...
 *
 * Lives in `src/shared` because the renderer raises the dialog and may not import `src/core`;
 ...
 *  never-answered clone window and the re-arrival of a `true` the user declined: a recorded "no"
 *  makes a re-appearing switch a NEW event to be told about, not a settled one. Off needs no
 ...
 * This is the predicate any consuming feature's admission check consults per call — never cached
 * at lease start.
 */

...
and ordinary push events cannot publish. Automatic publication is therefore disabled, while the
workflow remains manually dispatchable. Manual publication is pending the final packaged update
interactions. Once deliberately dispatched, one job:
...
removed, or separately disabled before it can be treated as harmless. The current workflow being
manually dispatchable does not retroactively protect that old file. Removing those refs is
...
manual dispatch, and its first step accepts only `main`. Automatic publication is disabled, but
the workflow remains dispatchable. Manual publication of `0.4.0` is pending both the real
production-identity `0.3.0` → `0.4.0` Setup migration and the isolated new-code updater fixture
proof. This change does not dispatch or publish it.

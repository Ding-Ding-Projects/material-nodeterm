...
- **Where the token lives.** A Home Assistant token is sealed at rest via the platform's
  ...
  `'home-assistant'`) deletes its canonical and recognized temp files (`pruneOrphanedTokens`, awaited
  after every save). The token boundary accepts only the lowercase RFC 4122 v4 ids generated by the
  editor; a hand-edited alias such as `a/b` cannot collide with `a_b` in the credential filename.
  Set, clear, format-switch cleanup and prune are one ordered transaction stream. If a plausible
  live writer or an I/O error prevents complete cleanup, the schedule remains saved but the save
  result and the editor say that related credentials could not be fully cleared.

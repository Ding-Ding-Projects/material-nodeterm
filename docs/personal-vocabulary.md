...
- ✅ Settings producer ownership is explicit. `SettingsVocabularyResolution.fields` is `section`,
  ...
  section or explicit child row may skip its search metadata mapping. The checker records each section's exact registered
...
- ✅ The Settings audit also carries an explicit mixed-string boundary manifest for the cited
  Accounts, App Identity, Schedule, School, Kids, Narrator, Speech, Terminal, Workspace, Custom
  Agents, SSH, and Support Tickets callsites. Each row names its
  source file and exact stable callsite id, so a section cannot pass merely because some other text
  in the same file happens to be mapped. The checker mutates each real callsite id in a fresh
  fixture, rather than deleting a broad file marker.

The Settings copy/fact inventory uses these stable callsite ids:

| Callsite id | Source |
| --- | --- |
| `settings.accounts.adding-on`, `settings.accounts.unavailable`, `settings.accounts.cli-version-warning`, `settings.accounts.working-directory`, `settings.accounts.credentials-disclosure` | `AccountsSection.tsx` |
| `settings.app-identity.display-name-facts`, `settings.app-identity.current-name`, `settings.app-identity.logo-error`, `settings.app-identity.crop-facts` | `AppIdentitySection.tsx` |
| `settings.appearance.presets-import-result` | `AppearanceEditor.tsx` |
| `settings.schedule.source-error`, `settings.schedule.source-success`, `settings.schedule.source-unchecked`, `settings.schedule.load-error-kind`, `settings.schedule.load-error-path`, `settings.schedule.value-source-status` | `ScheduleSection.tsx` |
| `settings.school.toggle`, `settings.school.no-pin`, `settings.school.error`, `settings.school.recovery`, `settings.school.pin-change-message` | `SchoolModeSection.tsx` |
| `settings.kids.toggle`, `settings.kids.no-pin`, `settings.kids.error`, `settings.kids.recovery`, `settings.kids.disclosure`, `settings.kids.refused-modes`, `settings.kids.refused-mode-reason`, `settings.kids.pin-change-message` | `KidsModeSection.tsx` |
| `settings.narrator.voice-picker`, `settings.narrator.choose-automatically`, `settings.narrator.preview`, `settings.narrator.voices-loading`, `settings.narrator.no-voice`, `settings.narrator.missing-voice`, `settings.narrator.network-voice`, `settings.narrator.active-voice`, `settings.narrator.unavailable`, `settings.narrator.enabled` | `NarratorSection.tsx` |
| `settings.speech.engine`, `settings.speech.shortcut`, `settings.speech.models-heading`, `settings.speech.models-loading`, `settings.speech.language` | `SpeechSection.tsx` |
| `settings.terminal.theme`, `settings.terminal.font`, `settings.terminal.size-weight`, `settings.terminal.spacing`, `settings.terminal.word-separators`, `settings.terminal.cursor`, `settings.terminal.cursor-unfocused`, `settings.terminal.middle-click`, `settings.terminal.bold-bright`, `settings.terminal.minimum-contrast`, `settings.terminal.rendering` | `TerminalSection.tsx` |
| `settings.workspace-storage.split` | `WorkspaceStorageSection.tsx` |
| `settings.custom-agents.label`, `settings.custom-agents.launch-command`, `settings.custom-agents.prompt-injection` | `CustomAgentsSection.tsx` |
| `settings.ssh.label`, `settings.ssh.host`, `settings.ssh.user`, `settings.ssh.port`, `settings.ssh.remote-directory`, `settings.ssh.identity-file`, `settings.ssh.extra-args` | `SshSection.tsx` |
| `settings.support.advance`, `settings.support.recovery-path`, `settings.support.copy-path`, `settings.support.open-folder`, `settings.support.browser-boundary`, `settings.support.no-delete`, `settings.support.close-ticket`, `settings.support.disclosure`, `settings.support.status-label`, `settings.support.canned-response`, `settings.support.severity-value`, `settings.support.category-value`, `settings.support.category-option`, `settings.support.severity-option`, `settings.support.submit`, `settings.support.export`, `settings.support.empty` | `SupportTicketsSection.tsx` |

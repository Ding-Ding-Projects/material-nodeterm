...
the status identifies qcow2 (`QFI\\xfb`) without trusting a filename extension. Raw disks remain
available only after the user explicitly chooses and confirms the raw format.
...
memory, CPU count, ISO, optional disk, snapshot mode, a loopback VNC socket, and a loopback QMP
...
No bundled VNC viewer or authenticated loopback proxy is included yet. The manager therefore keeps
the display unavailable rather than emitting a custom VNC URL to an arbitrary external handler.
Server Edition keeps the display host-local and reports that a browser-safe proxy is not available,
so it never tells a browser to connect to its own `127.0.0.1` by mistake.

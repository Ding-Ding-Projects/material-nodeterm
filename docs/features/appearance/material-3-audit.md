...
| `settings-page` | Settings scope and every production section host | `src/renderer/components/settings/SettingsPage.tsx` | `renderSettingsSectionHosts(` |
| `settings-page-registration` | Settings page vocabulary section registry host | `src/renderer/components/settings/SettingsPage.tsx` | `data-settings-section-host={entry.id}` |
...
| `settings-callsite-owners` | Exact runtime ownership for settings callsite ids | `src/renderer/components/settings/SettingsText.tsx` | `SETTINGS_CALLSITE_OWNERS` |
| `settings-copy-facts` | Typed copy/fact segments and template facts | `src/renderer/components/settings/SettingsText.tsx`, `FieldRow.tsx` | `SettingsTextSegment`, `labelSegments`, `descriptionSegments`, `noteSegments` |
...
| `settings-section-registry` | Shared Settings section identity and renderer for routing and navigation | `src/renderer/components/settings/nav.ts`, `SettingsPage.tsx`, `SettingsSidebar.tsx` | `SETTINGS_SECTION_REGISTRY`, `entry.render` |

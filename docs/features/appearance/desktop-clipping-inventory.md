...
| --- | --- | --- | --- |
| Notifications panel | The search field's row-shaped recipe carried `flex: 1 1 220px`; inside the panel's column flex that basis became the main-axis size, so the pill grew to ~220 px tall around a 34 px input. | The search is the `SearchField` primitive (`min-height` + `flex: 0 0 auto`, never `height` + `flex-basis`), filters are `Chip`s in a `ChipRow`, bulk actions are `Button`s. | Open the panel at 100%, 125%, 150%, and 200% scale in bilingual mode; confirm the search pill is one row tall and every filter chip wraps rather than clips. |
...
| Responsive top app bar | The project switcher, search, notification, and utility actions compete for one row below laptop widths, while duplicated alternate controls can steal focus. | `TopAppBar` measures its rendered width and mounts exactly one wide or compact branch. Compact mode keeps the project switcher, command palette, live collaborator facepile, notifications, and an anchored searchable More menu. More opens the existing AWS Universe, Multiverse, Portals, phone-pairing, dictation, and Help paths through their existing state and callbacks. Narrow mode makes the project and More controls icon-first below 720 CSS pixels, and a shared More-button anchor returns focus after nested pickers close. | Capture 1280, 1279, 720, and 719 CSS-pixel bars in the built desktop app. Confirm the same control instance remains focused across resize and that More search, regex, AWS, Multiverse, and Help focus return to the trigger. |
...
| Node Catalog dialog | The dialog's flex children could shrink together, compressing profiles and rows when many entries or bilingual descriptions were present; the profile and category rows were then height-capped (132 px / 82 px) and clipped their third row of chips mid-glyph. | Fixed sections are non-shrinking and the result list is the sole flexible height consumer. The profile and category rows are `ChipRow`s: never height-capped, a long profile tail folds behind a "+N more" chip. Rows keep intrinsic height and the dialog owns overflow. | Open all 73 entries with profiles at 100%, 125%, 150%, and 200% display scale. Confirm the first and last result, documentation actions, and keyboard hint remain reachable. |

...
Linux VM, Home Assistant control and sensor, Calendar, Timer, Alarm Clock, Planner,
...
Wild dim sum is a current entry: it creates a guided public-catalog node whose live network state
stays outside schema 3 while the validated selected dish remains portable.

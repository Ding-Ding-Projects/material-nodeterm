...
Everything on a nodeterm canvas is a node. The canvas includes document, service, and recovery
kinds, and every one is rendered by
...
| **Portal recovery** | A fixed-grid, offline recovery exercise with three energy keys, hazards, and an activation core. It stores only portable progress and never authenticates or bypasses a portal credential. See [Portal recovery game](./portal-recovery.md). |

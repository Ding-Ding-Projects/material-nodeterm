...
| **Managed Nextcloud** | A fixed no-socket PostgreSQL, Redis, and Nextcloud web stack with guided local binding, secret files, update, backup, restore, and rollback operations. See [Managed Nextcloud, no socket](../integrations/nextcloud-managed.md). |

...
Each observation carries its own status, timestamp, bounded detail, and bounded recovery reason.
...
record ids, connector ids, process ids, local configuration paths, host labels, timestamps, and all
observations stay outside the transferable project file. Credentials are not represented by either
the portable intent or the live state type.
...
- Shared model: `src/shared/tunnel-state.ts`.
- Guided display: `src/renderer/components/tunnel/TunnelStatePanel.tsx`.

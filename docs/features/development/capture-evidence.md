...
`docs/assets/shots/capture-manifest.json` is schema version 2. The `entries` map is authoritative: each capture id owns its file name, byte count, dimensions, hash, timestamp, tuple, commit, artifact version, bounded build kind, method, and optional launch receipt. The root `captured` array remains for older status readers.
...
`--launch` is retired. A target launched through the approved hidden-desktop route is attached without launching another application: `npm run shots -- --attach 9222 --receipt C:\path\to\capture-receipt.json --output-root C:\task-output --only app-settings-language`.

...
| Unified Node Catalog and special-universe Shops | Shared catalog and Shop implementation plus focused source contracts are present; built-artifact interaction, complete factory-route audit, and captures remain. | `manual:catalog and Shop source contracts pass and the built desktop evidence is still required` | `src/shared/node-catalog.ts`, `src/renderer/state/nodeCreationCoordinator.ts`, `src/core/universe-shop.ts`, `src/renderer/components/NodeCatalogDialog.tsx`, `src/renderer/nodes/ShopNode.tsx` |

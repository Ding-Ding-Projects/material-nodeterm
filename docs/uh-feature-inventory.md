...
| Nextcloud AIO hosting profile | Profile and fixed deployment plan are present; host-runtime execution and evidence remain open. | `contract:nextcloud-aio-profile` | `src/shared/nextcloud-aio.ts` and `src/renderer/components/nextcloud/NextcloudAioPanel.tsx` landed in issue #52's ultra-speed lane. The offline docs bundle, focused tests, packaged interaction, and captures still need the complete release pass. |

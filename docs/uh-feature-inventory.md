...
| Torrent downloader | Focused source Chuts, red-green parser and lifecycle checks, generated offline docs refresh, and full built-artifact interaction proof remain. | `manual:the downloader lane still needs the requested source and built-artifact verification before it can be marked shipped` | Bundled-only WebTorrent descriptor, consent boundary, safe metadata parsing, machine-local task service, typed bridges, and canvas node are present; this lane intentionally did not run general checks or captures. |
...
| Proxy and isolated debugging browser sessions | Focused behavior tests, built-artifact interaction, and capture evidence remain pending. | `manual:run focused browser-debug contract and lifecycle tests plus built-artifact interaction and captures before release` | `src/shared/browser-debug.ts`, `src/main/browser-debug-session.ts`, `src/renderer/nodes/DebugBrowserNode.tsx`, and `docs/features/browser/proxy-debug-sessions.md` implement the bounded proxy, fresh-profile, and loopback-only session contract. |

...
| Linux ISO VM node | `src/core/virtual-machine/manager.ts`, `src/renderer/nodes/VirtualMachineNode.tsx`, `dependencies.manifest.json`, `resources/qemu/README.md` | `docs/features/integrations/linux-iso-vm.md` | `src/core/virtual-machine/manager.test.ts` (written, not run in focused lane) |

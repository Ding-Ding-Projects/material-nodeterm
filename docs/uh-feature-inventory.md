...
| Cloudflare Access, Zero Trust, Workers, Pages, R2, D1 and Queues managers | `src/shared/cloudflare-zero-trust.ts`, `src/core/cloudflare-zero-trust/service.ts`, `src/renderer/components/cloudflare/CloudflareZeroTrustPanel.tsx` | `docs/features/integrations/cloudflare-zero-trust-managers.md` | `src/renderer/state/workspace.test.ts` (focused node-registration coverage; intentionally unrun in issue #58 ultra-speed lane) |
